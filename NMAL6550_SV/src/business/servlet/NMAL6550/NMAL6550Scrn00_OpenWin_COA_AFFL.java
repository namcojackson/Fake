...
public class NMAL6550Scrn00_OpenWin_COA_AFFL extends S21CommonHandler implements NMAL6550Constant {
    ...
    private List<EZDBStringItem> createNMAL6050Parameters(NMAL6550BMsg scrnMsg, int line) {
        ...
        paramItem.setValue(new COA_AFFLTMsg().getTableName());
        param.add(paramItem);
        ...
        paramItem.setValue(scrnMsg.xxTblNm_PP.getValue() + "_SORT_NUM");
        param.add(paramItem);

        // [4]:SCR_NM
        paramItem = scrnMsg.xxScrNm_PP;
        paramItem.setValue("COA Affiliate Search");
        param.add(paramItem);

        // [5]:HDR_CD_LB_NM
        paramItem = scrnMsg.xxHdrCdLbNm_PP;
        paramItem.setValue("COA Affiliate Code");
        param.add(paramItem);

        // [6]:HDR_NM_LB_NM
        paramItem = scrnMsg.xxHdrNmLbNm_PP;
        paramItem.setValue("COA Affiliate Name");
        param.add(paramItem);
        ...
        paramItem.setValue(scrnMsg.A.no(line).coaAfflCd_A1.getValue());
        param.add(paramItem);
