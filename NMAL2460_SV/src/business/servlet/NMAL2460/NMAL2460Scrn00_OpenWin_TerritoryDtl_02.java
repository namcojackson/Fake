 ...
 *</pre>
 */
public class NMAL2460Scrn00_OpenWin_TerritoryDtl_02 extends S21CommonHandler {

    @Override
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        NMAL2460BMsg scrnMsg = (NMAL2460BMsg) bMsg;

        int eventLine = this.getButtonSelectNumber();
        ZYPEZDItemValueSetter.setValue(scrnMsg.xxNum_EV, BigDecimal.valueOf(eventLine));

        if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(eventLine).orgNm_B2)) {
            scrnMsg.addCheckItem(scrnMsg.B.no(eventLine).orgNm_B2);
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        ZYPEZDItemValueSetter.setValue(scrnMsg.asgTrtyAttrbCd, ASG_TRTY_ATTRB.ATTRIBUTE2_ID);

        NMAL2460CommonLogic.clearParam(scrnMsg);
        // Mod Start 2017/11/28 QC#21044
        //Object[] params = NMAL2460CommonLogic.setParamForTerritoryPopupDtl(scrnMsg, scrnMsg.B.no(scrnMsg.xxNum_EV.getValueInt()).orgNm_B2);
        Object[] params = NMAL2460CommonLogic.setParamForTerritoryPopupDtl(scrnMsg, //
                scrnMsg.B.no(scrnMsg.xxNum_EV.getValueInt()).orgNm_B2, scrnMsg.slsCrQuotFlg_02);
