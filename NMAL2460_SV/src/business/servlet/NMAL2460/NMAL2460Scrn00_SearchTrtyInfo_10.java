 ...
 *</pre>
 */
public class NMAL2460Scrn00_SearchTrtyInfo_10 extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        ZYPEZDItemValueSetter.setValue(scrnMsg.xxNum_EV, new BigDecimal(eventLine));

        // Error if territory Name are not entered
        if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(eventLine).orgNm_BA)) {
            scrnMsg.B.no(eventLine).orgNm_BA.setErrorInfo(1, NMAM8368E, new String[] {MSG_PARAM_TRTY_NM });
            scrnMsg.addCheckItem(scrnMsg.B.no(eventLine).orgNm_BA);
            scrnMsg.putErrorScreen();
        }

        ZYPEZDItemValueSetter.setValue(scrnMsg.asgTrtyAttrbCd, ASG_TRTY_ATTRB.ATTRIBUTE10_ID);
    }
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        scrnMsg.setFocusItem(scrnMsg.B.no(scrnMsg.xxNum_EV.getValueInt()).orgNm_BA);
    }
}
