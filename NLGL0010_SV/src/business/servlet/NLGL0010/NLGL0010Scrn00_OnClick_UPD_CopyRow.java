 ...
 *</pre>
 */
public class NLGL0010Scrn00_OnClick_UPD_CopyRow extends S21CommonHandler implements NLGL0010Constant {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (scrnMsg.O.getValidCount() == scrnMsg.O.length()) {
            scrnMsg.setMessageInfo(NLGM0054E, new String[] {NAME_FOR_MESSAGE_LIST });
        }

        if (scrnMsg.O.getValidCount() == FLD_VALUE_INT_ZERO) {
        ...
        if (outGetSelectedRows.size() > 1) {
            ...
            for (int i = 0; i < scrnMsg.O.getValidCount(); i++) {
                ...
                if (ZYPConstant.CHKBOX_ON_Y.equals(scrnMsg.O.no(i).xxChkBox_O1.getValue())) {
                    scrnMsg.addCheckItem(scrnMsg.O.no(i).xxChkBox_O1);
                    scrnMsg.O.no(i).xxChkBox_O1.setErrorInfo(FLD_VALUE_INT_1, NLGM0035E);
                }
            }
        }
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        NLGL0010CommonLogic.busUPDControl_Copy(scrnMsg);
    }
}
