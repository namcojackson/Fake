 ...
 *</pre>
 */
public class NLGL0010Scrn00_OnClick_DNLD_DeleteRow extends S21CommonHandler implements NLGL0010Constant {

    @Override
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {

        NLGL0010BMsg scrnMsg = (NLGL0010BMsg) bMsg;
        List<Integer> outGetSelectedRows = ZYPTableUtil.getSelectedRows(scrnMsg.K, FIELD_NAME_XXCHKBOX_K1, ZYPConstant.CHKBOX_ON_Y);

        if (outGetSelectedRows.isEmpty()) {

            for (int i = 0; i < scrnMsg.K.getValidCount(); i++) {
                scrnMsg.addCheckItem(scrnMsg.K.no(i).xxChkBox_K1);
                scrnMsg.K.no(i).xxChkBox_K1.setErrorInfo(FLD_VALUE_INT_1, NLGM0036E);
            }
        }
