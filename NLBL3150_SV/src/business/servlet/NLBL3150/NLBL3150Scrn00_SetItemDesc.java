 ...
 *</pre>
 */
public class NLBL3150Scrn00_SetItemDesc extends S21CommonHandler {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        scrnMsg.addCheckItem(scrnMsg.A.no(scrnMsg.xxCellIdx.getValueInt()).mdseCd_A);
        scrnMsg.putErrorScreen();

        if (scrnMsg.getMessageType() == EZDMessageInfo.MSGTYPE_ERROR) {
            throw new EZDFieldErrorException();
        }
        scrnMsg.setFocusItem(scrnMsg.A.no(scrnMsg.xxCellIdx.getValueInt()).mdseCd_A);
