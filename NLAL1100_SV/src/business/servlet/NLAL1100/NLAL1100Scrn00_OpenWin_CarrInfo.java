 ...
 *</pre>
 */
public class NLAL1100Scrn00_OpenWin_CarrInfo extends S21CommonHandler {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        ZYPEZDItemValueSetter.setValue(scrnMsg.xxPopPrm_EV, NLAL1100Constant.EVENT_NM_OPENWIN_CARR_LINE);
    ...
    private String getSelectSQL(NLAL1100BMsg scrnMsg, int index) {
        ...
        if (ZYPCommonFunc.hasValue(scrnMsg.A.no(index).carrAcctNum_A1)) {
            ...
            sb.append("            WHERE").append("\n");
            sb.append("                AC.GLBL_CMPY_CD     = OC.GLBL_CMPY_CD").append("\n");
            sb.append("            AND AC.VND_CD           = OC.CARR_CD").append("\n");
            sb.append("            AND AC.DS_CARR_ACCT_NUM = '#DS_CARR_ACCT_NUM#'").append("\n");
            sb.append("            AND AC.DS_ACCT_NUM      = '#DS_ACCT_NUM#'").append("\n");
            sb.append("            AND AC.EFF_FROM_DT     <= '#SLS_DT#'").append("\n");
            sb.append("            AND (AC.EFF_THRU_DT    >= '#SLS_DT#' OR AC.EFF_THRU_DT IS NULL)").append("\n");
        ...
        if (ZYPCommonFunc.hasValue(scrnMsg.A.no(index).carrAcctNum_A1)) {
            ...
            sql = sql.replaceAll("#DS_ACCT_NUM#", scrnMsg.A.no(index).dsAcctNum_A1.getValue());
        }
