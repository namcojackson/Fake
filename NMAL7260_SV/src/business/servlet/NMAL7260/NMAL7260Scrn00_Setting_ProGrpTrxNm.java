...
/**
 *<pre>
 * NMAL7260Scrn00_Setting_ProGrpTrxNm
 *
 ...
 *</pre>
 */
public class NMAL7260Scrn00_Setting_ProGrpTrxNm extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(idx).prcRuleCondFromTxt_14)) {
            scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_14);
        } else {
            if (!ZYPCommonFunc.isNumberCheck(scrnMsg.B.no(idx).prcRuleCondFromTxt_14.getValue())) {
                scrnMsg.B.no(idx).prcRuleCondFromTxt_14.setErrorInfo(1, ZZM9004E, new String[] {scrnMsg.B.no(idx).prcRuleCondFromTxt_14.getNameForMessage()});
                scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_14);
            }
        }
