 ...
 *</pre>
 */
public class NMAL7260Scrn00_OnBlur_Setting_PrcGrpMatQtyBrkNm extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(idx).prcRuleCondFromTxt_57)) {
            scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_57);
        } else {
            if (!ZYPCommonFunc.isNumberCheck(scrnMsg.B.no(idx).prcRuleCondFromTxt_57.getValue())) {
                scrnMsg.B.no(idx).prcRuleCondFromTxt_57.setErrorInfo(1, ZZM9004E, new String[] {scrnMsg.B.no(idx).prcRuleCondFromTxt_05.getNameForMessage() });
                scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_57);
            }
        }
