...
/**
 *<pre>
 * NMAL7260Scrn00_OnBlur_Setting_FreightZone
 *
 ...
 *</pre>
 */
public class NMAL7260Scrn00_OnBlur_Setting_FreightZone extends S21CommonHandler {
...
//        if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(idx).prcRuleCondFromTxt_49)) {
//            scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_49);
//        }
        if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(idx).prcRuleCondFromTxt_49)) {
            scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_49);
        } else {
            if (!ZYPCommonFunc.isNumberCheck(scrnMsg.B.no(idx).prcRuleCondFromTxt_49.getValue())) {
                scrnMsg.B.no(idx).prcRuleCondFromTxt_49.setErrorInfo(1, ZZM9004E, new String[] {scrnMsg.B.no(idx).prcRuleCondFromTxt_49.getNameForMessage() });
                scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_49);
            }
        }
