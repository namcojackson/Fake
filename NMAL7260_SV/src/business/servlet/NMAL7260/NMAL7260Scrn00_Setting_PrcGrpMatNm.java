...
/**
 *<pre>
 * NMAL7260Scrn00_Setting_PrcGrpMatNm
 *
 ...
 *</pre>
 */
public class NMAL7260Scrn00_Setting_PrcGrpMatNm extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(idx).prcRuleCondFromTxt_05)) {
            scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_05);
        } else {
            if (!ZYPCommonFunc.isNumberCheck(scrnMsg.B.no(idx).prcRuleCondFromTxt_05.getValue())) {
                scrnMsg.B.no(idx).prcRuleCondFromTxt_05.setErrorInfo(1, ZZM9004E, new String[] {scrnMsg.B.no(idx).prcRuleCondFromTxt_05.getNameForMessage()});
                scrnMsg.addCheckItem(scrnMsg.B.no(idx).prcRuleCondFromTxt_05);
            }
        }
