<!-- $Header: canon_E193_csTaxIssue.jsp $ -->
<%--========================================================================
 |
 | FILE 
 |	canon_E193_csTaxIssue.jsp - Tax issues.
 |   
 | DESCRIPTION
...
<%
	...
	if (hPath == null)
		...
		if(hPath.indexOf(strTaxIssueT1) < 0)
			hPath = hPath + " -> " + strTaxIssueT1;
...
<script language="JavaScript">
	...
	function action_func1() {
		...
		var controlForm = '<%=strTaxCharge%>';
		if(controlForm == '') {
			if(!objForm.taxCharge[0].checked && !objForm.taxCharge[1].checked) {
				displayErrorMsg("Please select charged");				
			}else{
				objForm.action = "canon_E193_csBTaxIssue.jsp";
				objForm.submit();
			}
		}else{
			if(objForm.iLineId.value != '' && objForm.iLineId.value != '0') {
				...
				var v = '';
				var notes = objForm.notes.value;
				if(controlForm == 'Y' && objForm.expTaxAmt != null) 
					v = objForm.expTaxAmt;
				//else if(controlForm == 'N') v = objForm.newTaxAmt;
				if(controlForm == 'Y' && !objForm.taxExemption[0].checked && !objForm.taxExemption[1].checked){
					displayErrorMsg("Please select tax exemption");
				}else if(objForm.reasonCdDesc.value == '') {
					...
					displayErrorMsg("Total Size of Notes for an Issue Line is restricted to 32000 characters. Please use the attachment facility to add additional details.");
					objForm.notes.focus();
				}else if(controlForm == 'Y' && (v.value == '' || isNaN(v.value))) {
					displayErrorMsg("please enter valid amount");
					v.focus();
				}else{
					objForm.submit();
				}
...
%> 
  ...
  <div id="main_content">
	<div id="page_top">
		<h1 style="padding-top:20px;" id="headerText">Customer Care - <%=strTaxIssueT1 %></h1>
		<div class="breadcrumb"><%=hPath%></div>
	</div>	
	<form name="taxIssueForm" action="canon_E193_csTicketSummaryB2Controller.jsp" method="post">

	<input type="hidden" name="hPageFrom" value="TaxIssue">
	...
	<input type="hidden" name="iLineId"  id="iLineId"  id="iLineId" value="<%=request.getParameter("iLineId")==null?"":request.getParameter("iLineId")%>">
	...
	<input type="hidden" name="iTaxOrig" value="<%=strTaxOrig%>">
	...
	<table class="request-service" style="align:center;" cellspacing="0" cellpadding="1" border="0">			
	<% if(objSessionAcctInfo != null) { %>
		...
		<!-- <tr><td height="10" colspan="3"></td></tr> -->
   		<tr>
   		         <td  colspan="3"><font class="promptReadOnly">Selected Invoice: <a href="#" onClick="javascript:void InvoiceDetailsPDF('<%=strInvNo%>');"><%=strInvNo%></a><%=strInvoiceNoN1%></font></td>
   			<%-- <td  colspan="3"><font class="promptReadOnly">Selected Invoice: <a href="#" onClick="javascript:InvoiceLineDetailsPDF('<%=strInvNo%>');"><%=strInvNo%></a><%=strInvoiceNoN1%></font></td> --%>
   		</tr>
		<!-- <tr><td height="10" colspan="3"></td></tr> -->
   		<tr>
   			<td class="tableQuestionCell" colspan="3">
   				<font class="promptReadOnly">
   					<b><%=strTaxIssueQ1%></b>
   				</font>
   			</td>
   		</tr>
   		<%
   			String strYDisabled = "", strNDisabled = "";
   			if("Y".equals(strTaxCharge)) strNDisabled = "disabled";
   			else if("N".equals(strTaxCharge)) strYDisabled = "disabled";
   		%>
		<tr>
			<td width="10">&nbsp;</td>
			<td align="left" colspan="2">			
				<input type="radio" name="taxCharge" value="Y" <%="Y".equalsIgnoreCase(strTaxCharge)?"checked":strYDisabled%>>&nbsp;&nbsp;&nbsp;<font class="promptReadOnly">Yes, and the customer feels they should not be charged</font>
			</td>
		</tr>
		<tr>
			<td width="10">&nbsp;</td>
			<td align="left" colspan="2">
				<input type="radio" name="taxCharge" value="N" <%="N".equalsIgnoreCase(strTaxCharge)?"checked":strNDisabled%>>&nbsp;&nbsp;&nbsp;<font class="promptReadOnly">No, and the customer needs to be charged</font>
			</td>
		</tr>  		
	<%
	...
	%>
			...
			<tr>
				<td width="10">&nbsp;</td>
				<td align="left">
					<font class="promptReadOnly">Existing Tax</font>
				</td>
				<td align="left">
			...
			<tr>
				...
				<td align="left" >
					<input name="expTaxAmt" type="text" value="<%=request.getParameter("expTaxAmt")==null?"":request.getParameter("expTaxAmt")%>" size="12" maxlength="30" class="inTxt">
				</td>
			</tr>  		
			<!-- <tr><td height="10" colspan="3"></td></tr> -->
	   		<tr>
			...
			<tr>
				<td width="10">&nbsp;</td>
				<td align="left" colspan="2">			
					<input type="radio" name="taxExemption" value="Y" <%="Y".equalsIgnoreCase(strTExep)?"checked":""%>>&nbsp;&nbsp;&nbsp;<font class="promptReadOnly">Yes, please send us a copy</font>
				</td>
			</tr>
			<tr>
				<td width="10">&nbsp;</td>
				<td align="left" colspan="2">
					<input type="radio" name="taxExemption" value="N" <%="N".equalsIgnoreCase(strTExep)?"checked":""%>>&nbsp;&nbsp;&nbsp;<font class="promptReadOnly">No</font>
				</td>
			</tr>
  	<%
		}
		//else if("N".equals(strTaxCharge)) {
  	%><%--
	   		<tr>
	   			<td class="tableQuestionCell" colspan="3">
	   				<font class="promptReadOnly">
			...
			<tr>
				<td width="10">&nbsp;</td>	
				<td align="left">
					<font class="promptReadOnly">New Tax Amount</font>
				</td>
				<td align="left" >
					<input name="newTaxAmt" type="text" value="<%=request.getParameter("expTaxAmt")==null?"":request.getParameter("expTaxAmt")%>" size="12" maxlength="30" class="inTxt">
				</td>
			</tr>  		
  	--%><%
  		//}
  		if(!"".equals(strTaxCharge)) {
  	%>
