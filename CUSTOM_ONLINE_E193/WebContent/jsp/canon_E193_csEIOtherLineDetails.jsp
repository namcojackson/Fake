<!-- $Header: canon_E193_csEIOtherLineDetails.jsp $ -->
<%--========================================================================
 |
 | FILE 
 |	canon_E193_csEIOtherLineDetails.jsp - ARManula/Misc/Merchandise line details
 |   
 | DESCRIPTION
...
<script language="JavaScript">
	function action_func1() {
		resetErroMsg();
		var objForm = document.otherLineForm;
	...
	function fnGetReasonCode() {
		 showPleaseWait();	
			...
			var val = document.otherLineForm.hParentCatCode.value
		      var modelName ="#dlg";
		 	   $(modelName).dialog({
							height: 500,
							title: "Reason Code", 
	...
	function fnValidateCheckbox(objForm) {
		...
		if(len > 0) {
			// Commente the below lines for test
			/* for(i=0; i<len; i++) {
			 ...
			 */
			for(i=0; i<len; i++) {
				if((objForm.supplyChkCredit[i].checked && objForm.supplyChkCredit[i].value == '') ||
					(objForm.supplyChkCredit[i].checked && objForm.supplyChkCredit[i].value != '')) {
					if(objForm.supplyChkCredit[i].checked && objForm.supplyChkCredit[i].value == '') {
						errMsg = "Please select a credit reason";
						objForm.supplyChkCredit[i].focus();
					}
					isChecked = false;
					break;
				}
		...
		else if((objForm.supplyChkCredit != null && objForm.supplyChkCredit.checked)) isChecked = false;
		
		if(isChecked) {
			 displayErrorMsg("please select a credit");
	...
	function fnValidateField(objForm) {
		...
		if(objForm.supplyCreditAmt != null) 
			len = objForm.supplyCreditAmt.length;
...
%>
<%
	String strOtherLineDetailsTitle = "";
	if("AR MANUAL".equals(strInvSource))
		 strOtherLineDetailsTitle = strOtherLineDetailsT1;
	else if("MERCHANDIS".equals(strInvSource)) 
		 strOtherLineDetailsTitle = strOtherLineDetailsT2;
	else if("MISC".equals(strInvSource)) 
		strOtherLineDetailsTitle = strOtherLineDetailsT3;
	
	String hPath = request.getParameter("hPath");
	if (hPath == null)
		hPath = strOtherLineDetailsTitle;
	else
		if(hPath.indexOf(strOtherLineDetailsTitle) < 0)
			hPath = hPath + " -> " + strOtherLineDetailsTitle;
				
%>	
 
 <div id="main_content">
	<div id="page_top">
		<h1 style="padding-top:20px;" id="headerText">Customer Care - <%=strOtherLineDetailsTitle%></h1>
		<div class="breadcrumb"><%=hPath%></div>		
	</div>	
	<form name="otherLineForm" id="otherLineForm" action="canon_E193_csTicketSummaryB1Controller.jsp" method="post">
	...
	<input type="hidden" name="hParentCatCode" id="hParentCatCode" value="<%=request.getParameter("hParentCatCode")%>">
	...
	<% if(objSessionAcctInfo != null) { %>
		...
		<tr><td height="10" colspan="3"></td></tr>
  		<tr>
  			...
  			<div style="overflow:auto; width:1370px; height:250;">
  			<% } %>
  				<table cellspacing="1" class="customer-care">
  					<tr>
  						<th>
		...
		<tr><td height="10" colspan="3"></td></tr>
  		<tr>
  			...
  			<td colspan="3">
  				<table cellspacing="1" class="customer-care">
  					<tr>
  						<th><a href="javascript:void(0)" onClick="javascript:fnGetReasonCode();">Reason Code</a><%=strReasonCodeN1%></th>
  						<th>Urgency</th>
  					</tr>
  					<tr >
  						<td><input type="text" name="reasonCdDesc" id="reasonCdDesc" onClick="javascript:fnGetReasonCode();" value="<%=request.getParameter("reasonCdDesc")==null?"":request.getParameter("reasonCdDesc")%>" class="inTxt required" style="background-color:#ffff00" size="80" readonly=true></td>
  						<td>
