<!-- $Header: canon_E193_csBFreightIssue.jsp $ -->
<%--========================================================================
 |
 | FILE 
 |	canon_E193_csBFreightIssue.jsp - Freight issues.
 |   
 | DESCRIPTION
...
<%
	...
	if (hPath == null)
		hPath = strFreightIssueT1;
	else
		if(hPath.indexOf(strFreightIssueT1) < 0)
			hPath = hPath + " -> " + strFreightIssueT1;	
%>
...
<script language="JavaScript">
	...
	/*function fnGetReasonCode() {
		var val = document.freightIssueForm.hParentCatCode.value;;
	...
	function action_func1() {
		resetErroMsg();
		var objForm = document.freightIssueForm;
		if(objForm.iLineId.value != '' && objForm.iLineId.value != '0') {
			...
			var v = objForm.expFreightAmt;
			var notes = objForm.notes.value;
			if(v.value == '' || isNaN(v.value)) {
				displayErrorMsg("please enter valid freight amount");
				v.focus();
...
%> 
	<div id="main_content">
		<div id="page_top">
			<h1 style="padding-top:20px;" id="headerText">Customer Care - <%=strFreightIssueT1%></h1>
			<div class="breadcrumb"><%=hPath%></div>
		</div>
	
	<form name="freightIssueForm" action="canon_E193_csTicketSummaryB2Controller.jsp" method="post">

	<input type="hidden" name="hPageFrom" value="BFreightIssue">
	...
	<input type="hidden" name="hCatId" id="hCatId" value="<%=request.getParameter("hCatId")%>">
	<input type="hidden" name="hParentCatId" id="hParentCatId" value="<%=request.getParameter("hParentCatId")%>">
	...
	<input type="hidden" name="iFreightOrig" value="<%=strFreightOrig%>">
	...
	<% if(objSessionAcctInfo != null) { %>
		...
		<tr>
			<td width="10">&nbsp;</td>
			<td align="left">
				<font class="promptReadOnly">Existing Freight</font>
			</td>
			<td align="left">
		...
		<tr>
			...
			<td align="left" >
				<input name="expFreightAmt" type="text" value="<%=request.getParameter("expFreightAmt")==null?"":request.getParameter("expFreightAmt")%>" size="12" maxlength="30" class="inTxt">
			</td>
		</tr>  		
		<tr><td height="10" colspan="3"></td></tr>
   		<tr>
