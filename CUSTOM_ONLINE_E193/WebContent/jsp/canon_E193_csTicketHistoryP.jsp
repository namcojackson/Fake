<!-- $Header: canon_E193_csTicketHistory.jsp $ -->
<%--========================================================================
 |
 | FILE 
...
<script language="JavaScript">
	...
	function TicketStatus(tktVal, accessVal) {
		...
		if(accessVal == 1) {
			...
			displayErrorMsg("Permission denied");
		}
		else {
	...
	<tr> 
		<td class="tdTableCellStyle" colspan="2">
			<table cellspacing="1" class="request-service">
				<tr>
	   				<td width="100"><font class="promptReadOnly">Tickets History</font></td>
	   				<td>
	    	...
	    	<table cellspacing="1" class="request-service">
				...
				%>
						   	<tr class="tdTableCellStyle">
				               	<td><a href="javascript:void TicketStatus('<%=objTicketHeader.getTicketNo()%>', <%=tktSummaryNoAccess%>);"><%= objTicketHeader.getTicketNo()==null?"":objTicketHeader.getTicketNo()%></a></td>
				...
				<%
					...
					%>
									&nbsp;<a class="btn_red" href="javascript:action_func2(<%=j%>);"><%=j%></a>
					<%
								}
							} // close for loop
					%>
