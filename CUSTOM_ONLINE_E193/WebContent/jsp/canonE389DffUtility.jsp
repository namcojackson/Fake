<!-- $Header: canonE389DffUtility.jsp $ -->
<%--========================================================================
 |
 | FILE
 | canonE389DffUtility.jsp - JSP DFF Utility.
 |
 | DESCRIPTION
...
<% String ctxPath = request.getContextPath(); %>	
	...
	<!--<link href="/s21extn/common/jquery/jquery-ui.css" rel='stylesheet' type='text/css' />-->
...
<%@ include file="canon_E193_csCheckSessionInc.jsp" %>
      ...
      <jsp:useBean class="com.canon.oracle.custapp.custinq.beans.CanonE389DffUtilityBean" scope="request" id="CanonE389DffUtilityBean" />
     <% //End changes for S21 by Mangala%>
      <jsp:setProperty name="CanonE389DffUtilityBean" property="*" />
      <%
      ...
      $(document).ready(function() {
        ...
        {
          document.forms['canonE389DffUtility'].elements[attrId].value = finalValue;
          document.forms['canonE389DffUtility'].elements[attrVal].value = finalValue;
        }
		
        function OpenLinkDetails(documentId) {
          var vLink = "canonE389ActualDocumentLink.jsp?DocumentId=" + documentId;
      ...
      <form action="canonE389DffUtility.jsp" name="canonE389DffUtility" id="canonE389DffUtility" method="post">
      <p id="msg"></p>
        <%
          java.util.ArrayList alDffResults = CanonE389DffUtilityBean.getDffDetails(sourceContext);
        //System.out.println("alDffResults = "+alDffResults);
          CanonE389DffDataBean DBInsert    = new CanonE389DffDataBean();

          int cnt = alDffResults.size();
          System.out.println("alDffResults cnt = "+cnt);
          ...
          if (Mode != null && Mode.equals("insert"))
          {
            ...
            for (int idx = 0; idx < cnt; idx++)
            {
              ...
              //System.out.println("E389 Values -------> : " +AttrIds +" : " +AttrValues);
            ...
            if (vAttrIds.size() > 0)
            {
              System.out.println("E389 context " +sourceApplication+" "+sourceContext+" "+contextValue+" "+userId);
        ...
        <%
          ...
          vDisplayData = CanonE389DffDataBean.displayData (sourceApplication, sourceContext, contextValue);
        ...
        <%
          ...
          for (int i = 0; i < alDffResults.size(); i++)
          {
            ...
            CanonE389DffUtilityBean objdff = (CanonE389DffUtilityBean)alDffResults.get(i);
            CanonE389DffDataBean objData = new CanonE389DffDataBean();
            objData = (CanonE389DffDataBean)vDisplayData.elementAt(i);
            ...
            <td width="50%" align="left" class="<%= bclr %>">
			  ...
			  <% } else if (defaultValue.equals("LINK")) { %>
                   ...
                   <%} else { %>
                     ...
                     <% } else { %>
                        ...
                        <% } else { %> 
                          <a href="javascript:void(0);" style="color: blue;" onclick="NoPrompt();OpenLinkDetails('<%=blankNull(vAttrValue)%>');"><b>Click Here...</b></a>
                        <% } %>
                     <% } %>
                   <% } %>
              ...
              <% } else if (defaultValue.equals("LOV")) { %>
                   <% if (securityEnabledFlag.equals("Y")) { int dSize = (Integer.parseInt(displaySize)*8); %>
                     <%=CanonE389DffUtilityBean.valueSetLovHTML(attributeId, valueSetName, vAttrValue, securityEnabledFlag, enabledFlag, dSize) %>
              ...
              <% } else if (defaultValue.equals("MINUTES")) { %>
				   ...
				   <input type="hidden" name="<%=attributeId%>" id="<%=attributeId%>" value="<%=vAttrValue%>">
                     <%=CanonE389DffUtilityBean.valueSetLovHTML("hours"+i, "CANON_E389_HOURS",h , securityEnabledFlag, enabledFlag, dSize) %>
					 <b>&nbsp:&nbsp</b>
                     <%=CanonE389DffUtilityBean.valueSetLovHTML("minutes"+i, "CANON_E389_MINUTES", m, securityEnabledFlag, enabledFlag, dSize) %>
        ...
        <table width="100%" border="0" cellspacing="6" cellpadding="3">
          ...
          <% } else { %>
            ...
            <a class="btn_red" href="#" name="Save" id="Save" onclick="NoPrompt();checkAllData('canonE389DffUtility');">Save</a>
            
            </td>

            <td width="50%" align="left"> 
            <!-- <input type="button" name="Close" id="Close" size="15" value="&nbsp;&nbsp;&nbsp;&nbsp;Close&nbsp;&nbsp;&nbsp;&nbsp;"   onClick="javascript:window.close();" /> -->
                <a name="Close" id="Close" class="btn_red" href="javascript:void(0)" onclick="javascript: $(this).closest('.ui-dialog').find('.ui-dialog-titlebar-close').trigger('click');" >Close</a> 
            </td>
          <% } %>
