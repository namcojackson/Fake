 ...
 *</pre>
 */
public class NPAL1230Scrn00_GetItemInfo extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectNum).mdseCd_A)) {
            scrnMsg.A.no(selectNum).mdseCd_A.setErrorInfo(1, ZZM9000E, new String[] {"Item#", " Please select." });
            scrnMsg.addCheckItem(scrnMsg.A.no(selectNum).mdseCd_A);
        }
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (scrnMsg.A.no(selectNum).mdseCd_A.isError()) {
            scrnMsg.addCheckItem(scrnMsg.A.no(selectNum).mdseCd_A);
            scrnMsg.putErrorScreen();
        } else {
            // set focus
