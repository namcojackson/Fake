 ...
 *</pre>
 */
public class NSAL0720Scrn00_SelectBllgMtrAll extends S21CommonHandler {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (CHKBOX_ON_Y.equals(scrnMsg.xxChkBox_BL.getValue())) {
            checkFlg = true;
        }
        for (int i = 0; i < scrnMsg.A.getValidCount(); i++) {
            if (checkFlg) {
                selectAll(scrnMsg.A, "xxChkBox_A2", CHKBOX_ON_Y);
            } else {
                unSelectAll(scrnMsg.A, "xxChkBox_A2");
            }
        }
    }
}
