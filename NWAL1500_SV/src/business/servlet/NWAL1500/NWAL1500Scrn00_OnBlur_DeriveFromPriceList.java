 ...
 * </pre>
 */
public class NWAL1500Scrn00_OnBlur_DeriveFromPriceList extends S21CommonHandler {
    ...
    protected EZDCMsg setRequestData(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(scrnMsg.xxDplyTab.getValue())) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(selectIndex).prcCatgNm_LL)) {
                return null;
            }
        } else if (TAB_RMA.equals(scrnMsg.xxDplyTab.getValue())) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.D.no(selectIndex).prcCatgNm_RL)) {
                return null;
            }
        } else {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(scrnMsg.xxDplyTab.getValue())) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.B.no(selectIndex).prcCatgNm_LL)) {
                scrnMsg.setFocusItem(scrnMsg.B.no(selectIndex).flPrcListNm_LL);
                return;
            }
        } else if (TAB_RMA.equals(scrnMsg.xxDplyTab.getValue())) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.D.no(selectIndex).prcCatgNm_RL)) {
                scrnMsg.setFocusItem(scrnMsg.D.no(selectIndex).flPrcListNm_RL);
                return;
            }
        } else {
            if (!ZYPCommonFunc.hasValue(scrnMsg.prcCatgNm)) {
        ...
        if (TAB_LINE_CONFIG.equals(scrnMsg.xxDplyTab.getValue())) {
            scrnMsg.addCheckItem(scrnMsg.B.no(selectIndex).prcCatgNm_LL);
        } else if (TAB_RMA.equals(scrnMsg.xxDplyTab.getValue())) {
            scrnMsg.addCheckItem(scrnMsg.D.no(selectIndex).prcCatgNm_RL);
        } else {
        ...
        if (ZYPConstant.FLG_ON_Y.equals(scrnMsg.xxRqstFlg.getValue())) {
            ...
            Object[] params = NWAL1500CommonLogic.getParamNWAL1760(scrnMsg, PRC_CTX_TP.SALES_PRICING, true);
