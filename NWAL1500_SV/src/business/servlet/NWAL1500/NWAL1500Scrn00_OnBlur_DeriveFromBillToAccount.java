 ...
 *</pre>
 */
public class NWAL1500Scrn00_OnBlur_DeriveFromBillToAccount extends S21CommonHandler {
    ...
    protected EZDCMsg setRequestData(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).billToCustAcctCd_LC)) {
                return null;
            }
        } else if (TAB_RMA.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.C.no(selectIndex).billToCustAcctCd_RC)) {
                return null;
            }
        } else {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).billToCustAcctCd_LC)) {
                scrnMsg.A.no(selectIndex).billToCustAcctNm_LC.clear();
                scrnMsg.A.no(selectIndex).billToCustCd_LC.clear();
                scrnMsg.A.no(selectIndex).billToCustLocAddr_LC.clear();
                scrnMsg.setFocusItem(scrnMsg.A.no(selectIndex).billToCustCd_LC);
                return;
            }
        } else if (TAB_RMA.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.C.no(selectIndex).billToCustAcctCd_RC)) {
                scrnMsg.C.no(selectIndex).billToCustAcctNm_RC.clear();
                scrnMsg.C.no(selectIndex).billToCustCd_RC.clear();
                scrnMsg.C.no(selectIndex).billToCustLocAddr_RC.clear();
                scrnMsg.setFocusItem(scrnMsg.C.no(selectIndex).billToCustCd_RC);
                return;
            }
        } else {
            if (!ZYPCommonFunc.hasValue(scrnMsg.billToCustAcctCd)) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            scrnMsg.addCheckItem(scrnMsg.A.no(selectIndex).billToCustAcctCd_LC);
        } else if (TAB_RMA.equals(dplyTab)) {
            scrnMsg.addCheckItem(scrnMsg.C.no(selectIndex).billToCustAcctCd_RC);
        } else {
