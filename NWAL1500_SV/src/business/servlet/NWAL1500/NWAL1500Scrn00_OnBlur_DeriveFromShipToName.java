 ...
 * 2016/02/09   Fujitsu         H.Mikio         Create          QC#1636
 ...
 *</pre>
 */
public class NWAL1500Scrn00_OnBlur_DeriveFromShipToName extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            strItem = scrnMsg.A.no(selectIndex).shipToCustAcctNm_LC;
        } else if (TAB_RMA.equals(dplyTab)) {
            strItem = scrnMsg.C.no(selectIndex).shipToCustAcctNm_RC;
        } else {
            strItem = scrnMsg.shipToCustAcctNm;
        }
    ...
    protected EZDCMsg setRequestData(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).shipToCustAcctNm_LC)) {
                return null;
            }
        } else if (TAB_RMA.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.C.no(selectIndex).shipToCustAcctNm_RC)) {
                return null;
            }
        } else {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).shipToCustAcctNm_LC)) {
                ...
                scrnMsg.A.no(selectIndex).shipToCustCd_LC.clear();
            ...
            if (!ZYPCommonFunc.hasValue(scrnMsg.C.no(selectIndex).shipToCustAcctNm_RC)) {
                ...
                scrnMsg.C.no(selectIndex).shipToCustCd_RC.clear();
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            scrnMsg.addCheckItem(scrnMsg.A.no(selectIndex).shipToCustAcctNm_LC);
        } else if (TAB_RMA.equals(dplyTab)) {
            scrnMsg.addCheckItem(scrnMsg.C.no(selectIndex).shipToCustAcctNm_RC);
        } else {
