 ...
 *</pre>
 */
public class NWAL1500Scrn00_OnBlur_DeriveFromShipToAccount extends S21CommonHandler {
    ...
    protected EZDCMsg setRequestData(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).shipToCustAcctCd_LC)) {
                return null;
            }
        } else if (TAB_RMA.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.C.no(selectIndex).shipToCustAcctCd_RC)) {
                return null;
            }
        } else {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).shipToCustAcctCd_LC)) {
                scrnMsg.A.no(selectIndex).shipToCustAcctNm_LC.clear();
            ...
            if (!ZYPCommonFunc.hasValue(scrnMsg.C.no(selectIndex).shipToCustAcctCd_RC)) {
                scrnMsg.C.no(selectIndex).shipToCustAcctNm_RC.clear();
                scrnMsg.C.no(selectIndex).shipToCustCd_RC.clear();
                scrnMsg.C.no(selectIndex).shipToCustLocAddr_RC.clear();
                scrnMsg.setFocusItem(scrnMsg.C.no(selectIndex).shipToCustCd_RC);
                return;
            }
        } else {
            if (!ZYPCommonFunc.hasValue(scrnMsg.shipToCustAcctCd)) {
        ...
        if (TAB_LINE_CONFIG.equals(dplyTab)) {
            scrnMsg.addCheckItem(scrnMsg.A.no(selectIndex).shipToCustAcctCd_LC);
        } else if (TAB_RMA.equals(dplyTab)) {
            scrnMsg.addCheckItem(scrnMsg.C.no(selectIndex).shipToCustAcctCd_RC);
        } else {
