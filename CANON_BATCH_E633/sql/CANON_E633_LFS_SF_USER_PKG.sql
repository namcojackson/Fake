CREATE OR REPLACE PACKAGE canon_e633_lfs_sf_user_pkg
AS
TYPE x_role_data IS REF CURSOR ;
G_PACKAGE_NAME    VARCHAR2(30) := 'CANON_E633_LFS_SF_USER_PKG';
...
END CANON_e633_lfs_sf_user_pkg;
/

CREATE OR REPLACE PACKAGE BODY canon_e633_lfs_sf_user_pkg
AS
...
AS
    ...
    SELECT distinct psn.psn_num employee_number--employee_number
       ...
       -- ,psn.eml_addr || '.lfs' email_address --|| '.example.com' email_address ---THIS HAS TO BE CHANGED WHEN GOING TO PRD INSTANCE
		,psn.eml_addr email_address --QC51308
        ,psn.hr_ttl_nm job_title --job title
        ,(CASE WHEN psn.psn_last_nm LIKE 'OPEN TERR%' OR psn.psn_first_nm LIKE 'OPEN TERR%' --QC51560
             THEN
               'TRUE' --NULL QC52566
             ELSE
               'TRUE'
...
--                     ,ds_org_unit_rg dou
                ...
                WHERE ofa.glbl_cmpy_cd = 'ADB'
                  ...
                  AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(ofa.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(ofa.eff_THRU_DT, 'yyyymmdd'),SYSDATE))                  AND dou.org_cd = t.org_cd
...
--                    and TRUNC(SYSDATE) BETWEEN TRUNC(NVL(val_tbl.val4'dd-MON-yyyy'),SYSDATE)) AND TRUNC(NVL(to_date(val_tbl.val5, 'dd-MON-yyyy'),SYSDATE))
    ...
    --to make sure if the PSN is not already in user mapping
     AND NOT EXISTS (SELECT '1'
                     FROM canon_e633_lfs_user_map_tbl e633
...
CURSOR updt_user_cur IS
     SELECT psn.psn_cd rid
        ...
        --   ,psn.eml_addr || '.lfs' emailAddr --|| '.example.com' emailAddr --THIS HAS TO BE CHANGED FOR PRD INSTANCE
           ...
           /*,(CASE WHEN psn.psn_last_nm LIKE 'OPEN TERR%' OR  --QC51650
					   ...
					   (psn.eff_thru_dt IS NOT NULL AND TRUNC(SYSDATE) > TRUNC(to_date(psn.eff_thru_dt,'yyyymmdd')))*/ --QC52566
...
BEGIN
  ...
  UPDATE canon_e633_lfs_user_map_tbl e633
     SET status_flag = 'U'
...
--        dbms_output.put_line('Insert : l_sf_role_id : ' ||l_sf_role_id);
        ...
        BEGIN
         INSERT INTO canon_e633_lfs_user_map_tbl
                (
                employee_number,
                first_name,
                last_name,
...
--               -- validate_flag,
              ...
              (
                ...
                (CASE WHEN l_instance <> 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN (ins_user_rec.email_address || '.lfs.' || l_instance ) --QC51308
                      WHEN l_instance = 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN ins_user_rec.email_address 
                      ELSE NULL
                      END),
                (CASE WHEN l_instance <> 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN (ins_user_rec.email_address || '.lfs.' || l_instance) --QC51308
                      WHEN l_instance = 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN ins_user_rec.email_address || '.lfs' --QC51308 
                      ELSE NULL
                      END),
...
--        dbms_output.put_line('l_u_mgr_sfdc_id: ' );
        ...
        UPDATE canon_e633_lfs_user_map_tbl e633
           SET first_name = updt_user_rec.fname,
			   ...
			   email_address = (CASE WHEN l_instance <> 'CSAPRD' THEN updt_user_rec.emailAddr || '.lfs.' || l_instance ELSE updt_user_rec.emailAddr END), --QC51308
               user_name = (CASE WHEN l_instance <> 'CSAPRD' THEN updt_user_rec.emailAddr || '.lfs.' || l_instance ELSE updt_user_rec.emailAddr || '.lfs' END), --QC51308
                   ...
                   ,(SELECT val1 employee_number, val2 group_name
                      FROM canon_s21_cd_val_tbl val_tbl
                       ...
                       WHERE cd_tbl.cd_id = val_tbl.cd_id
                         AND CD_TBL.CD_NAME = 'CANON_E633_LFS_USER_ORG_MAP'
    ...
    IF (v_org_name IS NOT NULL) THEN
        SELECT ofrt.org_func_role_tp_nm
          ...
          FROM org_func_role_tp ofrt
               ,org_func_asg ofa
               ,toc t
--               ,CANON_E633_LFS_SF_PROFILE_TBL cpr
          WHERE ofa.psn_cd = v_resource_id
		    AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(ofa.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(ofa.eff_THRU_DT, 'yyyymmdd'),SYSDATE))
            AND ofa.toc_cd = t.toc_cd
...
BEGIN
   -- IF (v_last_name like '%ADJ%' OR v_last_name like '%TERRITORY%') THEN QC52566
	/*IF (v_last_name like '%OPEN TERR%' OR v_last_name like '%OPEN TERR%') THEN
        SELECT e633_profile.profile_id
         INTO x_sf_profile_id
         FROM CANON_E633_LFS_SF_PROFILE_TBL e633_profile
        WHERE e633_profile.NAME = 'CSA LFS Adjustment Rep' ;
    ELSE*/
    ...
    -- so this should some from role table.CONFIRM AND CHANGE THE CODE ACCORDINGLY.
       ...
       --   FROM CANON_E633_LFS_SF_PROFILE_TBL e633_profile
        ...
        -- WHERE cd_tbl.cd_name = 'CANON_E633_LFS_USR_POFILE'
        --   AND cd_tbl.cd_id = val_tbl.cd_id
...
-- Should be using this after getting confirmation from Yukiko
		SELECT e633.profile_id
            INTO x_sf_profile_id
            FROM org_func_role_tp ofrt
                 ,canon_e633_lfs_sf_profile_tbl e633
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        ELSE
		 ...
		 canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'ERROR','role: ' ||v_role,NULL,NULL,NULL,SUBSTR(SQLERRM,2000));
...
BEGIN
    SELECT e633.role_id
     INTO x_sf_role_id
     FROM CANON_E633_LFS_SF_ROLE_TBL e633
           ,ds_org_unit dou
          ,org_func_asg ofa
          ,toc t
--          ,org_func_role_tp_rg ofrt
    WHERE dou.org_cd = l_mapped_org
      AND ofa.psn_cd = l_resource_id
      AND ofa.toc_cd = t.toc_cd
      AND dou.org_cd = t.org_cd
    --  AND dou.line_biz_tp_cd IN ('LFS','ALL')
      AND dou.glbl_cmpy_cd = 'ADB'
      AND dou.ezcancelflag = '0'
      AND ofa.glbl_cmpy_cd = 'ADB'
      AND ofa.ezcancelflag = '0'
