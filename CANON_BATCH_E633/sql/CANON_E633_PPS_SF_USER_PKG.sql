create or replace PACKAGE CANON_E633_PPS_SF_USER_PKG
AS
TYPE x_role_data IS REF CURSOR ;
G_PACKAGE_NAME    VARCHAR2(30) := 'CANON_E633_PPS_SF_USER_PKG';
...
END CANON_E633_PPS_SF_USER_PKG;
/

create or replace PACKAGE BODY              CANON_E633_PPS_SF_USER_PKG
AS
...
AS
    ...
    SELECT distinct psn.psn_num employee_number--employee_number
       ...
       -- ,psn.eml_addr || '.pps' email_address --|| '.example.com'  ---THIS HAS TO BE CHANGED WHEN GOING TO PRD INSTANCE
		,psn.eml_addr email_address --QC51308
        ,psn.hr_ttl_nm job_title --job title
        ,(CASE WHEN psn.psn_last_nm LIKE 'OPEN TERR%' OR psn.psn_first_nm LIKE 'OPEN TERR%' --QC51650
             THEN
               'TRUE' --NULL QC52566
             ELSE
               'TRUE'
...
--        ,'I' load_status --load_status
        ,'I' status_flag
        ,SYSDATE oracle_load_date --oracle_load_date
        ,NULL sf_user_id --sf_user_id
--        ,NULL run_status --run_Status ---as we are having only 1 column holder to record the status
...
--        ,JOB_TTL jt
--        ,line_biz_tp_dev lbt
   WHERE psn.glbl_cmpy_cd = 'ADB' --AND psn.PSN_CD = '800001' -- PSN_NUM = 'K04749'--AND psn_cd = 'K02292' --'K00298'
...
--     AND psn.line_biz_tp_cd = lbt.line_biz_tp_cd
    ...
    AND EXISTS (SELECT 1
                ...
                WHERE ofa.glbl_cmpy_cd = 'ADB'
				  ...
				  AND ofrt.CRM_SLS_EXCL_FLG = 'N'  -- QC 61521
--                  AND dou.line_biz_tp_cd = 'PPS'
...
--                  AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(toc.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(toc.eff_THRU_DT, 'yyyymmdd'),SYSDATE))
                  AND dou.org_cd = t.org_cd
				  --AND dou.trty_grp_tp_cd IN ('PPS', 'ALL')
...
--                    and TRUNC(SYSDATE) BETWEEN TRUNC(NVL(val_tbl.val4'dd-MON-yyyy'),SYSDATE)) AND TRUNC(NVL(to_date(val_tbl.val5, 'dd-MON-yyyy'),SYSDATE))
                    )*/
    --to make sure if the PSN is not already in user mapping
     AND NOT EXISTS (SELECT '1'
                     FROM canon_e633_pps_user_map_tbl e633
                    WHERE e633.employee_number = psn.psn_num
...
CURSOR updt_user_cur IS
     SELECT psn.psn_cd rid
          ...
          -- ,psn.eml_addr || '.pps' emailAddr --|| '.example.com'  --THIS HAS TO BE CHANGED FOR PRD INSTANCE
		   ,psn.eml_addr emailAddr --QC51308
           ...
           ,psn.post_cd hpc
           --,get_sf_profile_id(psn.psn_last_nm, ofrt.org_func_role_tp_nm) profId
           ...
           /*,(CASE WHEN psn.psn_last_nm LIKE 'OPEN TERR%' OR  --QC51650
					   psn.psn_first_nm LIKE 'OPEN TERR%' OR  --QC51650
					   (psn.eff_thru_dt IS NOT NULL AND TRUNC(SYSDATE) > TRUNC(to_date(psn.eff_thru_dt,'yyyymmdd')))*/
		   ,(CASE WHEN (psn.eff_thru_dt IS NOT NULL AND TRUNC(SYSDATE) > TRUNC(to_date(psn.eff_thru_dt,'yyyymmdd')))
			 THEN
				'FALSE'
			 ELSE
				'TRUE'
          ...
          -- ,ofrt.org_func_role_tp_nm salesCompRole
           ,e633.*
      FROM canon_e633_pps_user_map_tbl e633
           ,s21_psn psn
--           ,s21_psn_rg psn
...
--           ,line_biz_tp_dev lbt
--           ,line_biz_tp lbt
     WHERE psn.psn_cd = e633.resource_id
       AND psn.glbl_cmpy_cd = 'ADB'
       AND psn.ezcancelflag = '0'
	   AND psn.LINE_BIZ_TP_CD IN ('PPS', 'ALL')
     --  AND psn.job_ttl_cd = jt.job_ttl_cd(+)
--       AND psn.line_biz_tp_cd = lbt.line_biz_tp_cd
     --  AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(psn.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(psn.eff_THRU_DT, 'yyyymmdd'),SYSDATE)) --QC52566
       AND e633.employee_number = psn.psn_num
       AND e633.sf_user_id IS NOT NULL;
...
l_mapped_org        VARCHAR2(500) := NULL;
...
l_u_comments        VARCHAR2(32767) := NULL;
...
l_procedure_name VARCHAR2(20) := 'extract_users';
...
BEGIN
  ...
  DELETE
    FROM canon_e633_pps_user_map_tbl e633
   WHERE e633.sf_user_id IS NULL
     AND status_flag IN ('I','U','E');

  dbms_output.put_line('Deleted records to reprocess :' ||SQL%ROWCOUNT);
  canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'INFO','Deleted records to reprocess :' ||SQL%ROWCOUNT,NULL,NULL,NULL,NULL);
  ...
  UPDATE canon_e633_pps_user_map_tbl e633
     SET status_flag = 'U'
 ...
 canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'INFO','Update flags to U for reprocess :' ||SQL%ROWCOUNT,NULL,NULL,NULL,NULL);
...
--        dbms_output.put_line('Insert : l_mapped_org : ' ||l_mapped_org);
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            l_toc_cd := NULL;
            canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'ERROR',NULL,NULL,NULL,NULL,'Unexpected Error:'||SUBSTR(SQLERRM,2000));
    END;
...
--        dbms_output.put_line('Insert : l_sf_role_id : ' ||l_sf_role_id);
        ...
        BEGIN
         INSERT INTO canon_e633_pps_user_map_tbl
                (
                employee_number,
                first_name,
                last_name,
...
--               -- validate_flag,
              ...
              (
                ...
                (CASE WHEN l_instance <> 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN (ins_user_rec.email_address || '.pps.' || l_instance ) --QC51308
                      WHEN l_instance = 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN ins_user_rec.email_address 
                      ELSE NULL
                      END),
                (CASE WHEN l_instance <> 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN (ins_user_rec.email_address || '.pps.' || l_instance) --QC51308
                      WHEN l_instance = 'CSAPRD' AND ins_user_rec.email_address IS NOT NULL THEN ins_user_rec.email_address || '.pps' --QC51308
                      ELSE NULL
                      END),
...
--              dbms_output.put_line('Inserted : ' || ins_user_rec.employee_number);
        EXCEPTION
            WHEN OTHERS THEN
              ...
              canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'ERROR',NULL,NULL,NULL,NULL,'Error during Insert:'||SUBSTR(SQLERRM,2000));
...
--        END LOOP;
   ...
   canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'INFO','# of records to insert: ' ||l_inserted_count,NULL,NULL,NULL,NULL);
...
--        dbms_output.put_line('fname : ' ||updt_user_rec.fname);
        ...
        IF(upper(l_instance) = upper('CSAPRD')) THEN
        ...
        EXCEPTION
            ...
            WHEN OTHERS THEN
                l_toc_cd := NULL;
                canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'ERROR',NULL,NULL,NULL,NULL,'Error fetching toc_cd:'||SUBSTR(SQLERRM,2000));
        END;
...
--        dbms_output.put_line('l_u_mgr_sfdc_id: ' );
        ...
        UPDATE canon_e633_pps_user_map_tbl e633
           SET first_name = updt_user_rec.fname,
               ...
               --email_address = updt_user_rec.emailAddr,
               --user_name = updt_user_rec.emailAddr,
			   email_address = (CASE WHEN l_instance <> 'CSAPRD' THEN updt_user_rec.emailAddr || '.pps.' || l_instance ELSE updt_user_rec.emailAddr END), --QC51308
               user_name = (CASE WHEN l_instance <> 'CSAPRD' THEN updt_user_rec.emailAddr || '.pps.' || l_instance ELSE updt_user_rec.emailAddr || '.pps' END), --QC51308
               job_title = updt_user_rec.jTitle,
               group_id = l_u_mapped_org,
               forecast_enabled = updt_user_rec.frcstEnbl,
               sf_role_id = l_u_sf_role,
               ...
               sf_profile_id = (CASE WHEN l_val_flag = 'FALSE' OR l_u_role IS NULL THEN updt_user_rec.sf_profile_id ELSE l_u_sf_profile END),
...
--               run_status = NULL,
               ...
               company_name = updt_user_rec.cmpyNm --'Canon Solutions America' QC52566
         WHERE e633.sf_user_id = updt_user_rec.sf_user_id;
         END IF;
    END LOOP;
	...
	canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'INFO','# of records to update: ' ||l_updated_count,NULL,NULL,NULL,NULL);
...
WHEN OTHERS THEN
    ...
    canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'ERROR',NULL,NULL,NULL,NULL,SUBSTR(SQLERRM,2000));
END extract_users;
...
l_procedure_name VARCHAR2(20) := 'get_org_code';
BEGIN
    BEGIN
...
--              AND dou.line_biz_tp_cd = 'PPS'
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                l_mapped_org := NULL;
                l_comments := 'Multiple Org''s assigned. No mapped org found in E633_SF_GRP_MAP_VS for PSN';
...
PROCEDURE get_role_name(v_resource_id IN VARCHAR2
                        ...
                        ,x_comments  IN OUT VARCHAR2)
AS
	l_procedure_name VARCHAR2(20) := 'get_role_name';
BEGIN
    IF (v_org_name IS NOT NULL) THEN
        SELECT ofrt.org_func_role_tp_nm
          ...
          FROM org_func_role_tp ofrt
               ,org_func_asg ofa
               ,toc t
--               ,CANON_E633_PPS_SF_PROFILE_TBL cpr
          WHERE ofa.psn_cd = v_resource_id
            AND ofa.toc_cd = t.toc_cd
			AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(ofa.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(ofa.eff_THRU_DT, 'yyyymmdd'),SYSDATE))
            ...
            AND t.glbl_cmpy_cd = 'ADB'
            AND t.ezcancelflag = '0';
--            AND UPPER(ofrt.org_func_role_tp_nm) = cpr.role_name;
    END IF;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        x_role := NULL;
...
PROCEDURE get_sf_profile_id(v_last_name IN VARCHAR2
                            ...
                            ,l_comments    IN OUT VARCHAR2)
AS
	l_procedure_name VARCHAR2(20) := 'get_sf_profile_id';
BEGIN
    /*IF (v_last_name like '%ADJ%' OR v_last_name like '%TERRITORY%') THEN --QC52566
        SELECT e633_profile.profile_id
         INTO x_sf_profile_id
         FROM CANON_E633_PPS_SF_PROFILE_TBL e633_profile
        WHERE e633_profile.NAME = 'CBS Adjustment' ;
    ELSE */ 
    ...
    -- so this should some from role table.CONFIRM AND CHANGE THE CODE ACCORDINGLY.
        --SELECT e633_profile.profile_id
         ...
         --WHERE cd_tbl.cd_name = 'CANON_E633_PPS_USR_POFILE'
         --  AND cd_tbl.cd_id = val_tbl.cd_id
         --  AND val_tbl.val1 = e633_profile.name
          -- AND val_tbl.val2 = v_role;
...
--           AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(profile_vs.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(profile_vs.eff_TO_DT, 'yyyymmdd'),SYSDATE));
		...
		SELECT e633.profile_id
            INTO x_sf_profile_id
            FROM org_func_role_tp ofrt
                 ,canon_e633_pps_sf_profile_tbl e633
            WHERE ofrt.org_func_role_tp_nm = v_role
              AND ofrt.crm_sls_prfl_nm = e633.name
              AND ofrt.ezcancelflag = '0'
              AND ofrt.glbl_cmpy_cd = 'ADB';
   -- END IF; --QC52566
EXCEPTION
...
PROCEDURE get_sf_role_id(l_mapped_org IN VARCHAR2
                         ...
                         ,l_comments IN OUT VARCHAR2)
AS
	l_procedure_name varchar2(20) := 'get_sf_role_id';
BEGIN
    SELECT e633.role_id
     INTO x_sf_role_id
     FROM CANON_E633_PPS_SF_ROLE_TBL e633
           ,ds_org_unit dou
          ,org_func_asg ofa
          ,toc t
    WHERE dou.org_cd = l_mapped_org
      ...
      AND ofa.toc_cd = t.toc_cd
      AND dou.org_cd = t.org_cd
     -- AND dou.line_biz_tp_cd IN ('PPS','ALL')
      AND dou.glbl_cmpy_cd = 'ADB'
      AND dou.ezcancelflag = '0'
      AND ofa.glbl_cmpy_cd = 'ADB'
      AND ofa.ezcancelflag = '0'
...
--      AND t.org_func_role_tp_cd = ofrt.org_func_role_tp_cd
      AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(dou.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(dou.eff_THRU_DT, 'yyyymmdd'),SYSDATE))
	  AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(ofa.eff_FROM_DT,'yyyymmdd'),SYSDATE)) AND TRUNC(NVL(to_date(ofa.eff_THRU_DT, 'yyyymmdd'),SYSDATE))
      AND UPPER(e633.NAME) = UPPER(trim(dou.org_nm));
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        ELSE
         ...
         END IF;
		 canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'ERROR','resource_id: ' ||l_resource_id,'org: ' ||l_mapped_org,NULL,NULL,SUBSTR(SQLERRM,2000));
END get_sf_role_id;
...
l_ctr   NUMBER := 0;
l_procedure_name varchar2(20) := 'get_hrch_name';
BEGIN
...
--        dbms_output.put_line('tier cd :' ||hrch_tier_rec.tier_cd);
        ...
        LOOP
            l_nvl := l_nvl ||
            (CASE l_ctr WHEN 0 THEN 'FIRST'  --QC51650
                                   WHEN 1 THEN 'SCD'
                                   WHEN 2 THEN 'THIRD'
                                   WHEN 3 THEN 'FRTH'
                                   WHEN 4 THEN 'FIFTH'
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        l_tier_org_nm := NULL;
        canon_e633_sf_error_log_pkg.log_error(G_PACKAGE_NAME,l_procedure_name,'ERROR','resource_id: ' ||v_resource_id,'toc: ' ||v_toc_cd,'tier: ' ||v_tier_cd,NULL,SUBSTR(SQLERRM,2000));
END get_hrch_name;
...
l_procedure_name varchar2(20) := 'get_hrch_name_psn';
BEGIN
 ...
 l_from := ' FROM toc_org_mgr_info tomi WHERE tomi.psn_cd = ''' || v_resource_id || ''''
                   ...
                   || ' AND TRUNC(SYSDATE) BETWEEN TRUNC(NVL(to_date(tomi.eff_FROM_DT,''yyyymmdd''),SYSDATE)) AND TRUNC(NVL(to_date(tomi.eff_THRU_DT, ''yyyymmdd''),SYSDATE))';
--        dbms_output.put_line('tier cd :' ||hrch_tier_rec.tier_cd);
        l_tier_nm := (CASE v_tier_cd WHEN 0 THEN 'FIRST' --QC51650
                                   WHEN 1 THEN 'SCD'
                                   WHEN 2 THEN 'THIRD'
                                   WHEN 3 THEN 'FRTH'
                                   WHEN 4 THEN 'FIFTH'
...
AS
		...
		l_procedure_name varchar2(20) := 'get_sfdc_id';
BEGIN
    SELECT sf_user_id
      INTO x_sfdc_id
      FROM canon_e633_pps_user_map_tbl
     WHERE resource_id = v_psn_cd
       AND status_flag = 'P';
     RETURN x_sfdc_id;
EXCEPTION
    WHEN NO_DATA_FOUND THEN
