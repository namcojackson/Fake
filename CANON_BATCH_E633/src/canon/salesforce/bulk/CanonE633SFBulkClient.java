...
import canon.salesforce.bulk.CanonE633SFBulkHandler;
...
public class CanonE633SFBulkClient{
	...
	CanonSalesForceCommon commonObj = new CanonSalesForceCommon();
	 ...
	 public static void main(String[] args) throws ConnectionException, IOException, SQLException, Exception{
		...
		// in.close();
		 ...
		 String procToCall4IDUpd = null;
		 CallableStatement setupStmt = null;
		 CanonE633SFBulkClient bulkClient = new CanonE633SFBulkClient();
		 ...
		 try {
			 ...
			 if ( handlerName != null) {
				 ...
				 if (oracleConnection == null)
					...
					setupStmt = (CallableStatement)oracleConnection.prepareCall("{call CANON_E633_SF_BULK_PKG.get_handler_setup(?,?)}");
					if (setupStmt != null) {
					...
					while (setupCur.next()){
						...
						procToCall4IDUpd = setupCur.getString("id_upd_proc");
					}
					setupStmt.close();
				 ...
				 if (process != null && process.equalsIgnoreCase("DEFAULT")) {
					 ...
					 if (procToCall4IDUpd != null){
						  setupStmt = (CallableStatement)oracleConnection.prepareCall("{call CANON_E633_SF_LFSPPS_PKG." + procToCall4IDUpd +"(?,?)}");
						  if(setupStmt != null) {
							  setupStmt.setObject(1, handlerName, OracleTypes.VARCHAR);
							  setupStmt.registerOutParameter(2, OracleTypes.VARCHAR);
							  setupStmt.execute();
						  }
							if (setupStmt.getString(2).equalsIgnoreCase("s")){
								System.out.println("calling run....");
								bulkClient.run(new CanonE633SFBulkHandler(handlerName, operation, sObject, externalIdFieldName));
							}else {
								System.out.println("Program exiting as it couldn't update the dependent ID's");
						}
					  } else {
						System.out.println("calling run....");
                        bulkClient.run(new CanonE633SFBulkHandler(handlerName, operation, sObject, externalIdFieldName));
				 ...
				 if (process != null && process.equalsIgnoreCase("REPROCESS")){
					 ...
					 if (reprocessJobId != null){
						 ...
						 SyncHandler handler = new CanonE633SFBulkHandler(handlerName, operation, sObject, externalIdFieldName);
			 ...
			 System.out.println("Exception Error cause : "+ex.getCause());
			 	ex.printStackTrace();
				System.exit(99);
		 } catch (Throwable e) {
			// TODO Auto-generated catch block
			 System.out.println("Throwable Error : " +e.getMessage());
			 System.out.println("Throwable Error cause : "+e.getCause());
			e.printStackTrace();
			System.exit(99);
		} finally{
			 try {
				 ...
				 System.out.println("Final Exception Error : " +ep.getMessage());
				 System.out.println("Final Exception cause : "+ep.getCause());
				 ep.printStackTrace();
				 System.exit(99);
			 }
		 }
	 ...
	 public LoginResponse getLoginResult() throws Throwable{
		 ...
		 try {
			 	System.out.println("getLoginResult::handlerName: " +handlerName);
				...
				// System.out.println("Salesforce pwd: " + lReq.getPassword());
	 ...
	 public JobBatchInfo createBatches(SyncHandler handler) throws Throwable{
		 ...
		 System.out.println("CreateBatches :: maxRowsPerBatch::"+maxRowsPerBatch);		 
...
//	            tmpFile.delete();
	 ...
	 * @throws Throwable 
	 ...
	 private void addJobDetails(CreateJobResponse jobInfo, String sObjectType, String operation) throws SQLException, Throwable{
		 ...
		 String dbCall = "{call CANON_E633_SF_BULK_PKG.add_job_details(?,?,?,?,?,?)}";
	 ...
	 private static void addBatchDetails(CreateBatchWithBinaryAttachmentsResponse batchInfo, String jobId, boolean isReprocess) throws Throwable{
		System.out.println("In addBatchDetails...........");
		 ...
		 if (isReprocess)
			 dbCall = "{call CANON_E633_SF_BULK_PKG.add_reprocess_details(?,?,?,?,?)}";
		 else dbCall = "{call CANON_E633_SF_BULK_PKG.add_batch_details(?,?,?,?,?,?)}";
	 ...
	 private void updateJobDetails(String jobID, String status, int totalBatches) throws Throwable{
		 ...
		 String dbCall = "{call CANON_E633_SF_BULK_PKG.update_job_details(?,?,?)}";
		 CallableStatement stmt = null;
		 try {
	        	if (oracleConnection == null)
	        			oracleConnection = TransactionScope.getConnection();
	 ...
	 private void awaitCompletion(CreateJobResponse jobResp, List<CreateBatchWithBinaryAttachmentsResponse> batchRespList) throws Throwable{
		 ...
		 while (!incompleteBatches.isEmpty()){
			 ...
			 for(BatchInfo b : statusList) {
				 ...
				 // To push Timeout exception to next level
				 throw e;
	 ...
	 private void updateResults(SyncHandler handler, JobInfo job, boolean isReprocess
		 ...
		 try {
		 	...
		 	for(BatchInfo b : batchInfoList.getBatchInfo()){
		 		...
		 		updateBatchDetailsE633Flag(b.getId(), b.getJobId(), isReprocess);
		 	}
		 }catch (Exception ex){
			 ex.printStackTrace();
			 throw ex;
	...
	private void updateBatchDetails(BatchInfo b, boolean isReprocess) throws Throwable{
		...
		String dbCall = null;
		if (!isReprocess)
		  dbCall = "{call CANON_E633_SF_BULK_PKG.update_batch_details(?,?,?,?,?,?,?,?,?)}";
		else dbCall = "{call CANON_E633_SF_BULK_PKG.update_reprocess_batch_details(?,?,?,?,?,?,?,?)}";
		 CallableStatement stmt = null;
	 ...
	 private void updateBatchDetailsE633Flag(String batchID, String jobID, boolean isReprocess) throws Throwable{
		 //boolean retStatus = false;
		 System.out.println("In updateBatchDetailsE633Flag.............");
		 String dbCall = "{call CANON_E633_SF_BULK_PKG.update_batch_e633_flag(?,?,?)}";
		 CallableStatement stmt = null;
	...
	public abstract static class AbstractSyncHandler implements SyncHandler {
	        ...
	        public int getBatchSize() {
	            //String batchSize = System.getProperty("VAR_INPUT_PARAM3");
				 String handler = System.getenv("VAR_USER1");
				 System.out.println("getBatchsize :: handler ::"+handler);
				 String batchSize = null;
				  if(handler.equalsIgnoreCase("PPSContractUpload") || handler.equalsIgnoreCase("LFSContractUpload") || handler.equalsIgnoreCase("LFSMtrReadUpload")  || handler.equalsIgnoreCase("PPSMtrReadUpload"))
					 batchSize = "2000";
				  else if (handler.equalsIgnoreCase("LFSCFSLeaseUpload") || handler.equalsIgnoreCase("PPSCFSLeaseUpload"))
					 batchSize = "1000";		
				  else
					 batchSize = "5000";
				 
				 System.out.println("getBatchsize :: batchSize ::"+batchSize);
	        ...
	        public static String csvDateTime (String str_date) {
	            SimpleDateFormat formatter ;
	            Date date  = null ;

	            //String str_date="Thu Jan 01 00:00:00 EST 2099";
	            //String str_date="2011-10-17 11:57:22.0";
	            //String pattern = "EEE MMM dd HH:mm:ss zzz yyyy";
	            String pattern = "yyyy-MM-dd HH:mm:ss.S";
	            SimpleDateFormat df = new SimpleDateFormat(pattern);
	            try {
	                    if (str_date.length() > 0)
	    			str_date = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(df.parse(str_date));
	                    		String dt = str_date.substring(0,10);
	                    		String tm = str_date.substring(11);
	                    		str_date = dt + "T" + tm + "Z" ;
	            }
	            catch(Exception ex) {
	                            //log.info("csvDateTime:Exception: " +e.getMessage());
	                    ex.printStackTrace();
