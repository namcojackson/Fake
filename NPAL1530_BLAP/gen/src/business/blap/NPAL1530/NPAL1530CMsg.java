//This file was automatically generated by Business Component Interface Definition Document (Business Component Interface Message Data Definition) and XLA200710010.
// Generated on    :20180116114850000
// Velocity macro:EZDMsg.vm V200607010
// Excel Add-in :BusinessComponentInterfaceDefinitionDocumentAddIn_XLS200710010.xla XLA200710010
/*
 *NPAL1530CMsg.java  Copyright  FUJITSU LIMITED 2007
 *Outline :
 *    1.
 *Release:
 *    No.  Date  Ver  Updater  Content
 *    1
 *
 */
package business.blap.NPAL1530;

import parts.common.*;
import parts.common.EZDSchemaItemDefines;

/**
 * It is NPAL1530 Business Component Interface Message class.
 * @author
 * @version XLA200710010
 */
public class NPAL1530CMsg extends EZDCMsg implements EZDSchemaItemDefines {

	// Serial Version UID
	private static final long serialVersionUID = 1L;

	// Data Definition
    /** GLBL_CMPY_CD*/
	public final EZDCStringItem              glblCmpyCd;

    /** SLS_DT*/
	public final EZDCDateItem              slsDt;

    /** XX_CMNT_TXT_CD*/
	public final EZDCStringItemArray              xxCmntTxt_CD;

    /** XX_CMNT_TXT_NM*/
	public final EZDCStringItemArray              xxCmntTxt_NM;

    /** XX_CMNT_TXT_SL*/
	public final EZDCStringItem              xxCmntTxt_SL;

    /** MRP_PLN_NM*/
	public final EZDCStringItem              mrpPlnNm;

    /** XX_LINK_ANCR_PN*/
	public final EZDCStringItem              xxLinkAncr_PN;

    /** RTL_WH_CD*/
	public final EZDCStringItem              rtlWhCd;

    /** RTL_WH_NM*/
	public final EZDCStringItem              rtlWhNm;

    /** RTL_SWH_CD*/
	public final EZDCStringItem              rtlSwhCd;

    /** RTL_SWH_NM*/
	public final EZDCStringItem              rtlSwhNm;

    /** LOC_TP_CD*/
	public final EZDCStringItem              locTpCd;

    /** DMND_CTOFF_DT*/
	public final EZDCDateItem              dmndCtoffDt;

    /** DMND_CTOFF_DAYS_AOT*/
	public final EZDCBigDecimalItem              dmndCtoffDaysAot;

    /** SPLY_CTOFF_DT*/
	public final EZDCDateItem              splyCtoffDt;

    /** SPLY_CTOFF_DAYS_AOT*/
	public final EZDCBigDecimalItem              splyCtoffDaysAot;

    /** CRAT_PRCH_REQ_FLG*/
	public final EZDCStringItem              cratPrchReqFlg;

    /** PRINT_ITEM_STS_FLG*/
	public final EZDCStringItem              printItemStsFlg;

    /** PRINT_ITEM_DESC_FLG*/
	public final EZDCStringItem              printItemDescFlg;

    /** XX_POP_PRM_P0*/
	public final EZDCStringItem              xxPopPrm_P0;

    /** XX_POP_PRM_P1*/
	public final EZDCStringItem              xxPopPrm_P1;

    /** XX_POP_PRM_P2*/
	public final EZDCStringItem              xxPopPrm_P2;

    /** XX_POP_PRM_P3*/
	public final EZDCStringItem              xxPopPrm_P3;

    /** XX_POP_PRM_P4*/
	public final EZDCStringItem              xxPopPrm_P4;

    /** XX_POP_PRM_P5*/
	public final EZDCStringItem              xxPopPrm_P5;

    /** XX_POP_PRM_P6*/
	public final EZDCStringItem              xxPopPrm_P6;

    /** XX_POP_PRM_P7*/
	public final EZDCStringItem              xxPopPrm_P7;

    /** XX_POP_PRM_P8*/
	public final EZDCStringItem              xxPopPrm_P8;

    /** XX_POP_PRM_P9*/
	public final EZDCStringItem              xxPopPrm_P9;

    /** XX_POP_PRM_PA*/
	public final EZDCStringItem              xxPopPrm_PA;

    /** P*/
	public final business.blap.NPAL1530.NPAL1530_PCMsgArray              P;


	/**
	 * NPAL1530CMsg is constructor.
	 * The initialization when the instance of NPAL1530CMsg is generated.
	 */
	public NPAL1530CMsg() {
		this(false, -1);
	}

	/**
	 * NPAL1530CMsg is constructor.
	 * The initialization when the instance of NPAL1530CMsg is generated.
	 * @param child  Flag whether it is child message
	 * @param eleNo  Index Number of array
	 */
	public NPAL1530CMsg(boolean child, int eleNo) {
		super(child, eleNo);

		// Initialization of item

		glblCmpyCd = (EZDCStringItem)newItem("glblCmpyCd");
		slsDt = (EZDCDateItem)newItem("slsDt");
		xxCmntTxt_CD = (EZDCStringItemArray)newItemArray("xxCmntTxt_CD");
		xxCmntTxt_NM = (EZDCStringItemArray)newItemArray("xxCmntTxt_NM");
		xxCmntTxt_SL = (EZDCStringItem)newItem("xxCmntTxt_SL");
		mrpPlnNm = (EZDCStringItem)newItem("mrpPlnNm");
		xxLinkAncr_PN = (EZDCStringItem)newItem("xxLinkAncr_PN");
		rtlWhCd = (EZDCStringItem)newItem("rtlWhCd");
		rtlWhNm = (EZDCStringItem)newItem("rtlWhNm");
		rtlSwhCd = (EZDCStringItem)newItem("rtlSwhCd");
		rtlSwhNm = (EZDCStringItem)newItem("rtlSwhNm");
		locTpCd = (EZDCStringItem)newItem("locTpCd");
		dmndCtoffDt = (EZDCDateItem)newItem("dmndCtoffDt");
		dmndCtoffDaysAot = (EZDCBigDecimalItem)newItem("dmndCtoffDaysAot");
		splyCtoffDt = (EZDCDateItem)newItem("splyCtoffDt");
		splyCtoffDaysAot = (EZDCBigDecimalItem)newItem("splyCtoffDaysAot");
		cratPrchReqFlg = (EZDCStringItem)newItem("cratPrchReqFlg");
		printItemStsFlg = (EZDCStringItem)newItem("printItemStsFlg");
		printItemDescFlg = (EZDCStringItem)newItem("printItemDescFlg");
		xxPopPrm_P0 = (EZDCStringItem)newItem("xxPopPrm_P0");
		xxPopPrm_P1 = (EZDCStringItem)newItem("xxPopPrm_P1");
		xxPopPrm_P2 = (EZDCStringItem)newItem("xxPopPrm_P2");
		xxPopPrm_P3 = (EZDCStringItem)newItem("xxPopPrm_P3");
		xxPopPrm_P4 = (EZDCStringItem)newItem("xxPopPrm_P4");
		xxPopPrm_P5 = (EZDCStringItem)newItem("xxPopPrm_P5");
		xxPopPrm_P6 = (EZDCStringItem)newItem("xxPopPrm_P6");
		xxPopPrm_P7 = (EZDCStringItem)newItem("xxPopPrm_P7");
		xxPopPrm_P8 = (EZDCStringItem)newItem("xxPopPrm_P8");
		xxPopPrm_P9 = (EZDCStringItem)newItem("xxPopPrm_P9");
		xxPopPrm_PA = (EZDCStringItem)newItem("xxPopPrm_PA");
		P = (business.blap.NPAL1530.NPAL1530_PCMsgArray)newMsgArray("P");
	}

	/**
	 * get the type of array which is stored
	 * @return NPAL1530CMsgArray
	 */
	public EZDMsgArray getMyArray() {
		return new NPAL1530CMsgArray();
	}


	/**
	 * Array of schema data(Basic data)
	 */
	private static final String[][] BASE_CONTENTS = {

	{"glblCmpyCd", "glblCmpyCd", null, null, TYPE_HANKAKUEISU, "4", null},
	{"slsDt", "slsDt", null, null, TYPE_NENTSUKIHI, "8", null},
	{"xxCmntTxt_CD", "xxCmntTxt_CD", "CD", "99", TYPE_HANKAKUEISU, "60", null},
	{"xxCmntTxt_NM", "xxCmntTxt_NM", "NM", "99", TYPE_HANKAKUEISU, "60", null},
	{"xxCmntTxt_SL", "xxCmntTxt_SL", "SL", null, TYPE_HANKAKUEISU, "60", null},
	{"mrpPlnNm", "mrpPlnNm", null, null, TYPE_HANKAKUEISU, "70", null},
	{"xxLinkAncr_PN", "xxLinkAncr_PN", "PN", null, TYPE_HANKAKUEISU, "30", null},
	{"rtlWhCd", "rtlWhCd", null, null, TYPE_HANKAKUEISU, "20", null},
	{"rtlWhNm", "rtlWhNm", null, null, TYPE_HANKAKUEISU, "30", null},
	{"rtlSwhCd", "rtlSwhCd", null, null, TYPE_HANKAKUEISU, "20", null},
	{"rtlSwhNm", "rtlSwhNm", null, null, TYPE_HANKAKUEISU, "30", null},
	{"locTpCd", "locTpCd", null, null, TYPE_HANKAKUEISU, "2", null},
	{"dmndCtoffDt", "dmndCtoffDt", null, null, TYPE_NENTSUKIHI, "8", null},
	{"dmndCtoffDaysAot", "dmndCtoffDaysAot", null, null, TYPE_SEISU_SYOSU, "3", "0"},
	{"splyCtoffDt", "splyCtoffDt", null, null, TYPE_NENTSUKIHI, "8", null},
	{"splyCtoffDaysAot", "splyCtoffDaysAot", null, null, TYPE_SEISU_SYOSU, "3", "0"},
	{"cratPrchReqFlg", "cratPrchReqFlg", null, null, TYPE_HANKAKUEISU, "1", null},
	{"printItemStsFlg", "printItemStsFlg", null, null, TYPE_HANKAKUEISU, "1", null},
	{"printItemDescFlg", "printItemDescFlg", null, null, TYPE_HANKAKUEISU, "1", null},
	{"xxPopPrm_P0", "xxPopPrm_P0", "P0", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P1", "xxPopPrm_P1", "P1", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P2", "xxPopPrm_P2", "P2", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P3", "xxPopPrm_P3", "P3", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P4", "xxPopPrm_P4", "P4", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P5", "xxPopPrm_P5", "P5", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P6", "xxPopPrm_P6", "P6", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P7", "xxPopPrm_P7", "P7", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P8", "xxPopPrm_P8", "P8", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_P9", "xxPopPrm_P9", "P9", null, TYPE_HANKAKUEISU, "300", null},
	{"xxPopPrm_PA", "xxPopPrm_PA", "PA", null, TYPE_HANKAKUEISU, "300", null},
	{"P", "P", null, "100", "business.blap.NPAL1530.NPAL1530_PCMsgArray", null, null},
	
	};

	/**
	 * Array of schema data(Visible Field)
	 */
	private static final String[][] DISP_CONTENTS = {

        {"GLBL_CMPY_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//glblCmpyCd
        {"SLS_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//slsDt
        {"XX_CMNT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxCmntTxt_CD
        {"XX_CMNT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxCmntTxt_NM
        {"XX_CMNT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxCmntTxt_SL
        {"MRP_PLN_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//mrpPlnNm
        {"XX_LINK_ANCR",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxLinkAncr_PN
        {"RTL_WH_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rtlWhCd
        {"RTL_WH_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rtlWhNm
        {"RTL_SWH_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rtlSwhCd
        {"RTL_SWH_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rtlSwhNm
        {"LOC_TP_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//locTpCd
        {"DMND_CTOFF_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dmndCtoffDt
        {"DMND_CTOFF_DAYS_AOT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dmndCtoffDaysAot
        {"SPLY_CTOFF_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//splyCtoffDt
        {"SPLY_CTOFF_DAYS_AOT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//splyCtoffDaysAot
        {"CRAT_PRCH_REQ_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//cratPrchReqFlg
        {"PRINT_ITEM_STS_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//printItemStsFlg
        {"PRINT_ITEM_DESC_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//printItemDescFlg
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P0
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P1
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P2
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P3
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P4
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P5
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P6
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P7
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P8
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_P9
        {"XX_POP_PRM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPopPrm_PA
		null,	//P
	};

	/**
	 * get Array of common (basic) data.
	 * @return Array of common (basis) data
	 */
	protected String[][] getBaseContents() {
		return BASE_CONTENTS;
	}

	/**
	 * get Array of Display Field.
	 * @return Array of  Display  Fields
	 */
	protected String[][] getDispContents() {
		return DISP_CONTENTS;
	}

}

