package business.servlet.ZZML0080.common;

import business.servlet.ZZML0080.ZZML0080BMsg;
...
public class ZZML0080CommonLogic {
    ...
    /** command button enum */
    private enum CMN_BTN { SAVE, SUBMIT, APPLY, APPROVE, REJECT, DOWNLOAD, DELETE, CLEAR, RESET, CLOSE };
    ...
    public static void setButtonPropertiesInit(S21CommonHandler handler) {
        setButtonProperties(handler, CMN_BTN.SAVE, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.SUBMIT, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.APPLY, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.APPROVE, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.REJECT, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.DOWNLOAD, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.DELETE, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.CLEAR, BTN_COND.ENABLE);
        setButtonProperties(handler, CMN_BTN.RESET, BTN_COND.NONE);
        setButtonProperties(handler, CMN_BTN.CLOSE, BTN_COND.ENABLE);
    }
    ...
    public static void setPagenationButtonProperties(S21CommonHandler handler, ZZML0080BMsg scrnMsg) {

        if (scrnMsg.xxPageShowOfNum.getValueInt() == 0) {
            handler.setButtonEnabled(BTN_PREV, false);
        } else {
            handler.setButtonEnabled(BTN_PREV, true);
        }

        if (scrnMsg.A.getValidCount() < 200) {
            handler.setButtonEnabled(BTN_NEXT, false);
        } else {
            handler.setButtonEnabled(BTN_NEXT, true);
        }
    }
}
