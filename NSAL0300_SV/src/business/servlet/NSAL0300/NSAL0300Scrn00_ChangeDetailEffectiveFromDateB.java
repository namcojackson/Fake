 ...
 *</pre>
 */
public class NSAL0300Scrn00_ChangeDetailEffectiveFromDateB extends S21CommonHandler {

    @Override
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        NSAL0300BMsg scrnMsg = (NSAL0300BMsg) bMsg;
        int rowNum = getButtonSelectNumber();
        if (ZYPCommonFunc.hasValue(scrnMsg.contrVrsnEffFromDt)) {
            scrnMsg.addCheckItem(scrnMsg.contrVrsnEffFromDt);
        }
        if (ZYPCommonFunc.hasValue(scrnMsg.B.no(rowNum).contrEffFromDt_B)) {
            scrnMsg.addCheckItem(scrnMsg.B.no(rowNum).contrEffFromDt_B);
        }
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        int rowNum = scrnMsg.xxRowNum_B.getValue().intValue();
        scrnMsg.setFocusItem(scrnMsg.B.no(rowNum).contrEffThruDt_B);
    }
}
