...
public class S21CMSearchTableBiz extends S21NEBusiness {
    ...
    protected void execute(S21NEContainerDataBean req) {
        ...
        List<String> list = searchTable(sib.getTableSearchKey(), sib.getSearchType());

        slb.setSearchTableNames(list);
        slb.setSearchTableLongNames(S21CMSearchCommon.createLongNameList(list));
        slb.setSearchTableComments(S21CMSearchCommon.createCommentList(list));
     ...
     * @return
     */
    private List<String> searchTable(String cond, String searchType) {
        ...
        // 全テーブル取得
        List<String> listAll = S21CMSearchCommon.createTableList();

        // 検索対象
        if (searchType.equals(S21CMRequestDef.REQ_SEARCH_TABLE_LONGNAME)) {
            // 対象をLongNameとする
            listSubj = S21CMSearchCommon.createLongNameList(listAll);
        } else if (searchType.equals(S21CMRequestDef.REQ_SEARCH_TABLE_COMMENT)) {
        	// 対象を Comment とする
        	listSubj = S21CMSearchCommon.createCommentList(listAll);
        } else {
            // 対象を通常のテーブル名とする
            listSubj = listAll;
        }
