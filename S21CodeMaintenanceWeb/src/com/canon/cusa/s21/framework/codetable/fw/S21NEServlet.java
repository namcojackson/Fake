...
public class S21NEServlet extends HttpServlet {
    ...
    protected S21NEContainerDataBean createContainerInstance(HttpServletRequest req) {
        ...
        String val = req.getParameter("file_upload");
        S21InfoLogOutput.println(String.format("S21NEContainerDataBean.createContainerInstance file_upload=%s", val));

        if (val != null){
            isUpload = Boolean.parseBoolean(val);
        }

        S21NEContainerDataBean bean = null;

        if (isUpload){
            bean = new S21NEContainerUploadDataBean(req);
        } else {
            bean = new S21NEContainerDataBean(req);
        }
