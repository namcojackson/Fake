...
/**
 *<pre>
 * NWWL0020Scrn00_ViewActionDetail
 *
 ...
 *</pre>
 */
public class NWWL0020Scrn00_ViewActionDetail extends S21CommonHandler {

    @Override
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {

        NWWL0020BMsg scrnMsg = (NWWL0020BMsg) bMsg;
        NWWL0020_ABMsg abMsg = null;

        int selectNum = getButtonSelectNumber();

        // Edit Event
        if (selectNum < 0) {
            selectNum = scrnMsg.xxRadioBtn_A0.getValueInt();

            if (selectNum < 0 || scrnMsg.A.getValidCount() == 0) {
                scrnMsg.setMessageInfo(NWWM0002E, new String[] {scrnMsg.xxRadioBtn_A0.getNameForMessage() });
                scrnMsg.xxRadioBtn_A0.setErrorInfo(1, NWWM0002E, new String[] {scrnMsg.xxRadioBtn_A0.getNameForMessage() });
                scrnMsg.addCheckItem(scrnMsg.xxRadioBtn_A0);
                scrnMsg.putErrorScreen();
                return;
            }
        }
        abMsg = scrnMsg.A.no(selectNum);
        ZYPEZDItemValueSetter.setValue(scrnMsg.ntfyActDtlPk_PR, abMsg.ntfyActDtlPk_A0.getValue());
    }
