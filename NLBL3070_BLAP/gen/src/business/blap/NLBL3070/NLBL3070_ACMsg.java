//This file was automatically generated by Business Component Interface Definition Document (Business Component Interface Message Data Definition) and XLA200710010.
// Generated on    :20210521173555000
// Velocity macro:EZDMsg.vm V200607010
// Excel Add-in :BusinessComponentInterfaceDefinitionDocumentAddIn_XLS200710010.xla XLA200710010
/*
 *NLBL3070_ACMsg.java  Copyright  FUJITSU LIMITED 2007
 *Outline :
 *    1.
 *Release:
 *    No.  Date  Ver  Updater  Content
 *    1
 *
 */
package business.blap.NLBL3070;

import parts.common.*;
import parts.common.EZDSchemaItemDefines;

/**
 * It is NLBL3070 Business Component Interface Message class.
 * @author
 * @version XLA200710010
 */
public class NLBL3070_ACMsg extends EZDCMsg implements EZDSchemaItemDefines {

	// Serial Version UID
	private static final long serialVersionUID = 1L;

	// Data Definition
    /** XX_CHK_BOX_A1*/
	public final EZDCStringItem              xxChkBox_A1;

    /** TRX_HDR_NUM_A1*/
	public final EZDCStringItem              trxHdrNum_A1;

    /** SVC_CONFIG_MSTR_PK_A1*/
	public final EZDCBigDecimalItem              svcConfigMstrPk_A1;

    /** BACK_ORD_IMPCT_TP_DESC_TXT_A1*/
	public final EZDCStringItem              backOrdImpctTpDescTxt_A1;

    /** SO_NUM_A1*/
	public final EZDCStringItem              soNum_A1;

    /** T_MDL_NM_A1*/
	public final EZDCStringItem              t_MdlNm_A1;

    /** RDD_DT_A1*/
	public final EZDCDateItem              rddDt_A1;

    /** SCHD_CARR_PICK_UP_DT_A1*/
	public final EZDCDateItem              schdCarrPickUpDt_A1;

    /** SCHD_DELY_DT_A1*/
	public final EZDCDateItem              schdDelyDt_A1;

    /** TECH_MEET_TRUCK_FLG_A1*/
	public final EZDCStringItem              techMeetTruckFlg_A1;

    /** SCHD_DELY_TS_DPLY_TXT_A2*/
	public final EZDCStringItem              schdDelyTsDplyTxt_A2;

    /** RQST_RCV_DT_TXT_S2*/
	public final EZDCStringItem              rqstRcvDtTxt_S2;

    /** CARR_CD_A1*/
	public final EZDCStringItem              carrCd_A1;

    /** CARR_NM_A1*/
	public final EZDCStringItem              carrNm_A1;

    /** SCHD_ISTL_DT_A1*/
	public final EZDCDateItem              schdIstlDt_A1;

    /** SCHD_DELY_TS_DPLY_TXT_A1*/
	public final EZDCStringItem              schdDelyTsDplyTxt_A1;

    /** RQST_RCV_DT_TXT_A1*/
	public final EZDCStringItem              rqstRcvDtTxt_A1;

    /** SCHD_DURN_TM_NUM_A1*/
	public final EZDCBigDecimalItem              schdDurnTmNum_A1;

    /** FSR_NUM_A1*/
	public final EZDCStringItem              fsrNum_A1;

    /** SVC_TASK_SCHD_DT_A1*/
	public final EZDCDateItem              svcTaskSchdDt_A1;

    /** TECH_NM_A1*/
	public final EZDCStringItem              techNm_A1;

    /** SCHD_COORD_STS_CD_A1*/
	public final EZDCStringItem              schdCoordStsCd_A1;

    /** SCHD_COORD_STS_CD_AL*/
	public final EZDCStringItemArray              schdCoordStsCd_AL;

    /** SCHD_COORD_STS_DESC_TXT_AL*/
	public final EZDCStringItemArray              schdCoordStsDescTxt_AL;

    /** SCHD_COORD_STS_DESC_TXT_A1*/
	public final EZDCStringItem              schdCoordStsDescTxt_A1;

    /** MAN_UPD_PRTY_NUM_A1*/
	public final EZDCBigDecimalItem              manUpdPrtyNum_A1;

    /** XX_PSN_FIRST_MID_LAST_NM_A2*/
	public final EZDCStringItem              xxPsnFirstMidLastNm_A2;

    /** TOC_NM_A1*/
	public final EZDCStringItem              tocNm_A1;

    /** CTAC_PSN_TEL_NUM_A1*/
	public final EZDCStringItem              ctacPsnTelNum_A1;

    /** DS_ACCT_NM_A1*/
	public final EZDCStringItem              dsAcctNm_A1;

    /** FIRST_LINE_ADDR_A1*/
	public final EZDCStringItem              firstLineAddr_A1;

    /** SCD_LINE_ADDR_A1*/
	public final EZDCStringItem              scdLineAddr_A1;

    /** CTY_ADDR_A1*/
	public final EZDCStringItem              ctyAddr_A1;

    /** ST_CD_A1*/
	public final EZDCStringItem              stCd_A1;

    /** POST_CD_A1*/
	public final EZDCStringItem              postCd_A1;

    /** CTRY_CD_A1*/
	public final EZDCStringItem              ctryCd_A1;

    /** RTL_WH_NM_A1*/
	public final EZDCStringItem              rtlWhNm_A1;

    /** WMS_DROP_FLG_A1*/
	public final EZDCStringItem              wmsDropFlg_A1;

    /** WMS_DROP_RQST_FLG_A1*/
	public final EZDCStringItem              wmsDropRqstFlg_A1;

    /** SEND_RQST_FLG_A1*/
	public final EZDCStringItem              sendRqstFlg_A1;

    /** SHPG_SVC_LVL_CD_A1*/
	public final EZDCStringItem              shpgSvcLvlCd_A1;

    /** CARR_ACCT_NUM_A1*/
	public final EZDCStringItem              carrAcctNum_A1;

    /** XX_PSN_FIRST_MID_LAST_NM_A1*/
	public final EZDCStringItem              xxPsnFirstMidLastNm_A1;

    /** TEMP_SCHD_RSN_CD_A1*/
	public final EZDCStringItem              tempSchdRsnCd_A1;

    /** TEMP_SCHD_CMNT_TXT_A1*/
	public final EZDCStringItem              tempSchdCmntTxt_A1;

    /** PDD_DT_A1*/
	public final EZDCDateItem              pddDt_A1;

    /** DS_ORD_CATG_DESC_TXT_A1*/
	public final EZDCStringItem              dsOrdCatgDescTxt_A1;

    /** DS_ORD_TP_DESC_TXT_A1*/
	public final EZDCStringItem              dsOrdTpDescTxt_A1;

    /** OPEN_FLG_A1*/
	public final EZDCStringItem              openFlg_A1;

    /** SCHD_OPEN_FLG_A1*/
	public final EZDCStringItem              schdOpenFlg_A1;

    /** MAN_SEND_RQST_FLG_AW*/
	public final EZDCStringItem              manSendRqstFlg_AW;

    /** MAN_SEND_RQST_FLG_AC*/
	public final EZDCStringItem              manSendRqstFlg_AC;

    /** CARR_CD_OD*/
	public final EZDCStringItem              carrCd_OD;

    /** NEXT_ACT_DT_A1*/
	public final EZDCDateItem              nextActDt_A1;

    /** NEXT_ACT_DT_BK*/
	public final EZDCDateItem              nextActDt_BK;

    /** NEXT_ACT_DT_BW*/
	public final EZDCDateItem              nextActDt_BW;

    /** SCE_ORD_TP_CD_A1*/
	public final EZDCStringItem              sceOrdTpCd_A1;


	/**
	 * NLBL3070_ACMsg is constructor.
	 * The initialization when the instance of NLBL3070_ACMsg is generated.
	 */
	public NLBL3070_ACMsg() {
		this(false, -1);
	}

	/**
	 * NLBL3070_ACMsg is constructor.
	 * The initialization when the instance of NLBL3070_ACMsg is generated.
	 * @param child  Flag whether it is child message
	 * @param eleNo  Index Number of array
	 */
	public NLBL3070_ACMsg(boolean child, int eleNo) {
		super(child, eleNo);

		// Initialization of item

		xxChkBox_A1 = (EZDCStringItem)newItem("xxChkBox_A1");
		trxHdrNum_A1 = (EZDCStringItem)newItem("trxHdrNum_A1");
		svcConfigMstrPk_A1 = (EZDCBigDecimalItem)newItem("svcConfigMstrPk_A1");
		backOrdImpctTpDescTxt_A1 = (EZDCStringItem)newItem("backOrdImpctTpDescTxt_A1");
		soNum_A1 = (EZDCStringItem)newItem("soNum_A1");
		t_MdlNm_A1 = (EZDCStringItem)newItem("t_MdlNm_A1");
		rddDt_A1 = (EZDCDateItem)newItem("rddDt_A1");
		schdCarrPickUpDt_A1 = (EZDCDateItem)newItem("schdCarrPickUpDt_A1");
		schdDelyDt_A1 = (EZDCDateItem)newItem("schdDelyDt_A1");
		techMeetTruckFlg_A1 = (EZDCStringItem)newItem("techMeetTruckFlg_A1");
		schdDelyTsDplyTxt_A2 = (EZDCStringItem)newItem("schdDelyTsDplyTxt_A2");
		rqstRcvDtTxt_S2 = (EZDCStringItem)newItem("rqstRcvDtTxt_S2");
		carrCd_A1 = (EZDCStringItem)newItem("carrCd_A1");
		carrNm_A1 = (EZDCStringItem)newItem("carrNm_A1");
		schdIstlDt_A1 = (EZDCDateItem)newItem("schdIstlDt_A1");
		schdDelyTsDplyTxt_A1 = (EZDCStringItem)newItem("schdDelyTsDplyTxt_A1");
		rqstRcvDtTxt_A1 = (EZDCStringItem)newItem("rqstRcvDtTxt_A1");
		schdDurnTmNum_A1 = (EZDCBigDecimalItem)newItem("schdDurnTmNum_A1");
		fsrNum_A1 = (EZDCStringItem)newItem("fsrNum_A1");
		svcTaskSchdDt_A1 = (EZDCDateItem)newItem("svcTaskSchdDt_A1");
		techNm_A1 = (EZDCStringItem)newItem("techNm_A1");
		schdCoordStsCd_A1 = (EZDCStringItem)newItem("schdCoordStsCd_A1");
		schdCoordStsCd_AL = (EZDCStringItemArray)newItemArray("schdCoordStsCd_AL");
		schdCoordStsDescTxt_AL = (EZDCStringItemArray)newItemArray("schdCoordStsDescTxt_AL");
		schdCoordStsDescTxt_A1 = (EZDCStringItem)newItem("schdCoordStsDescTxt_A1");
		manUpdPrtyNum_A1 = (EZDCBigDecimalItem)newItem("manUpdPrtyNum_A1");
		xxPsnFirstMidLastNm_A2 = (EZDCStringItem)newItem("xxPsnFirstMidLastNm_A2");
		tocNm_A1 = (EZDCStringItem)newItem("tocNm_A1");
		ctacPsnTelNum_A1 = (EZDCStringItem)newItem("ctacPsnTelNum_A1");
		dsAcctNm_A1 = (EZDCStringItem)newItem("dsAcctNm_A1");
		firstLineAddr_A1 = (EZDCStringItem)newItem("firstLineAddr_A1");
		scdLineAddr_A1 = (EZDCStringItem)newItem("scdLineAddr_A1");
		ctyAddr_A1 = (EZDCStringItem)newItem("ctyAddr_A1");
		stCd_A1 = (EZDCStringItem)newItem("stCd_A1");
		postCd_A1 = (EZDCStringItem)newItem("postCd_A1");
		ctryCd_A1 = (EZDCStringItem)newItem("ctryCd_A1");
		rtlWhNm_A1 = (EZDCStringItem)newItem("rtlWhNm_A1");
		wmsDropFlg_A1 = (EZDCStringItem)newItem("wmsDropFlg_A1");
		wmsDropRqstFlg_A1 = (EZDCStringItem)newItem("wmsDropRqstFlg_A1");
		sendRqstFlg_A1 = (EZDCStringItem)newItem("sendRqstFlg_A1");
		shpgSvcLvlCd_A1 = (EZDCStringItem)newItem("shpgSvcLvlCd_A1");
		carrAcctNum_A1 = (EZDCStringItem)newItem("carrAcctNum_A1");
		xxPsnFirstMidLastNm_A1 = (EZDCStringItem)newItem("xxPsnFirstMidLastNm_A1");
		tempSchdRsnCd_A1 = (EZDCStringItem)newItem("tempSchdRsnCd_A1");
		tempSchdCmntTxt_A1 = (EZDCStringItem)newItem("tempSchdCmntTxt_A1");
		pddDt_A1 = (EZDCDateItem)newItem("pddDt_A1");
		dsOrdCatgDescTxt_A1 = (EZDCStringItem)newItem("dsOrdCatgDescTxt_A1");
		dsOrdTpDescTxt_A1 = (EZDCStringItem)newItem("dsOrdTpDescTxt_A1");
		openFlg_A1 = (EZDCStringItem)newItem("openFlg_A1");
		schdOpenFlg_A1 = (EZDCStringItem)newItem("schdOpenFlg_A1");
		manSendRqstFlg_AW = (EZDCStringItem)newItem("manSendRqstFlg_AW");
		manSendRqstFlg_AC = (EZDCStringItem)newItem("manSendRqstFlg_AC");
		carrCd_OD = (EZDCStringItem)newItem("carrCd_OD");
		nextActDt_A1 = (EZDCDateItem)newItem("nextActDt_A1");
		nextActDt_BK = (EZDCDateItem)newItem("nextActDt_BK");
		nextActDt_BW = (EZDCDateItem)newItem("nextActDt_BW");
		sceOrdTpCd_A1 = (EZDCStringItem)newItem("sceOrdTpCd_A1");
	}

	/**
	 * get the type of array which is stored
	 * @return NLBL3070_ACMsgArray
	 */
	public EZDMsgArray getMyArray() {
		return new NLBL3070_ACMsgArray();
	}


	/**
	 * Array of schema data(Basic data)
	 */
	private static final String[][] BASE_CONTENTS = {

	{"xxChkBox_A1", "xxChkBox_A1", "A1", null, TYPE_HANKAKUEIJI, "1", null},
	{"trxHdrNum_A1", "trxHdrNum_A1", "A1", null, TYPE_HANKAKUEISU, "8", null},
	{"svcConfigMstrPk_A1", "svcConfigMstrPk_A1", "A1", null, TYPE_SEISU_SYOSU, "28", "0"},
	{"backOrdImpctTpDescTxt_A1", "backOrdImpctTpDescTxt_A1", "A1", null, TYPE_HANKAKUEISU, "50", null},
	{"soNum_A1", "soNum_A1", "A1", null, TYPE_HANKAKUEISU, "8", null},
	{"t_MdlNm_A1", "t_MdlNm_A1", "A1", null, TYPE_HANKAKUEISU, "50", null},
	{"rddDt_A1", "rddDt_A1", "A1", null, TYPE_NENTSUKIHI, "8", null},
	{"schdCarrPickUpDt_A1", "schdCarrPickUpDt_A1", "A1", null, TYPE_NENTSUKIHI, "8", null},
	{"schdDelyDt_A1", "schdDelyDt_A1", "A1", null, TYPE_NENTSUKIHI, "8", null},
	{"techMeetTruckFlg_A1", "techMeetTruckFlg_A1", "A1", null, TYPE_HANKAKUEISU, "1", null},
	{"schdDelyTsDplyTxt_A2", "schdDelyTsDplyTxt_A2", "A2", null, TYPE_HANKAKUEISU, "19", null},
	{"rqstRcvDtTxt_S2", "rqstRcvDtTxt_S2", "S2", null, TYPE_HANKAKUEISU, "10", null},
	{"carrCd_A1", "carrCd_A1", "A1", null, TYPE_HANKAKUEISU, "20", null},
	{"carrNm_A1", "carrNm_A1", "A1", null, TYPE_HANKAKUEISU, "60", null},
	{"schdIstlDt_A1", "schdIstlDt_A1", "A1", null, TYPE_NENTSUKIHI, "8", null},
	{"schdDelyTsDplyTxt_A1", "schdDelyTsDplyTxt_A1", "A1", null, TYPE_HANKAKUEISU, "19", null},
	{"rqstRcvDtTxt_A1", "rqstRcvDtTxt_A1", "A1", null, TYPE_HANKAKUEISU, "10", null},
	{"schdDurnTmNum_A1", "schdDurnTmNum_A1", "A1", null, TYPE_SEISU_SYOSU, "12", "0"},
	{"fsrNum_A1", "fsrNum_A1", "A1", null, TYPE_HANKAKUEISU, "10", null},
	{"svcTaskSchdDt_A1", "svcTaskSchdDt_A1", "A1", null, TYPE_NENTSUKIHI, "8", null},
	{"techNm_A1", "techNm_A1", "A1", null, TYPE_HANKAKUEISU, "45", null},
	{"schdCoordStsCd_A1", "schdCoordStsCd_A1", "A1", null, TYPE_HANKAKUEISU, "2", null},
	{"schdCoordStsCd_AL", "schdCoordStsCd_AL", "AL", "20", TYPE_HANKAKUEISU, "2", null},
	{"schdCoordStsDescTxt_AL", "schdCoordStsDescTxt_AL", "AL", "20", TYPE_HANKAKUEISU, "50", null},
	{"schdCoordStsDescTxt_A1", "schdCoordStsDescTxt_A1", "A1", null, TYPE_HANKAKUEISU, "50", null},
	{"manUpdPrtyNum_A1", "manUpdPrtyNum_A1", "A1", null, TYPE_SEISU_SYOSU, "4", "0"},
	{"xxPsnFirstMidLastNm_A2", "xxPsnFirstMidLastNm_A2", "A2", null, TYPE_HANKAKUEISU, "90", null},
	{"tocNm_A1", "tocNm_A1", "A1", null, TYPE_HANKAKUEISU, "50", null},
	{"ctacPsnTelNum_A1", "ctacPsnTelNum_A1", "A1", null, TYPE_HANKAKUEISU, "20", null},
	{"dsAcctNm_A1", "dsAcctNm_A1", "A1", null, TYPE_HANKAKUEISU, "360", null},
	{"firstLineAddr_A1", "firstLineAddr_A1", "A1", null, TYPE_HANKAKUEISU, "60", null},
	{"scdLineAddr_A1", "scdLineAddr_A1", "A1", null, TYPE_HANKAKUEISU, "60", null},
	{"ctyAddr_A1", "ctyAddr_A1", "A1", null, TYPE_HANKAKUEISU, "25", null},
	{"stCd_A1", "stCd_A1", "A1", null, TYPE_HANKAKUEISU, "2", null},
	{"postCd_A1", "postCd_A1", "A1", null, TYPE_HANKAKUEISU, "15", null},
	{"ctryCd_A1", "ctryCd_A1", "A1", null, TYPE_HANKAKUEISU, "3", null},
	{"rtlWhNm_A1", "rtlWhNm_A1", "A1", null, TYPE_HANKAKUEISU, "30", null},
	{"wmsDropFlg_A1", "wmsDropFlg_A1", "A1", null, TYPE_HANKAKUEISU, "1", null},
	{"wmsDropRqstFlg_A1", "wmsDropRqstFlg_A1", "A1", null, TYPE_HANKAKUEISU, "1", null},
	{"sendRqstFlg_A1", "sendRqstFlg_A1", "A1", null, TYPE_HANKAKUEISU, "1", null},
	{"shpgSvcLvlCd_A1", "shpgSvcLvlCd_A1", "A1", null, TYPE_HANKAKUEISU, "2", null},
	{"carrAcctNum_A1", "carrAcctNum_A1", "A1", null, TYPE_HANKAKUEISU, "20", null},
	{"xxPsnFirstMidLastNm_A1", "xxPsnFirstMidLastNm_A1", "A1", null, TYPE_HANKAKUEISU, "90", null},
	{"tempSchdRsnCd_A1", "tempSchdRsnCd_A1", "A1", null, TYPE_HANKAKUEISU, "3", null},
	{"tempSchdCmntTxt_A1", "tempSchdCmntTxt_A1", "A1", null, TYPE_HANKAKUEISU, "180", null},
	{"pddDt_A1", "pddDt_A1", "A1", null, TYPE_NENTSUKIHI, "8", null},
	{"dsOrdCatgDescTxt_A1", "dsOrdCatgDescTxt_A1", "A1", null, TYPE_HANKAKUEISU, "50", null},
	{"dsOrdTpDescTxt_A1", "dsOrdTpDescTxt_A1", "A1", null, TYPE_HANKAKUEISU, "50", null},
	{"openFlg_A1", "openFlg_A1", "A1", null, TYPE_HANKAKUEISU, "1", null},
	{"schdOpenFlg_A1", "schdOpenFlg_A1", "A1", null, TYPE_HANKAKUEISU, "1", null},
	{"manSendRqstFlg_AW", "manSendRqstFlg_AW", "AW", null, TYPE_HANKAKUEISU, "1", null},
	{"manSendRqstFlg_AC", "manSendRqstFlg_AC", "AC", null, TYPE_HANKAKUEISU, "1", null},
	{"carrCd_OD", "carrCd_OD", "OD", null, TYPE_HANKAKUEISU, "20", null},
	{"nextActDt_A1", "nextActDt_A1", "A1", null, TYPE_NENTSUKIHI, "8", null},
	{"nextActDt_BK", "nextActDt_BK", "BK", null, TYPE_NENTSUKIHI, "8", null},
	{"nextActDt_BW", "nextActDt_BW", "BW", null, TYPE_NENTSUKIHI, "8", null},
	{"sceOrdTpCd_A1", "sceOrdTpCd_A1", "A1", null, TYPE_HANKAKUEISU, "2", null},
	};

	/**
	 * Array of schema data(Visible Field)
	 */
	private static final String[][] DISP_CONTENTS = {

        {"XX_CHK_BOX",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxChkBox_A1
        {"TRX_HDR_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//trxHdrNum_A1
        {"SVC_CONFIG_MSTR_PK",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//svcConfigMstrPk_A1
        {"BACK_ORD_IMPCT_TP_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//backOrdImpctTpDescTxt_A1
        {"SO_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//soNum_A1
        {"T_MDL_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//t_MdlNm_A1
        {"RDD_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rddDt_A1
        {"SCHD_CARR_PICK_UP_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdCarrPickUpDt_A1
        {"SCHD_DELY_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdDelyDt_A1
        {"TECH_MEET_TRUCK_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//techMeetTruckFlg_A1
        {"SCHD_DELY_TS_DPLY_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdDelyTsDplyTxt_A2
        {"RQST_RCV_DT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_S2
        {"CARR_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrCd_A1
        {"CARR_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrNm_A1
        {"SCHD_ISTL_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdIstlDt_A1
        {"SCHD_DELY_TS_DPLY_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdDelyTsDplyTxt_A1
        {"RQST_RCV_DT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_A1
        {"SCHD_DURN_TM_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdDurnTmNum_A1
        {"FSR_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//fsrNum_A1
        {"SVC_TASK_SCHD_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//svcTaskSchdDt_A1
        {"TECH_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//techNm_A1
        {"SCHD_COORD_STS_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdCoordStsCd_A1
        {"SCHD_COORD_STS_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdCoordStsCd_AL
        {"SCHD_COORD_STS_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdCoordStsDescTxt_AL
        {"SCHD_COORD_STS_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdCoordStsDescTxt_A1
        {"MAN_UPD_PRTY_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//manUpdPrtyNum_A1
        {"XX_PSN_FIRST_MID_LAST_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPsnFirstMidLastNm_A2
        {"TOC_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//tocNm_A1
        {"CTAC_PSN_TEL_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//ctacPsnTelNum_A1
        {"DS_ACCT_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctNm_A1
        {"FIRST_LINE_ADDR",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//firstLineAddr_A1
        {"SCD_LINE_ADDR",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//scdLineAddr_A1
        {"CTY_ADDR",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//ctyAddr_A1
        {"ST_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//stCd_A1
        {"POST_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//postCd_A1
        {"CTRY_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//ctryCd_A1
        {"RTL_WH_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rtlWhNm_A1
        {"WMS_DROP_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//wmsDropFlg_A1
        {"WMS_DROP_RQST_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//wmsDropRqstFlg_A1
        {"SEND_RQST_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//sendRqstFlg_A1
        {"SHPG_SVC_LVL_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//shpgSvcLvlCd_A1
        {"CARR_ACCT_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrAcctNum_A1
        {"XX_PSN_FIRST_MID_LAST_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPsnFirstMidLastNm_A1
        {"TEMP_SCHD_RSN_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//tempSchdRsnCd_A1
        {"TEMP_SCHD_CMNT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//tempSchdCmntTxt_A1
        {"PDD_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//pddDt_A1
        {"DS_ORD_CATG_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsOrdCatgDescTxt_A1
        {"DS_ORD_TP_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsOrdTpDescTxt_A1
        {"OPEN_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//openFlg_A1
        {"SCHD_OPEN_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//schdOpenFlg_A1
        {"MAN_SEND_RQST_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//manSendRqstFlg_AW
        {"MAN_SEND_RQST_FLG",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//manSendRqstFlg_AC
        {"CARR_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrCd_OD
        {"NEXT_ACT_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//nextActDt_A1
        {"NEXT_ACT_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//nextActDt_BK
        {"NEXT_ACT_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//nextActDt_BW
        {"SCE_ORD_TP_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//sceOrdTpCd_A1
	};

	/**
	 * get Array of common (basic) data.
	 * @return Array of common (basis) data
	 */
	protected String[][] getBaseContents() {
		return BASE_CONTENTS;
	}

	/**
	 * get Array of Display Field.
	 * @return Array of  Display  Fields
	 */
	protected String[][] getDispContents() {
		return DISP_CONTENTS;
	}

}

