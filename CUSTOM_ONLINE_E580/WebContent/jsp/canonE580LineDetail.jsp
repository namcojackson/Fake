		...
		function askConfirmation(title,question,yes_func,no_func){
            ...
            $(function() {            	 
               ...
               /*  $(".select-container select")
                 ...
                 $("body").on("change",".service_package",function(event){
                 	event.preventDefault();
                	var elem=this;
                	var prev = $(this).data('val');
                	if($(elem).val()===""){
            			$(elem).val(prev);
                		popupError("Please select a valid Service Meter Package.");	
                	}else{
	                	askConfirmation(
	                		"Service Meter Package",
	                		"Changing the Service Meter Package will remove values for the existing Meter Types. Do you wish to Continue?",
	                		function(dialog){
								dialog.dialog( "close" );
								$("#service_package_change_flag").val("Y");
								$("#tabs-1 .save_button").trigger("click");
	                		},
	                		function(dialog){
	                			$(elem).val(prev);
	            				dialog.dialog("close");
