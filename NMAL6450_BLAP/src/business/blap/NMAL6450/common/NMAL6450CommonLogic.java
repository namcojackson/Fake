 ...
 *</pre>
 */
package business.blap.NMAL6450.common;
...
import business.blap.NMAL6450.NMAL6450Query;
import business.blap.NMAL6450.NMAL6450SMsg;
import business.blap.NMAL6450.NMAL6450_ASMsgArray;
...
public class NMAL6450CommonLogic implements NMAL6450Constant {
    ...
    /**
     * It copy 'NMAL6450CMsg.A' to 'NMAL6450SMsg.A' .
     * @param bizMsg
     * @param globalMsg
     */
    public static void saveCurrentPageToSMsg(NMAL6450CMsg bizMsg, NMAL6450SMsg globalMsg) {
    ...
    /**
     * It copy 'NMAL6450SMsg.A' to 'NMAL6450CMsg.A' .
     * @param bizMsg
     * @param globalMsg
     * @param index start index position
     */
    public static void loadOnePageToCMsg(NMAL6450CMsg bizMsg, NMAL6450SMsg globalMsg, int index) {
     ...
     * @param fromIdx
     */
    public static void pagenation(NMAL6450CMsg bizMsg, NMAL6450SMsg globalMsg, int fromIdx) {
     ...
     * @return
     */
    public static boolean inputCheck(NMAL6450CMsg bizMsg, NMAL6450SMsg globalMsg) {
        ...
        for (int i = 0; i < asMsgArray.getValidCount(); i++) {
            BigDecimal sortNum = asMsgArray.no(i).pmtTermSortNum_A1.getValue();
            String code = asMsgArray.no(i).pmtTermCd_A1.getValue();
             
            if (!sortNumMap.containsKey(sortNum)) {
                sortNumMap.put(sortNum, i);
            } else {
                int duplSortIdx = sortNumMap.get(sortNum);
                if (!asMsgArray.no(duplSortIdx).pmtTermSortNum_A1.isError()) {
                    asMsgArray.no(duplSortIdx).pmtTermSortNum_A1.setErrorInfo(1, MESSAGE_ID.NMAM0072E.toString(), new String[]{DISPLAY_NAME.SORT_NUM.toString()});
                    if (firstErrIdx == -1) {
                        firstErrIdx = duplSortIdx;
                    }
                }
                asMsgArray.no(i).pmtTermSortNum_A1.setErrorInfo(1, MESSAGE_ID.NMAM0072E.toString(), new String[]{DISPLAY_NAME.SORT_NUM.toString()});
                if (firstErrIdx == -1) {
                    firstErrIdx = i;
                }
                isNotError = false;
            ...
            if (!codeMap.containsKey(code)) {
                ...
                if (!asMsgArray.no(duplIdx).pmtTermCd_A1.isError() && !hasValue(asMsgArray.no(duplIdx).ezUpTime_A1)) {
                    asMsgArray.no(duplIdx).pmtTermCd_A1.setErrorInfo(1, MESSAGE_ID.NMAM0072E.toString(), new String[]{DISPLAY_NAME.CODE.toString()});
                    if (firstErrIdx == -1) {
                        firstErrIdx = duplIdx;
                    }
                }
                ...
                if (!hasValue(asMsgArray.no(i).ezUpTime_A1)) {
                    asMsgArray.no(i).pmtTermCd_A1.setErrorInfo(1, MESSAGE_ID.NMAM0072E.toString(), new String[]{DISPLAY_NAME.CODE.toString()});
                    if (firstErrIdx == -1) {
                        firstErrIdx = i;
                    }
                }
        ...
        if (!isNotError) {
            NMAL6450CommonLogic.pagenation(bizMsg, globalMsg, getPageStartRowIndex(firstErrIdx));
        }
        return isNotError;
    ...
    public static NMAL6450Query getQuery() {
        return NMAL6450Query.getInstance();
    }
}
