 ...
 *</pre>
 */
public class NSAL0740Scrn00_OnChangeEsclOvrg extends S21CommonHandler {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (lowerList != null && lowerList.size() > 0) {
            setContrAutoEsclOvrg(lowerList, abMsg);
        }
    ...
    private void setContrAutoEsclOvrg(List<NSAL0740_ABMsg> list, NSAL0740_ABMsg abMsg) {

        for (int i = 0; i < list.size(); i++) {
            NSAL0740_ABMsg targetABMsg = list.get(i);
            if (!targetABMsg.uplftMtrPrcUpRatio_D1.isInputProtected()) {
                setValue(targetABMsg.uplftMtrPrcUpRatio_D1, abMsg.uplftMtrPrcUpRatio_D1);
            }
        }
    }
}
