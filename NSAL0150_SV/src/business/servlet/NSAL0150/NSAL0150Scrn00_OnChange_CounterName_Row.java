 ...
 *</pre>
 */
public class NSAL0150Scrn00_OnChange_CounterName_Row extends S21CommonHandler {
    ...
    protected EZDCMsg setRequestData(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (NSAL0150Constant.PSEUDO_MTR_LB_CD_ALL.equals(scrnMsg.C.no(selectedRow).mtrLbCd_C.getValue())) {

            if (ZYPConstant.FLG_ON_Y.equals(scrnMsg.C.no(selectedRow).xxChkBox_C.getValue())) {

                for(int i=0; i<scrnMsg.C.getValidCount(); i++) {

                    if (!(NSAL0150Constant.PSEUDO_MTR_LB_CD_ALL.equals(scrnMsg.C.no(i).mtrLbCd_C.getValue()))) {
                        scrnMsg.C.no(i).xxChkBox_C.setValue(ZYPConstant.FLG_OFF_N);
                    }
