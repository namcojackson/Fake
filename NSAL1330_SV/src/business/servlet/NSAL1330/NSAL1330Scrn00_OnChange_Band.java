 ...
 *</pre>
 */
public class NSAL1330Scrn00_OnChange_Band extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        NSAL1330CommonLogic.deriveSvcPrcSelectNum(scrnMsg);

        int ixA = scrnMsg.xxNum_A.getValueInt();
        if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(ixA).prcMtrPkgPk_A)) {
            scrnMsg.A.no(ixA).prcMtrPkgPk_A.setErrorInfo(1, ZZM9000E, new String[] {"Service Package" });
            scrnMsg.addCheckItem(scrnMsg.A.no(ixA).prcMtrPkgPk_A);
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        NSAL1330_ZBMsg zScrnMsg = scrnMsg.Z.no(selectIndex);

        scrnMsg.addCheckItem(zScrnMsg.prcListBandDescTxt_Z);
        scrnMsg.putErrorScreen();
