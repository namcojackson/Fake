 ...
 * ----------------------------------------------------------------------
 * 2016/09/12   Fujitsu         C.Yokoi         Update          CSA-QC#12073
 *</pre>
 */
public class NMAL6700Scrn00_AddSpecialHandling extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        for (int i = 0; i < scrnMsg.S.getValidCount(); i++) {
            scrnMsg.addCheckItem(scrnMsg.S.no(i).effFromDt_S1);
            scrnMsg.addCheckItem(scrnMsg.S.no(i).effThruDt_S1);
        }
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (scrnMsg.S.getValidCount() > 0) {
            if (EZDMessageInfo.MSGTYPE_ERROR == scrnMsg.getMessageType()) {
                scrnMsg.setFocusItem(scrnMsg.S.no(0).dsSpclHdlgTpCd_S3);
            } else {
                scrnMsg.setFocusItem(scrnMsg.S.no(scrnMsg.S.getValidCount() - 1).dsSpclHdlgTpCd_S3);
            }
        }
    }
}
