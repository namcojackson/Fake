 ...
 */
package com.canon.cusa.s21.batch.NLC.NLCB024001;
...
import business.db.INVTY_DTL_SNAP_SHOTTMsg;
...
/**
 * <pre>
 * Inventory Detail Master Snapshot
 * 
 * [Daily Processing]
 *   Inventory Detail Master copy of Inventory Master Snapshot.
 *   Daily Shapshot information, "n" days hold, when obtaining the latest data, 
 *   to remove beyond the expiry date.
 *
 * [Monthly Processing]
 *   Update the Inventory Detail Snapshot's Monthly Close Date.
 ...
 * ----------------------------------------------------------------------
 * 2016/02/19   CSAI            K.Lee           Create          Create From Inventory Snapshot Batch
 *</pre>
 */
public class NLCB024001 extends S21BatchMain {
    ...
    private static final String CONST_KEY_DLY = "NLCB0240_DAYS";

    /** Constant table key : Month */
    private static final String CONST_KEY_MLY = "NLCB0240_MONTH";
    ...
    public static void main(String[] args) {
        ...
        new NLCB024001().executeBatch(NLCB024001.class.getSimpleName());
    }
    ...
    private void deleteSnapshot(String key, Map<String, String> params) {
        ...
        try {
            ...
            List<INVTY_DTL_SNAP_SHOTTMsg> snapShotList = new ArrayList<INVTY_DTL_SNAP_SHOTTMsg>();
            ...
            while (resSet.next()) {
                ...
                // Delete the Inventory Snapshot.
                INVTY_DTL_SNAP_SHOTTMsg snapShotTMsg = new INVTY_DTL_SNAP_SHOTTMsg();
                ...
                if (cnt == commitCount) {
                    ...
                    execCnt = S21FastTBLAccessor.removePhysical((INVTY_DTL_SNAP_SHOTTMsg[]) snapShotList.toArray(new INVTY_DTL_SNAP_SHOTTMsg[0]));
                    if (execCnt != cnt) {
...
//        BigDecimal snapShotPk = null;
        ...
        try {
            ...
            while (resSet.next()) {
                ...
                INVTY_DTL_SNAP_SHOTTMsg snapShotTMsg = new INVTY_DTL_SNAP_SHOTTMsg();
                S21ResultSetMapper.map(resSet, snapShotTMsg);
                ZYPEZDItemValueSetter.setValue(snapShotTMsg.invtyDtlSnapShotDt, procDt);
                ...
                if (cnt == commitCount) {
                    ...
                    execCnt = S21FastTBLAccessor.insert((INVTY_DTL_SNAP_SHOTTMsg[]) snapShotList.toArray(new INVTY_DTL_SNAP_SHOTTMsg[0]));
                    if (execCnt != cnt) {
        ...
        try {
            ...
            while (resSet.next()) {
                ...
                if (cnt == commitCount) {
                    ...
                    execCnt = S21FastTBLAccessor.update((INVTY_DTL_SNAP_SHOTTMsg[]) snapShotList.toArray(new INVTY_DTL_SNAP_SHOTTMsg[0]));
                    // not normal end
                    if (execCnt != cnt) {
                        throw new S21AbendException(MSG_ID_NLCM0053E);
                    }
