 ...
 *</pre>
 */
public class NSAL0380Scrn00_OnChangeBasePrcUpRatio extends S21CommonHandler {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        setBasePrcUpRatio(lowerList, abMsg);
        // mod start 2016/07/15 CSA Defect#8608
        if (ZYPConstant.FLG_ON_Y.equals(abMsg.rnwUsgFlg_A.getValue())) {
    ...
    private void setBasePrcUpRatio(List<NSAL0380_ABMsg> list, NSAL0380_ABMsg abMsg) {

        for (int i = 0; i < list.size(); i++) {
            NSAL0380_ABMsg targetABMsg = list.get(i);
            if (!targetABMsg.basePrcUpRatio_A.isInputProtected()) {
                setValue(targetABMsg.basePrcUpRatio_A, abMsg.basePrcUpRatio_A);
            }
        }
    }
}
