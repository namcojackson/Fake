 ...
 *</pre>
 */
public class NSAL0380Scrn00_OnChangeMtrPrcUpRatio extends S21CommonHandler {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        setMtrPrcUpRatio(lowerList, abMsg);
        scrnMsg.setFocusItem(abMsg.mtrPrcUpRatio_A);
    }

    private void setMtrPrcUpRatio(List<NSAL0380_ABMsg> list, NSAL0380_ABMsg abMsg) {

        for (int i = 0; i < list.size(); i++) {
            NSAL0380_ABMsg targetABMsg = list.get(i);
            if (!targetABMsg.mtrPrcUpRatio_A.isInputProtected()) {
                setValue(targetABMsg.mtrPrcUpRatio_A, abMsg.mtrPrcUpRatio_A);
            }
        }
    }
}
