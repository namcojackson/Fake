 ...
 *</pre>
 */
public class NSAL0380Scrn00_OnChangeBefEndRnwDays extends S21CommonHandler {
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        if (DS_CONTR_CATG.AGGREGATE.equals(abMsg.dsContrCatgCd_A.getValue())) {
            ...
            if (NSAL0380Constant.MACH_LVL_NUM_1.equals(scrnMsg.A.no(currentCnt).xxDplyByCtrlAncrLvlCd_A.getValue())) {
                ...
                scrnMsg.setFocusItem(abMsg.maxContrRnwCnt_A);
                return;
            }
        }
        setBefEndRnwDaysAot(lowerList, abMsg);
        scrnMsg.setFocusItem(abMsg.maxContrRnwCnt_A);
    }

    private void setBefEndRnwDaysAot(List<NSAL0380_ABMsg> list, NSAL0380_ABMsg abMsg) {

        for (int i = 0; i < list.size(); i++) {
            NSAL0380_ABMsg targetABMsg = list.get(i);
            if (!targetABMsg.befEndRnwDaysAot_A.isInputProtected()) {
                setValue(targetABMsg.befEndRnwDaysAot_A, abMsg.befEndRnwDaysAot_A);
            }
        }
    }
}
