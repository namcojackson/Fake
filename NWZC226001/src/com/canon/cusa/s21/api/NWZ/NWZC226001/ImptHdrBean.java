 ...
 * 04/12/2017   Fujitsu         S.Iidaka        Update          S21_NA#18284
 * 07/06/2017   Fujitsu         H.Sugawara      Update          QC#19450
 * 01/31/2018   Fujitsu         M.Ohno          Update          QC#22776-2
 * 03/14/2018   Fujitsu         S.Ohki          Update          QC#20153
 * </pre>
 */
public class ImptHdrBean implements IImportBean {

    private String glblCmpyCd; // global company code

    private String slsDt;

    private ONBATCH_TYPE onbtchType;
    ...
    private String addPmtTermCashDiscCd; // Payment Term Cash Discoun

    // 2018/01/31 QC#22776-2 add start
    private String sellToFirstRefCmntTxt; // Attention
    ...
    private BigDecimal loanPerDaysAot; // Loan Period Days Amount of
    ...
    private String maintOnlyFlg; // maintenance only flag

    // Loan Conversion
    private String configTpCd;

    private String loanOrigOrdNum;

    private String loanOrigDsOrdCatgCd;

    // L3#384
    private boolean isLeaseOrd;

    // *************************************************************************
    // Extension Field
    // *************************************************************************
    public final LinkedHashMap<BigDecimal, DsImptOrdConfigBean> dsImptOrdConfigMap;

    private final List<DsImptLineBean> dsImptOrdLineList;

    private final List<DsImptRtnLineBean> dsImptRtnLineList;

    /** Service Shell Service Detail */
    private final List<DsImptSvcDtlBean> dsImptSvcDtlList;

    /** Sales Credit(Header Level) */
    private final List<DS_IMPT_ORD_SLS_CRTMsg> dsImptSlsCrList;

    /** Delivery Data(Header Level) */
    private final List<DS_IMPT_ORD_DELY_INFOTMsg> dsImptDelyList;

    /** Site Survey Data(Header Level) */
    private final List<DS_IMPT_ORD_SITE_SRVYTMsg> dsImptSiteSrvyList;

    /** Install Data(Header Level) */
    private final List<DS_IMPT_ORD_ISTL_INFOTMsg> dsImptInstList;

    /** Contact Person Data(Header Level) */
    private final List<DS_IMPT_ORD_CTAC_PSNTMsg> dsImptCtacPsnList;
    ...
    private DS_CPO_SLS_CRTMsgArray origDsCpoSlsCrArray = null;

    private DS_CPO_CONFIGTMsgArray origDsCpoConfigArray = null;
    ...
    private final List<DS_IMPT_DTL_EXT_ATTRBTMsg> dsImptDtlExtAttrbTMsgList;
    ...
    private NSZC115001PMsg shellApiPMsg;

    // 2018/03/14 QC#20153 Add Start
    private boolean isNotLoanOrigOrdFlg;
    ...
    public ImptHdrBean() {
        ...
        this.dsImptSvcDtlList = new ArrayList<DsImptSvcDtlBean>();
        this.dsImptDtlExtAttrbTMsgList = new ArrayList<DS_IMPT_DTL_EXT_ATTRBTMsg>();

        this.shellApiPMsg = null;

        // 2018/03/14 QC#20153 Add Start
        this.isNotLoanOrigOrdFlg = false;
        // 2018/03/14 QC#20153 Add End
    }
    ...
    public boolean hasError(boolean doSearchChild) {
        ...
        if (doSearchChild) {
            ...
            for (DsImptSvcDtlBean svcDtlBean : dsImptSvcDtlList) {
                if (svcDtlBean.hasError(false)) {
                    return true;
                }
            }
        }
    ...
    public List<DsImptOrdErrBean> getAllErrorBean() {
        ...
        for (DsImptSvcDtlBean svcDtlBean : dsImptSvcDtlList) {
            errBeanList.addAll(svcDtlBean.getDsImptOrdErrList());
        }
    ...
    public boolean isLoanConversion() {
        return (ZYPCommonFunc.hasValue(this.getLoanOrigOrdNum()));
    }

    public boolean isExistValidatedSts() {
        if (isLoanConversion() && this.isUpdateData()) {
            return ORD_HDR_STS.CANCELLED.equals(this.origCpo.ordHdrStsCd.getValue()) || ORD_HDR_STS.CLOSED.equals(this.origCpo.ordHdrStsCd.getValue());
        }
        return this.isUpdateData() && ORD_HDR_STS.VALIDATED.equals(this.origCpo.ordHdrStsCd.getValue());
    }

    public boolean isLoanNew() {
        return (ZYPCommonFunc.hasValue(this.getConfigTpCd()) && !ZYPCommonFunc.hasValue(this.getLoanOrigOrdNum()));
    }

    public boolean isLoanUpgrade() {
        return (ZYPCommonFunc.hasValue(this.getLoanOrigOrdNum()) && (CONFIG_TP.NEW.equals(this.getConfigTpCd()) || CONFIG_TP.ADD_TO_CONFIG.equals(this.getConfigTpCd())));
    }

    public boolean isLoanDowngrade() {
        return (ZYPCommonFunc.hasValue(this.getLoanOrigOrdNum()) && CONFIG_TP.RETURN_EXISTING_IB.equals(this.getConfigTpCd()));
    }

    public boolean isLoanToSales() {
        return (ZYPCommonFunc.hasValue(this.getLoanOrigOrdNum()) && CONFIG_TP.TO_SALES_CONVERSION.equals(this.getConfigTpCd()));
    }
    ...
    public ExpendMdseBean getImptMdseByDtlPk(BigDecimal linePkDecimal) {
        for (DsImptLineBean lineBean : dsImptOrdLineList) {
            if (!ZYPCommonFunc.hasValue(lineBean.dsImptOrdDtlPk) || !lineBean.dsImptOrdDtlPk.getValue().equals(linePkDecimal)) { // 2017/04/12 S21_NA#18284 Mod
                continue;
            }
    ...
    public String toString() {
        ...
        for (DsImptSvcDtlBean svcDtlBean : dsImptSvcDtlList) {
            if (svcDtlBean.hasError(false)) {
                sb.append(String.format("[SvcDtl Line Error] PK: %.0f\n", svcDtlBean.dsImptSvcDtlPk.getValue()));
                for (DsImptOrdErrBean errBean : svcDtlBean.dsImptOrdErrList) {
                    sb.append(errBean.dsImptOrdErrTxt.getValue()).append("\n");
                }
            }
        }
     ...
     * @return onbtchType
     */
    public ONBATCH_TYPE getOnbtchType() {
        return onbtchType;
    }
     ...
     * @param onbtchType
     */
    public void setOnbtchType(ONBATCH_TYPE onbtchType) {
        this.onbtchType = onbtchType;
    }
    ...
    public List<DsImptSvcDtlBean> getDsImptSvcDtlList() {
        return dsImptSvcDtlList;
    }
     ...
     * @return maintOnlyFlg
     */
    public String getMaintOnlyFlg() {
        return this.maintOnlyFlg;
    }

    /**
     * @param maintOnlyFlg
     */
    public void setMaintOnlyFlg(String maintOnlyFlg) {
        this.maintOnlyFlg = maintOnlyFlg;
    }
    ...
    /**
     * @return loanOrigOrdNum
     */
    public String getLoanOrigOrdNum() {
        return this.loanOrigOrdNum;
    }

    /**
     * @param loanOrigOrdNum
     */
    public void setLoanOrigOrdNum(String loanOrigOrdNum) {
        this.loanOrigOrdNum = loanOrigOrdNum;
    }

    /**
     * @return loanOrigDsOrdCatgCd
     */
    public String getLoanOrigDsOrdCatgCd() {
        return this.loanOrigDsOrdCatgCd;
    }

    /**
     * @param loanOrigDsOrdCatgCd
     */
    public void setLoanOrigDsOrdCatgCd(String loanOrigDsOrdCatgCd) {
        this.loanOrigDsOrdCatgCd = loanOrigDsOrdCatgCd;
    }

    public void setShellApiPMsg(NSZC115001PMsg shellApiPMsg) {

        this.shellApiPMsg = shellApiPMsg;
    }

    public NSZC115001PMsg getShellApiPMsg() {

        return this.shellApiPMsg;
    }

    public DS_CPO_SLS_CRTMsgArray getOrigDsCpoSlsCrArray() {
        return this.origDsCpoSlsCrArray;
    }

    public void setOrigDsCpoSlsCrArray(DS_CPO_SLS_CRTMsgArray origDsCpoSlsCrArray) {
        this.origDsCpoSlsCrArray = origDsCpoSlsCrArray;
    }

    // L3#384
    public void setIsLeaseOrd(boolean isLeaseOrd) {
        this.isLeaseOrd = isLeaseOrd;
    }

    // L3#384
    public boolean getIsLeaseOrd() {
        return isLeaseOrd;
    }
    ...
    public String getSellToFirstRefCmntTxt() {
        return this.sellToFirstRefCmntTxt;
    }

    /**
     * @param sellToFirstRefCmntTxt
     */
    ...
    // 2018/03/14 QC#20153 Add Start
    public void setIsNotLoanOrigOrdFlg(boolean isNotLoanOrigOrdFlg) {
        this.isNotLoanOrigOrdFlg = isNotLoanOrigOrdFlg;
    }

    public boolean getIsNotLoanOrigOrdFlg() {
        return this.isNotLoanOrigOrdFlg;
    }
    // 2018/03/14 QC#20153 Add Start
}
