 ...
 *</pre>
 */
public class NMAL7060Scrn00_InsertRow_Bllg extends S21CommonHandler {

    @Override
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        NMAL7060BMsg scrnMsg = (NMAL7060BMsg) bMsg;

        int len = scrnMsg.B.length();
        if (len < (scrnMsg.B.getValidCount() + 1)) {
            scrnMsg.setMessageInfo(NMAM8187E, new String[] {"Billing Meter", String.valueOf(scrnMsg.B.length())});
    ...
    protected EZDCMsg setRequestData(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        scrnMsg.B.setValidCount(scrnMsg.B.getValidCount() + 1);
