//This file was automatically generated by Screen Field Definition Document and XLA200710010.
// Generated on    :20230808131412000
// Velocity macro:EZDMsg.vm V200607010
// Excel Add-in :ScreenFieldDefinitionDocument_AddIn_XLS200710010.xla XLA200710010
/*
 *NLBL2020BMsg.java  Copyright  FUJITSU LIMITED 2007
 *Outline :
 *    1.
 *Release:
 *    No.  Date  Ver  Updater  Content
 *    1
 *
 */
package business.servlet.NLBL2020;

import parts.common.*;
import parts.common.EZDSchemaItemDefines;

/**
 * It is NLBL2020 Screen Data Message class.
 * @author
 * @version XLA200710010
 */
public class NLBL2020BMsg extends EZDBMsg implements EZDSchemaItemDefines {

	// Serial Version UID
	private static final long serialVersionUID = 1L;

	// Data Definition
    /** GLBL_CMPY_CD*/
	public final EZDBStringItem              glblCmpyCd;

    /** SLS_DT*/
	public final EZDBDateItem              slsDt;

    /** USR_ID*/
	public final EZDBStringItem              usrId;

    /** SRCH_OPT_PK_PS*/
	public final EZDBBigDecimalItem              srchOptPk_PS;

    /** SRCH_OPT_PK_PD*/
	public final EZDBBigDecimalItemArray              srchOptPk_PD;

    /** SRCH_OPT_NM_PD*/
	public final EZDBStringItemArray              srchOptNm_PD;

    /** SRCH_OPT_NM_H1*/
	public final EZDBStringItem              srchOptNm_H1;

    /** TRX_HDR_NUM_H1*/
	public final EZDBStringItem              trxHdrNum_H1;

    /** SO_NUM_H1*/
	public final EZDBStringItem              soNum_H1;

    /** DS_SO_LINE_STS_CD_PS*/
	public final EZDBStringItem              dsSoLineStsCd_PS;

    /** DS_SO_LINE_STS_CD_PD*/
	public final EZDBStringItemArray              dsSoLineStsCd_PD;

    /** DS_SO_LINE_STS_DESC_TXT_PD*/
	public final EZDBStringItemArray              dsSoLineStsDescTxt_PD;

    /** SCE_ORD_TP_CD_PS*/
	public final EZDBStringItem              sceOrdTpCd_PS;

    /** SCE_ORD_TP_CD_PD*/
	public final EZDBStringItemArray              sceOrdTpCd_PD;

    /** SCE_ORD_TP_NM_PD*/
	public final EZDBStringItemArray              sceOrdTpNm_PD;

    /** SHPG_SVC_LVL_CD_PS*/
	public final EZDBStringItem              shpgSvcLvlCd_PS;

    /** SHPG_SVC_LVL_CD_PD*/
	public final EZDBStringItemArray              shpgSvcLvlCd_PD;

    /** SHPG_SVC_LVL_DESC_TXT_PD*/
	public final EZDBStringItemArray              shpgSvcLvlDescTxt_PD;

    /** XX_RTRN_INVTY_LOC_SRCH_TXT_RW*/
	public final EZDBStringItem              xxRtrnInvtyLocSrchTxt_RW;

    /** RTL_WH_NM_H1*/
	public final EZDBStringItem              rtlWhNm_H1;

    /** XX_RTRN_INVTY_LOC_SRCH_TXT_SW*/
	public final EZDBStringItem              xxRtrnInvtyLocSrchTxt_SW;

    /** RTL_SWH_NM_H1*/
	public final EZDBStringItem              rtlSwhNm_H1;

    /** SHIP_TO_CUST_CD_H1*/
	public final EZDBStringItem              shipToCustCd_H1;

    /** DS_ACCT_NM_H1*/
	public final EZDBStringItem              dsAcctNm_H1;

    /** CARR_CD_H1*/
	public final EZDBStringItem              carrCd_H1;

    /** CARR_NM_H1*/
	public final EZDBStringItem              carrNm_H1;

    /** MDSE_CD_H1*/
	public final EZDBStringItem              mdseCd_H1;

    /** MDSE_DESC_SHORT_TXT_H1*/
	public final EZDBStringItem              mdseDescShortTxt_H1;

    /** SVC_CONFIG_MSTR_PK_H1*/
	public final EZDBBigDecimalItem              svcConfigMstrPk_H1;

    /** PRO_NUM_H1*/
	public final EZDBStringItem              proNum_H1;

    /** WMS_DROP_FLG_Y*/
	public final EZDBStringItem              wmsDropFlg_Y;

    /** WMS_DROP_FLG_N*/
	public final EZDBStringItem              wmsDropFlg_N;

    /** XX_TRX_DT_FR*/
	public final EZDBDateItem              xxTrxDt_FR;

    /** XX_TRX_DT_TO*/
	public final EZDBDateItem              xxTrxDt_TO;

    /** RDD_DT_FR*/
	public final EZDBDateItem              rddDt_FR;

    /** RDD_DT_TO*/
	public final EZDBDateItem              rddDt_TO;

    /** XX_TRX_DT_F3*/
	public final EZDBDateItem              xxTrxDt_F3;

    /** XX_TRX_DT_T3*/
	public final EZDBDateItem              xxTrxDt_T3;

    /** RQST_RCV_DT_TXT_CD*/
	public final EZDBStringItemArray              rqstRcvDtTxt_CD;

    /** RQST_RCV_DT_TXT_DI*/
	public final EZDBStringItemArray              rqstRcvDtTxt_DI;

    /** XX_DT_NM_F1*/
	public final EZDBStringItem              xxDtNm_F1;

    /** RQST_RCV_DT_TXT_F1*/
	public final EZDBStringItem              rqstRcvDtTxt_F1;

    /** XX_DT_NM_T1*/
	public final EZDBStringItem              xxDtNm_T1;

    /** RQST_RCV_DT_TXT_T1*/
	public final EZDBStringItem              rqstRcvDtTxt_T1;

    /** XX_DT_NM_F2*/
	public final EZDBStringItem              xxDtNm_F2;

    /** RQST_RCV_DT_TXT_F2*/
	public final EZDBStringItem              rqstRcvDtTxt_F2;

    /** XX_DT_NM_T2*/
	public final EZDBStringItem              xxDtNm_T2;

    /** RQST_RCV_DT_TXT_T2*/
	public final EZDBStringItem              rqstRcvDtTxt_T2;

    /** XX_DT_NM_F3*/
	public final EZDBStringItem              xxDtNm_F3;

    /** RQST_RCV_DT_TXT_F3*/
	public final EZDBStringItem              rqstRcvDtTxt_F3;

    /** XX_DT_NM_T3*/
	public final EZDBStringItem              xxDtNm_T3;

    /** RQST_RCV_DT_TXT_T3*/
	public final EZDBStringItem              rqstRcvDtTxt_T3;

    /** XX_EXST_FLG_BO*/
	public final EZDBStringItem              xxExstFlg_BO;

    /** XX_EXST_FLG_NO*/
	public final EZDBStringItem              xxExstFlg_NO;

    /** XX_DPLY_TAB*/
	public final EZDBStringItem              xxDplyTab;

    /** XX_TAB_PROT_01*/
	public final EZDBStringItem              xxTabProt_01;

    /** XX_TAB_PROT_02*/
	public final EZDBStringItem              xxTabProt_02;

    /** XX_PAGE_SHOW_FROM_NUM_A*/
	public final EZDBBigDecimalItem              xxPageShowFromNum_A;

    /** XX_PAGE_SHOW_TO_NUM_A*/
	public final EZDBBigDecimalItem              xxPageShowToNum_A;

    /** XX_PAGE_SHOW_OF_NUM_A*/
	public final EZDBBigDecimalItem              xxPageShowOfNum_A;

    /** XX_PAGE_SHOW_CUR_NUM_A*/
	public final EZDBBigDecimalItem              xxPageShowCurNum_A;

    /** XX_PAGE_SHOW_TOT_NUM_A*/
	public final EZDBBigDecimalItem              xxPageShowTotNum_A;

    /** A*/
	public final business.servlet.NLBL2020.NLBL2020_ABMsgArray              A;

    /** XX_PAGE_SHOW_FROM_NUM_B*/
	public final EZDBBigDecimalItem              xxPageShowFromNum_B;

    /** XX_PAGE_SHOW_TO_NUM_B*/
	public final EZDBBigDecimalItem              xxPageShowToNum_B;

    /** XX_PAGE_SHOW_OF_NUM_B*/
	public final EZDBBigDecimalItem              xxPageShowOfNum_B;

    /** XX_PAGE_SHOW_CUR_NUM_B*/
	public final EZDBBigDecimalItem              xxPageShowCurNum_B;

    /** XX_PAGE_SHOW_TOT_NUM_B*/
	public final EZDBBigDecimalItem              xxPageShowTotNum_B;

    /** B*/
	public final business.servlet.NLBL2020.NLBL2020_BBMsgArray              B;

    /** C*/
	public final business.servlet.NLBL2020.NLBL2020_CBMsgArray              C;

    /** XX_LINK_ANCR_D1*/
	public final EZDBStringItem              xxLinkAncr_D1;

    /** CARR_CD_D1*/
	public final EZDBStringItem              carrCd_D1;

    /** CARR_NM_D1*/
	public final EZDBStringItem              carrNm_D1;

    /** TOT_FRT_AMT_D1*/
	public final EZDBBigDecimalItem              totFrtAmt_D1;

    /** PRO_NUM_D1*/
	public final EZDBStringItem              proNum_D1;

    /** XX_NUM_EV*/
	public final EZDBBigDecimalItem              xxNum_EV;

    /** XX_COMN_COL_ORD_TXT*/
	public final EZDBStringItem              xxComnColOrdTxt;

    /** XX_WRN_SKIP_FLG*/
	public final EZDBStringItem              xxWrnSkipFlg;

    /** XX_TBL_NM_P1*/
	public final EZDBStringItem              xxTblNm_P1;

    /** XX_TBL_CD_COL_NM_P1*/
	public final EZDBStringItem              xxTblCdColNm_P1;

    /** XX_TBL_NM_COL_NM_P1*/
	public final EZDBStringItem              xxTblNmColNm_P1;

    /** XX_TBL_SORT_COL_NM_P1*/
	public final EZDBStringItem              xxTblSortColNm_P1;

    /** XX_SCR_NM_P1*/
	public final EZDBStringItem              xxScrNm_P1;

    /** XX_HDR_CD_LB_NM_P1*/
	public final EZDBStringItem              xxHdrCdLbNm_P1;

    /** XX_HDR_NM_LB_NM_P1*/
	public final EZDBStringItem              xxHdrNmLbNm_P1;

    /** XX_DTL_CD_LB_NM_P1*/
	public final EZDBStringItem              xxDtlCdLbNm_P1;

    /** XX_DTL_NM_LB_NM_P1*/
	public final EZDBStringItem              xxDtlNmLbNm_P1;

    /** XX_COND_CD_P1*/
	public final EZDBStringItem              xxCondCd_P1;

    /** XX_COND_NM_P1*/
	public final EZDBStringItem              xxCondNm_P1;

    /** P*/
	public final business.servlet.NLBL2020.NLBL2020_PBMsgArray              P;

    /** XX_SORT_TBL_NM*/
	public final EZDBStringItem              xxSortTblNm;

    /** XX_SORT_ITEM_NM*/
	public final EZDBStringItem              xxSortItemNm;

    /** XX_SORT_ORD_BY_TXT*/
	public final EZDBStringItem              xxSortOrdByTxt;

    /** S*/
	public final business.servlet.NLBL2020.NLBL2020_SBMsgArray              S;

    /** SVC_CONFIG_MSTR_PK_P1*/
	public final EZDBBigDecimalItem              svcConfigMstrPk_P1;

    /** XX_MNT_EVENT_NM*/
	public final EZDBStringItem              xxMntEventNm;

    /** ACTV_FLG_SC*/
	public final EZDBStringItem              actvFlg_SC;

    /** XX_MODE_CD_TB*/
	public final EZDBStringItem              xxModeCd_TB;

    /** XX_NUM*/
	public final EZDBBigDecimalItem              xxNum;

    /** T*/
	public final business.servlet.NLBL2020.NLBL2020_TBMsgArray              T;

    /** XX_FILE_DATA_DL*/
	public final EZDBMimeSourceItem              xxFileData_DL;


	/**
	 * NLBL2020BMsg is constructor.
	 * The initialization when the instance of NLBL2020BMsg is generated.
	 */
	public NLBL2020BMsg() {
		this(false, -1);
	}

	/**
	 * NLBL2020BMsg is constructor.
	 * The initialization when the instance of NLBL2020BMsg is generated.
	 * @param child  Flag whether it is child message
	 * @param eleNo  Index Number of array
	 */
	public NLBL2020BMsg(boolean child, int eleNo) {
		super(child, eleNo);

		// Initialization of item

		glblCmpyCd = (EZDBStringItem)newItem("glblCmpyCd");
		slsDt = (EZDBDateItem)newItem("slsDt");
		usrId = (EZDBStringItem)newItem("usrId");
		srchOptPk_PS = (EZDBBigDecimalItem)newItem("srchOptPk_PS");
		srchOptPk_PD = (EZDBBigDecimalItemArray)newItemArray("srchOptPk_PD");
		srchOptNm_PD = (EZDBStringItemArray)newItemArray("srchOptNm_PD");
		srchOptNm_H1 = (EZDBStringItem)newItem("srchOptNm_H1");
		trxHdrNum_H1 = (EZDBStringItem)newItem("trxHdrNum_H1");
		soNum_H1 = (EZDBStringItem)newItem("soNum_H1");
		dsSoLineStsCd_PS = (EZDBStringItem)newItem("dsSoLineStsCd_PS");
		dsSoLineStsCd_PD = (EZDBStringItemArray)newItemArray("dsSoLineStsCd_PD");
		dsSoLineStsDescTxt_PD = (EZDBStringItemArray)newItemArray("dsSoLineStsDescTxt_PD");
		sceOrdTpCd_PS = (EZDBStringItem)newItem("sceOrdTpCd_PS");
		sceOrdTpCd_PD = (EZDBStringItemArray)newItemArray("sceOrdTpCd_PD");
		sceOrdTpNm_PD = (EZDBStringItemArray)newItemArray("sceOrdTpNm_PD");
		shpgSvcLvlCd_PS = (EZDBStringItem)newItem("shpgSvcLvlCd_PS");
		shpgSvcLvlCd_PD = (EZDBStringItemArray)newItemArray("shpgSvcLvlCd_PD");
		shpgSvcLvlDescTxt_PD = (EZDBStringItemArray)newItemArray("shpgSvcLvlDescTxt_PD");
		xxRtrnInvtyLocSrchTxt_RW = (EZDBStringItem)newItem("xxRtrnInvtyLocSrchTxt_RW");
		rtlWhNm_H1 = (EZDBStringItem)newItem("rtlWhNm_H1");
		xxRtrnInvtyLocSrchTxt_SW = (EZDBStringItem)newItem("xxRtrnInvtyLocSrchTxt_SW");
		rtlSwhNm_H1 = (EZDBStringItem)newItem("rtlSwhNm_H1");
		shipToCustCd_H1 = (EZDBStringItem)newItem("shipToCustCd_H1");
		dsAcctNm_H1 = (EZDBStringItem)newItem("dsAcctNm_H1");
		carrCd_H1 = (EZDBStringItem)newItem("carrCd_H1");
		carrNm_H1 = (EZDBStringItem)newItem("carrNm_H1");
		mdseCd_H1 = (EZDBStringItem)newItem("mdseCd_H1");
		mdseDescShortTxt_H1 = (EZDBStringItem)newItem("mdseDescShortTxt_H1");
		svcConfigMstrPk_H1 = (EZDBBigDecimalItem)newItem("svcConfigMstrPk_H1");
		proNum_H1 = (EZDBStringItem)newItem("proNum_H1");
		wmsDropFlg_Y = (EZDBStringItem)newItem("wmsDropFlg_Y");
		wmsDropFlg_N = (EZDBStringItem)newItem("wmsDropFlg_N");
		xxTrxDt_FR = (EZDBDateItem)newItem("xxTrxDt_FR");
		xxTrxDt_TO = (EZDBDateItem)newItem("xxTrxDt_TO");
		rddDt_FR = (EZDBDateItem)newItem("rddDt_FR");
		rddDt_TO = (EZDBDateItem)newItem("rddDt_TO");
		xxTrxDt_F3 = (EZDBDateItem)newItem("xxTrxDt_F3");
		xxTrxDt_T3 = (EZDBDateItem)newItem("xxTrxDt_T3");
		rqstRcvDtTxt_CD = (EZDBStringItemArray)newItemArray("rqstRcvDtTxt_CD");
		rqstRcvDtTxt_DI = (EZDBStringItemArray)newItemArray("rqstRcvDtTxt_DI");
		xxDtNm_F1 = (EZDBStringItem)newItem("xxDtNm_F1");
		rqstRcvDtTxt_F1 = (EZDBStringItem)newItem("rqstRcvDtTxt_F1");
		xxDtNm_T1 = (EZDBStringItem)newItem("xxDtNm_T1");
		rqstRcvDtTxt_T1 = (EZDBStringItem)newItem("rqstRcvDtTxt_T1");
		xxDtNm_F2 = (EZDBStringItem)newItem("xxDtNm_F2");
		rqstRcvDtTxt_F2 = (EZDBStringItem)newItem("rqstRcvDtTxt_F2");
		xxDtNm_T2 = (EZDBStringItem)newItem("xxDtNm_T2");
		rqstRcvDtTxt_T2 = (EZDBStringItem)newItem("rqstRcvDtTxt_T2");
		xxDtNm_F3 = (EZDBStringItem)newItem("xxDtNm_F3");
		rqstRcvDtTxt_F3 = (EZDBStringItem)newItem("rqstRcvDtTxt_F3");
		xxDtNm_T3 = (EZDBStringItem)newItem("xxDtNm_T3");
		rqstRcvDtTxt_T3 = (EZDBStringItem)newItem("rqstRcvDtTxt_T3");
		xxExstFlg_BO = (EZDBStringItem)newItem("xxExstFlg_BO");
		xxExstFlg_NO = (EZDBStringItem)newItem("xxExstFlg_NO");
		xxDplyTab = (EZDBStringItem)newItem("xxDplyTab");
		xxTabProt_01 = (EZDBStringItem)newItem("xxTabProt_01");
		xxTabProt_02 = (EZDBStringItem)newItem("xxTabProt_02");
		xxPageShowFromNum_A = (EZDBBigDecimalItem)newItem("xxPageShowFromNum_A");
		xxPageShowToNum_A = (EZDBBigDecimalItem)newItem("xxPageShowToNum_A");
		xxPageShowOfNum_A = (EZDBBigDecimalItem)newItem("xxPageShowOfNum_A");
		xxPageShowCurNum_A = (EZDBBigDecimalItem)newItem("xxPageShowCurNum_A");
		xxPageShowTotNum_A = (EZDBBigDecimalItem)newItem("xxPageShowTotNum_A");
		A = (business.servlet.NLBL2020.NLBL2020_ABMsgArray)newMsgArray("A");
		xxPageShowFromNum_B = (EZDBBigDecimalItem)newItem("xxPageShowFromNum_B");
		xxPageShowToNum_B = (EZDBBigDecimalItem)newItem("xxPageShowToNum_B");
		xxPageShowOfNum_B = (EZDBBigDecimalItem)newItem("xxPageShowOfNum_B");
		xxPageShowCurNum_B = (EZDBBigDecimalItem)newItem("xxPageShowCurNum_B");
		xxPageShowTotNum_B = (EZDBBigDecimalItem)newItem("xxPageShowTotNum_B");
		B = (business.servlet.NLBL2020.NLBL2020_BBMsgArray)newMsgArray("B");
		C = (business.servlet.NLBL2020.NLBL2020_CBMsgArray)newMsgArray("C");
		xxLinkAncr_D1 = (EZDBStringItem)newItem("xxLinkAncr_D1");
		carrCd_D1 = (EZDBStringItem)newItem("carrCd_D1");
		carrNm_D1 = (EZDBStringItem)newItem("carrNm_D1");
		totFrtAmt_D1 = (EZDBBigDecimalItem)newItem("totFrtAmt_D1");
		proNum_D1 = (EZDBStringItem)newItem("proNum_D1");
		xxNum_EV = (EZDBBigDecimalItem)newItem("xxNum_EV");
		xxComnColOrdTxt = (EZDBStringItem)newItem("xxComnColOrdTxt");
		xxWrnSkipFlg = (EZDBStringItem)newItem("xxWrnSkipFlg");
		xxTblNm_P1 = (EZDBStringItem)newItem("xxTblNm_P1");
		xxTblCdColNm_P1 = (EZDBStringItem)newItem("xxTblCdColNm_P1");
		xxTblNmColNm_P1 = (EZDBStringItem)newItem("xxTblNmColNm_P1");
		xxTblSortColNm_P1 = (EZDBStringItem)newItem("xxTblSortColNm_P1");
		xxScrNm_P1 = (EZDBStringItem)newItem("xxScrNm_P1");
		xxHdrCdLbNm_P1 = (EZDBStringItem)newItem("xxHdrCdLbNm_P1");
		xxHdrNmLbNm_P1 = (EZDBStringItem)newItem("xxHdrNmLbNm_P1");
		xxDtlCdLbNm_P1 = (EZDBStringItem)newItem("xxDtlCdLbNm_P1");
		xxDtlNmLbNm_P1 = (EZDBStringItem)newItem("xxDtlNmLbNm_P1");
		xxCondCd_P1 = (EZDBStringItem)newItem("xxCondCd_P1");
		xxCondNm_P1 = (EZDBStringItem)newItem("xxCondNm_P1");
		P = (business.servlet.NLBL2020.NLBL2020_PBMsgArray)newMsgArray("P");
		xxSortTblNm = (EZDBStringItem)newItem("xxSortTblNm");
		xxSortItemNm = (EZDBStringItem)newItem("xxSortItemNm");
		xxSortOrdByTxt = (EZDBStringItem)newItem("xxSortOrdByTxt");
		S = (business.servlet.NLBL2020.NLBL2020_SBMsgArray)newMsgArray("S");
		svcConfigMstrPk_P1 = (EZDBBigDecimalItem)newItem("svcConfigMstrPk_P1");
		xxMntEventNm = (EZDBStringItem)newItem("xxMntEventNm");
		actvFlg_SC = (EZDBStringItem)newItem("actvFlg_SC");
		xxModeCd_TB = (EZDBStringItem)newItem("xxModeCd_TB");
		xxNum = (EZDBBigDecimalItem)newItem("xxNum");
		T = (business.servlet.NLBL2020.NLBL2020_TBMsgArray)newMsgArray("T");

		xxFileData_DL = (EZDBMimeSourceItem)newItem("xxFileData_DL");
	}

	/**
	 * get the type of array which is stored
	 * @return NLBL2020BMsgArray
	 */
	public EZDMsgArray getMyArray() {
		return new NLBL2020BMsgArray();
	}

	/**
	 * Definition of Sub class for Schema Item Array(Basic data,Visible Field)
	 */
	private static class NLBL2020BMsgSchemaContents1 {

		/**
		 * Array of Schema Items in Sub class(Basic data)
		 */
		private static final String[][] BASE_CONTENTS = {
		//["key name", "variable", "suffix", "array length", "item type", "digits", "after decimal point degits"] <- This is how to define.
			{"glblCmpyCd", "glblCmpyCd", null, null, TYPE_HANKAKUEISU, "4", null},
			{"slsDt", "slsDt", null, null, TYPE_NENTSUKIHI, "8", null},
			{"usrId", "usrId", null, null, TYPE_HANKAKUEISU, "16", null},
			{"srchOptPk_PS", "srchOptPk_PS", "PS", null, TYPE_SEISU_SYOSU, "28", "0"},
			{"srchOptPk_PD", "srchOptPk_PD", "PD", "99", TYPE_SEISU_SYOSU, "28", "0"},
			{"srchOptNm_PD", "srchOptNm_PD", "PD", "99", TYPE_HANKAKUEISU, "50", null},
			{"srchOptNm_H1", "srchOptNm_H1", "H1", null, TYPE_HANKAKUEISU, "50", null},
			{"trxHdrNum_H1", "trxHdrNum_H1", "H1", null, TYPE_HANKAKUEISU, "8", null},
			{"soNum_H1", "soNum_H1", "H1", null, TYPE_HANKAKUEISU, "8", null},
			{"dsSoLineStsCd_PS", "dsSoLineStsCd_PS", "PS", null, TYPE_HANKAKUEISU, "2", null},
			{"dsSoLineStsCd_PD", "dsSoLineStsCd_PD", "PD", "99", TYPE_HANKAKUEISU, "2", null},
			{"dsSoLineStsDescTxt_PD", "dsSoLineStsDescTxt_PD", "PD", "99", TYPE_HANKAKUEISU, "50", null},
			{"sceOrdTpCd_PS", "sceOrdTpCd_PS", "PS", null, TYPE_HANKAKUEISU, "2", null},
			{"sceOrdTpCd_PD", "sceOrdTpCd_PD", "PD", "99", TYPE_HANKAKUEISU, "2", null},
			{"sceOrdTpNm_PD", "sceOrdTpNm_PD", "PD", "99", TYPE_HANKAKUEISU, "30", null},
			{"shpgSvcLvlCd_PS", "shpgSvcLvlCd_PS", "PS", null, TYPE_HANKAKUEISU, "2", null},
			{"shpgSvcLvlCd_PD", "shpgSvcLvlCd_PD", "PD", "99", TYPE_HANKAKUEISU, "2", null},
			{"shpgSvcLvlDescTxt_PD", "shpgSvcLvlDescTxt_PD", "PD", "99", TYPE_HANKAKUEISU, "50", null},
			{"xxRtrnInvtyLocSrchTxt_RW", "xxRtrnInvtyLocSrchTxt_RW", "RW", null, TYPE_HANKAKUEISU, "1000", null},
			{"rtlWhNm_H1", "rtlWhNm_H1", "H1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxRtrnInvtyLocSrchTxt_SW", "xxRtrnInvtyLocSrchTxt_SW", "SW", null, TYPE_HANKAKUEISU, "1000", null},
			{"rtlSwhNm_H1", "rtlSwhNm_H1", "H1", null, TYPE_HANKAKUEISU, "30", null},
			{"shipToCustCd_H1", "shipToCustCd_H1", "H1", null, TYPE_HANKAKUEISU, "20", null},
			{"dsAcctNm_H1", "dsAcctNm_H1", "H1", null, TYPE_HANKAKUEISU, "360", null},
			{"carrCd_H1", "carrCd_H1", "H1", null, TYPE_HANKAKUEISU, "20", null},
			{"carrNm_H1", "carrNm_H1", "H1", null, TYPE_HANKAKUEISU, "60", null},
			{"mdseCd_H1", "mdseCd_H1", "H1", null, TYPE_HANKAKUEISU, "16", null},
			{"mdseDescShortTxt_H1", "mdseDescShortTxt_H1", "H1", null, TYPE_HANKAKUEISU, "250", null},
			{"svcConfigMstrPk_H1", "svcConfigMstrPk_H1", "H1", null, TYPE_SEISU_SYOSU, "28", "0"},
			{"proNum_H1", "proNum_H1", "H1", null, TYPE_HANKAKUEISU, "30", null},
			{"wmsDropFlg_Y", "wmsDropFlg_Y", "Y", null, TYPE_HANKAKUEISU, "1", null},
			{"wmsDropFlg_N", "wmsDropFlg_N", "N", null, TYPE_HANKAKUEISU, "1", null},
			{"xxTrxDt_FR", "xxTrxDt_FR", "FR", null, TYPE_NENTSUKIHI, "8", null},
			{"xxTrxDt_TO", "xxTrxDt_TO", "TO", null, TYPE_NENTSUKIHI, "8", null},
			{"rddDt_FR", "rddDt_FR", "FR", null, TYPE_NENTSUKIHI, "8", null},
			{"rddDt_TO", "rddDt_TO", "TO", null, TYPE_NENTSUKIHI, "8", null},
			{"xxTrxDt_F3", "xxTrxDt_F3", "F3", null, TYPE_NENTSUKIHI, "8", null},
			{"xxTrxDt_T3", "xxTrxDt_T3", "T3", null, TYPE_NENTSUKIHI, "8", null},
			{"rqstRcvDtTxt_CD", "rqstRcvDtTxt_CD", "CD", "2", TYPE_HANKAKUEISU, "10", null},
			{"rqstRcvDtTxt_DI", "rqstRcvDtTxt_DI", "DI", "2", TYPE_HANKAKUEISU, "10", null},
			{"xxDtNm_F1", "xxDtNm_F1", "F1", null, TYPE_HANKAKUEISU, "17", null},
			{"rqstRcvDtTxt_F1", "rqstRcvDtTxt_F1", "F1", null, TYPE_HANKAKUEISU, "10", null},
			{"xxDtNm_T1", "xxDtNm_T1", "T1", null, TYPE_HANKAKUEISU, "17", null},
			{"rqstRcvDtTxt_T1", "rqstRcvDtTxt_T1", "T1", null, TYPE_HANKAKUEISU, "10", null},
			{"xxDtNm_F2", "xxDtNm_F2", "F2", null, TYPE_HANKAKUEISU, "17", null},
			{"rqstRcvDtTxt_F2", "rqstRcvDtTxt_F2", "F2", null, TYPE_HANKAKUEISU, "10", null},
			{"xxDtNm_T2", "xxDtNm_T2", "T2", null, TYPE_HANKAKUEISU, "17", null},
			{"rqstRcvDtTxt_T2", "rqstRcvDtTxt_T2", "T2", null, TYPE_HANKAKUEISU, "10", null},
			{"xxDtNm_F3", "xxDtNm_F3", "F3", null, TYPE_HANKAKUEISU, "17", null},
			{"rqstRcvDtTxt_F3", "rqstRcvDtTxt_F3", "F3", null, TYPE_HANKAKUEISU, "10", null},
			{"xxDtNm_T3", "xxDtNm_T3", "T3", null, TYPE_HANKAKUEISU, "17", null},
			{"rqstRcvDtTxt_T3", "rqstRcvDtTxt_T3", "T3", null, TYPE_HANKAKUEISU, "10", null},
			{"xxExstFlg_BO", "xxExstFlg_BO", "BO", null, TYPE_HANKAKUEISU, "1", null},
			{"xxExstFlg_NO", "xxExstFlg_NO", NO, null, TYPE_HANKAKUEISU, "1", null},
			{"xxDplyTab", "xxDplyTab", null, null, TYPE_HANKAKUEISU, "50", null},
			{"xxTabProt_01", "xxTabProt_01", "01", null, TYPE_HANKAKUEISU, "1", null},
			{"xxTabProt_02", "xxTabProt_02", "02", null, TYPE_HANKAKUEISU, "1", null},
			{"xxPageShowFromNum_A", "xxPageShowFromNum_A", "A", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowToNum_A", "xxPageShowToNum_A", "A", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowOfNum_A", "xxPageShowOfNum_A", "A", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowCurNum_A", "xxPageShowCurNum_A", "A", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowTotNum_A", "xxPageShowTotNum_A", "A", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"A", "A", null, "20", "business.servlet.NLBL2020.NLBL2020_ABMsgArray", null, null},
			{"xxPageShowFromNum_B", "xxPageShowFromNum_B", "B", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowToNum_B", "xxPageShowToNum_B", "B", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowOfNum_B", "xxPageShowOfNum_B", "B", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowCurNum_B", "xxPageShowCurNum_B", "B", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxPageShowTotNum_B", "xxPageShowTotNum_B", "B", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"B", "B", null, "20", "business.servlet.NLBL2020.NLBL2020_BBMsgArray", null, null},
			{"C", "C", null, "20", "business.servlet.NLBL2020.NLBL2020_CBMsgArray", null, null},
			{"xxLinkAncr_D1", "xxLinkAncr_D1", "D1", null, TYPE_HANKAKUEISU, "30", null},
			{"carrCd_D1", "carrCd_D1", "D1", null, TYPE_HANKAKUEISU, "20", null},
			{"carrNm_D1", "carrNm_D1", "D1", null, TYPE_HANKAKUEISU, "60", null},
			{"totFrtAmt_D1", "totFrtAmt_D1", "D1", null, TYPE_SEISU_SYOSU, "19", "4"},
			{"proNum_D1", "proNum_D1", "D1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxNum_EV", "xxNum_EV", "EV", null, TYPE_SEISU_SYOSU, "5", "0"},
			{"xxComnColOrdTxt", "xxComnColOrdTxt", null, null, TYPE_HANKAKUEISU, "2000", null},
			{"xxWrnSkipFlg", "xxWrnSkipFlg", null, null, TYPE_HANKAKUEISU, "1", null},
			{"xxTblNm_P1", "xxTblNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxTblCdColNm_P1", "xxTblCdColNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxTblNmColNm_P1", "xxTblNmColNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxTblSortColNm_P1", "xxTblSortColNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxScrNm_P1", "xxScrNm_P1", "P1", null, TYPE_HANKAKUEISU, "70", null},
			{"xxHdrCdLbNm_P1", "xxHdrCdLbNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxHdrNmLbNm_P1", "xxHdrNmLbNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxDtlCdLbNm_P1", "xxDtlCdLbNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxDtlNmLbNm_P1", "xxDtlNmLbNm_P1", "P1", null, TYPE_HANKAKUEISU, "30", null},
			{"xxCondCd_P1", "xxCondCd_P1", "P1", null, TYPE_HANKAKUEISU, "20", null},
			{"xxCondNm_P1", "xxCondNm_P1", "P1", null, TYPE_HANKAKUEISU, "70", null},
			{"P", "P", null, "35", "business.servlet.NLBL2020.NLBL2020_PBMsgArray", null, null},
			{"xxSortTblNm", "xxSortTblNm", null, null, TYPE_HANKAKUEISU, "1", null},
			{"xxSortItemNm", "xxSortItemNm", null, null, TYPE_HANKAKUEISU, "50", null},
			{"xxSortOrdByTxt", "xxSortOrdByTxt", null, null, TYPE_HANKAKUEISU, "4", null},
			{"S", "S", null, "200", "business.servlet.NLBL2020.NLBL2020_SBMsgArray", null, null},
			{"svcConfigMstrPk_P1", "svcConfigMstrPk_P1", "P1", null, TYPE_SEISU_SYOSU, "28", "0"},
			{"xxMntEventNm", "xxMntEventNm", null, null, TYPE_HANKAKUEISU, "30", null},
			{"actvFlg_SC", "actvFlg_SC", "SC", null, TYPE_HANKAKUEISU, "1", null},
			{"xxModeCd_TB", "xxModeCd_TB", "TB", null, TYPE_HANKAKUEISU, "2", null},
			{"xxNum", "xxNum", null, null, TYPE_SEISU_SYOSU, "5", "0"},
			{"T", "T", null, "99", "business.servlet.NLBL2020.NLBL2020_TBMsgArray", null, null},
		};

		/**
		 * Array of Schema Items in Sub class(Visible Field)
		 */
		private static final String[][] DISP_CONTENTS = {
		// ["message display string","mandotry item", "minimum", "maximum", "I/O type", "yen", "comma", "zero suppress", "plus display flag", "display length", "display length after decimal point digit", "Dividing date", "Year display pattern", "zero pudding for date", "separator for date"] <- This is how to define
	        {"GLBL_CMPY_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//glblCmpyCd
	        {"SLS_DT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,UNI, SEIREKI4,  NO, YES},	//slsDt
	        {"USR_ID",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//usrId
	        {"SRCH_OPT_PK",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//srchOptPk_PS
	        {"SRCH_OPT_PK",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//srchOptPk_PD
	        {"SRCH_OPT_NM",  NO,  null,null,"0", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//srchOptNm_PD
	        {"SRCH_OPT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//srchOptNm_H1
	        {"TRX_HDR_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//trxHdrNum_H1
	        {"SO_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//soNum_H1
	        {"DS_SO_LINE_STS_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsSoLineStsCd_PS
	        {"DS_SO_LINE_STS_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsSoLineStsCd_PD
	        {"DS_SO_LINE_STS_DESC_TXT",  NO,  null,null,"0", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsSoLineStsDescTxt_PD
	        {"SCE_ORD_TP_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//sceOrdTpCd_PS
	        {"SCE_ORD_TP_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//sceOrdTpCd_PD
	        {"SCE_ORD_TP_NM",  NO,  null,null,"0", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//sceOrdTpNm_PD
	        {"SHPG_SVC_LVL_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//shpgSvcLvlCd_PS
	        {"SHPG_SVC_LVL_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//shpgSvcLvlCd_PD
	        {"SHPG_SVC_LVL_DESC_TXT",  NO,  null,null,"0", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//shpgSvcLvlDescTxt_PD
	        {"XX_RTRN_INVTY_LOC_SRCH_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRtrnInvtyLocSrchTxt_RW
	        {"RTL_WH_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rtlWhNm_H1
	        {"XX_RTRN_INVTY_LOC_SRCH_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRtrnInvtyLocSrchTxt_SW
	        {"RTL_SWH_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rtlSwhNm_H1
	        {"SHIP_TO_CUST_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//shipToCustCd_H1
	        {"DS_ACCT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctNm_H1
	        {"CARR_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrCd_H1
	        {"CARR_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrNm_H1
	        {"MDSE_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//mdseCd_H1
	        {"MDSE_DESC_SHORT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//mdseDescShortTxt_H1
	        {"SVC_CONFIG_MSTR_PK",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//svcConfigMstrPk_H1
	        {"PRO_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//proNum_H1
	        {"WMS_DROP_FLG",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//wmsDropFlg_Y
	        {"WMS_DROP_FLG",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//wmsDropFlg_N
	        {"XX_TRX_DT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,UNI, SEIREKI4,  NO, YES},	//xxTrxDt_FR
	        {"XX_TRX_DT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,UNI, SEIREKI4,  NO, YES},	//xxTrxDt_TO
	        {"RDD_DT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,UNI, SEIREKI4,  NO, YES},	//rddDt_FR
	        {"RDD_DT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,UNI, SEIREKI4,  NO, YES},	//rddDt_TO
	        {"XX_TRX_DT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,UNI, SEIREKI4,  NO, YES},	//xxTrxDt_F3
	        {"XX_TRX_DT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,UNI, SEIREKI4,  NO, YES},	//xxTrxDt_T3
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_CD
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"0", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_DI
	        {"XX_DT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtNm_F1
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_F1
	        {"XX_DT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtNm_T1
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_T1
	        {"XX_DT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtNm_F2
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_F2
	        {"XX_DT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtNm_T2
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_T2
	        {"XX_DT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtNm_F3
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_F3
	        {"XX_DT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtNm_T3
	        {"RQST_RCV_DT_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//rqstRcvDtTxt_T3
	        {"XX_EXST_FLG",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxExstFlg_BO
	        {"XX_EXST_FLG",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxExstFlg_NO
	        {"XX_DPLY_TAB",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDplyTab
	        {"XX_TAB_PROT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxTabProt_01
	        {"XX_TAB_PROT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxTabProt_02
	        {"XX_PAGE_SHOW_FROM_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowFromNum_A
	        {"XX_PAGE_SHOW_TO_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowToNum_A
	        {"XX_PAGE_SHOW_OF_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowOfNum_A
	        {"XX_PAGE_SHOW_CUR_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowCurNum_A
	        {"XX_PAGE_SHOW_TOT_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowTotNum_A
			null,	//A
	        {"XX_PAGE_SHOW_FROM_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowFromNum_B
	        {"XX_PAGE_SHOW_TO_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowToNum_B
	        {"XX_PAGE_SHOW_OF_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowOfNum_B
	        {"XX_PAGE_SHOW_CUR_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowCurNum_B
	        {"XX_PAGE_SHOW_TOT_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxPageShowTotNum_B
			null,	//B
			null,	//C
	        {"XX_LINK_ANCR",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxLinkAncr_D1
	        {"CARR_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrCd_D1
	        {"CARR_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//carrNm_D1
	        {"TOT_FRT_AMT",  NO,  null,null,"1", NO,YES, NO, NO,null,null,null, null,  NO,  NO},	//totFrtAmt_D1
	        {"PRO_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//proNum_D1
	        {"XX_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxNum_EV
	        {"XX_COMN_COL_ORD_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxComnColOrdTxt
	        {"XX_WRN_SKIP_FLG",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxWrnSkipFlg
	        {"XX_TBL_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxTblNm_P1
	        {"XX_TBL_CD_COL_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxTblCdColNm_P1
	        {"XX_TBL_NM_COL_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxTblNmColNm_P1
	        {"XX_TBL_SORT_COL_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxTblSortColNm_P1
	        {"XX_SCR_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxScrNm_P1
	        {"XX_HDR_CD_LB_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxHdrCdLbNm_P1
	        {"XX_HDR_NM_LB_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxHdrNmLbNm_P1
	        {"XX_DTL_CD_LB_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtlCdLbNm_P1
	        {"XX_DTL_NM_LB_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxDtlNmLbNm_P1
	        {"XX_COND_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxCondCd_P1
	        {"XX_COND_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxCondNm_P1
			null,	//P
	        {"XX_SORT_TBL_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxSortTblNm
	        {"XX_SORT_ITEM_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxSortItemNm
	        {"XX_SORT_ORD_BY_TXT",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxSortOrdByTxt
			null,	//S
	        {"SVC_CONFIG_MSTR_PK",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//svcConfigMstrPk_P1
	        {"XX_MNT_EVENT_NM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxMntEventNm
	        {"ACTV_FLG",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//actvFlg_SC
	        {"XX_MODE_CD",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxModeCd_TB
	        {"XX_NUM",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxNum
			null,	//T
		};

	}

	private static class NLBL2020BMsgSchemaContents2 {

		/**
		 * Array of Schema Items in Sub class(Basic data)
		 */
		private static final String[][] BASE_CONTENTS = {
		//["key name", "variable", "suffix", "array length", "item type", "digits", "after decimal point degits"] <- This is how to define.
			{"xxFileData_DL", "xxFileData_DL", "DL", null, TYPE_UPLOAD, null, null},
		};

		/**
		 * Array of Schema Items in Sub class(Visible Field)
		 */
		private static final String[][] DISP_CONTENTS = {
		// ["message display string","mandotry item", "minimum", "maximum", "I/O type", "yen", "comma", "zero suppress", "plus display flag", "display length", "display length after decimal point digit", "Dividing date", "Year display pattern", "zero pudding for date", "separator for date"] <- This is how to define
	        {"XX_FILE_DATA",  NO,  null,null,"1", NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxFileData_DL
		};

	}


	/**
	 * Array of schema data(Basic data)
	 */
	private static final String[][] BASE_CONTENTS = new String[101][NLBL2020BMsgSchemaContents1.BASE_CONTENTS[0].length];
	static {
		int arcpStartPoint=0;
	System.arraycopy(NLBL2020BMsgSchemaContents1.BASE_CONTENTS, 0, BASE_CONTENTS, arcpStartPoint, NLBL2020BMsgSchemaContents1.BASE_CONTENTS.length);
	arcpStartPoint = arcpStartPoint + NLBL2020BMsgSchemaContents1.BASE_CONTENTS.length;
	System.arraycopy(NLBL2020BMsgSchemaContents2.BASE_CONTENTS, 0, BASE_CONTENTS, arcpStartPoint, NLBL2020BMsgSchemaContents2.BASE_CONTENTS.length);
	arcpStartPoint = arcpStartPoint + NLBL2020BMsgSchemaContents2.BASE_CONTENTS.length;
	}

	/**
	 * Array of schema data(Visible Field)
	 */
	private static final String[][] DISP_CONTENTS = new String[101][15];
	static {
		int arcpStartPoint=0;
		System.arraycopy(NLBL2020BMsgSchemaContents1.DISP_CONTENTS, 0, DISP_CONTENTS, arcpStartPoint, NLBL2020BMsgSchemaContents1.DISP_CONTENTS.length);
		arcpStartPoint = arcpStartPoint + NLBL2020BMsgSchemaContents1.DISP_CONTENTS.length;
		System.arraycopy(NLBL2020BMsgSchemaContents2.DISP_CONTENTS, 0, DISP_CONTENTS, arcpStartPoint, NLBL2020BMsgSchemaContents2.DISP_CONTENTS.length);
		arcpStartPoint = arcpStartPoint + NLBL2020BMsgSchemaContents2.DISP_CONTENTS.length;
	}

	/**
	 * get Array of common (basic) data.
	 * @return Array of common (basis) data
	 */
	protected String[][] getBaseContents() {
		return BASE_CONTENTS;
	}

	/**
	 * get Array of Display Field.
	 * @return Array of  Display  Fields
	 */
	protected String[][] getDispContents() {
		return DISP_CONTENTS;
	}

}

