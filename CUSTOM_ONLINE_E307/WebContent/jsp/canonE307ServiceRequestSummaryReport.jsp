...
<% 
	...
	System.out.println("strModeCd : " +strModeCd+" tskSucc : "+tskSucc+" addTsk : "+addTsk);
...
/*	if("Y".equals(strCnclFlg)){
	...
	Object[] obj = detailObj.getServiceRequestDetails(strFSRNumber, strTskNum, "REPORT");
	...
	//String strOpenFsr = createObj.getOpenSR(strSerNum);
	...
	if(servObj!=null){
		...
		//strTmZn = utilObj.getTimeZone(postalCd, cntryCd);
		tmZone = utilObj.getTmZone(servObj.getStrCurPostCd(), servObj.getStrCurCtryCd(),"");
	}
	System.out.println("Mach Status: "+ servObj.getStrMachStatus()+ " servObj.getStrSrTpCd() : "+ servObj.getStrSrTpCd());
	String strMachStat = "";
	...
	if(arrAch!=null && arrAch.length>0){
		...
		trvlChrg = util.checkNull(arrAch[4]).trim();
	}
...
<div id="main_content" style="min-height: 924px;">  
  ...
  <table style="border:0px;margin:5px 0px 0px 0px;margin-left:25px;width:850px;">
		<tr>
...
<div class="table-wrapper"> 
  <table class="service-detail" style="width:100%;height:252px;" cellspacing="5" cellpadding="0">
 	<tr>
 		  <td style="width:25%;height:100%;" valign=top>
 		 	 <div class="machine-information" style="width:100%;height:98%;border:1px solid #CCC;"> 
	            <table cellspacing="5" cellpadding="0" style="width:100%;border:0px;">
					<tr>
						...
						<tr><td style="width:5%" nowrap>Vendor Call#</td>
							<td><input type="text" id="ittCallNumber" name="ittCallNumber" value="<%=util.checkNull(servObj.getStrITTNum())%>" style="width:130px;"></td>
						</tr>	
	...
	<!-- 					   		<input type="text" value="" id="custPhNum1" name="custPhNum1" class="numeric" size=1 maxlength="3" onkeyup="autoTab(this, callerPhnNum2)" style="width:20px">
								...
								<input type="text" value="" id="callerExtn" name="callerExtn" class="numeric" size=1 maxlength="4" style="width:20px">  -->
								 <input type="text"  size="12" maxlength="12" name="custPhNum" id="custPhNum" value="<%=util.checkNull(machObj.getCustTelNumber())%>" style="width:90px"/> - <input type="text"  size="4" name="custPhNumExt" id="custPhNumExt"  maxlength="5" value="<%=util.checkNull(machObj.getCustTelExtn())%>" style="width:45px"/>
						   </td>
						</tr>
						<tr><td nowrap>Mobile#</td>
						   <td  nowrap>
								 <input type="text"  size="12" maxlength="12" name="mobileNum" id="mobileNum" value="<%=util.checkNull(machObj.getMobileNum())%>" style="width:90px" class="rdl"/>
						   </td>
						</tr>	
		...
		<%-- 				<tr><td nowrap>ETA Time(Local) </td>
					...
					<tr><td nowrap><%=lblVal%> Arrival </td><td><input type="text" name="estmtArrival" id="estmtArrival" value="<%=util.checkNull(utilObj.getTmZone(servObj.getStrCurPostCd(), servObj.getStrCurCtryCd(), tskObj.getStrEstFrom())) %>" class="rdl" style="width:130px;"/>
							</td></tr>			 			
					...
					<tr>
						<td><b>Time Zone</b></td>
						<td>
				 		  <input type="text" id="tmZone" name="tmZone" value="<%=tmZone%>" style='width:130px;background-color: #33FF33;font-weight: bold;color:#000000' disabled/> 
						</td>
					</tr>
				</table>
			</div>
...
<%-- 							<td >Respond By Date</td>
						...
						<tr>
							<td>Customer P.O.#</td>
							<td><input type="text" name="custPoNumber" id="custPoNumber" value="<%=util.checkNull(servObj.getStrCustPONum())%>" ></td>
						...
						<td><input type="checkbox" name="verifiedPO" id="verifiedPO" <%=poVerifyFlag%> value="Y" style="border:0px;"></td>
						<td>Payment Terms</td> 
...
<!--  						<input type="text" value="" id="callerPhnNum1" name="callerPhnNum1" class="numeric" size=1 maxlength="3" onkeyup="autoTab(this, callerPhnNum2)" style="width:20px">
						...
						<input type="text" value="" id="callerExtn" name="callerExtn" class="numeric" size=1 maxlength="4" style="width:20px">  -->
				 	  <input type="text" name="callerPhn" id="callerPhn" value="<%=util.checkNull(machObj.getCallrTelNumber())%>" size="12" maxlength="12" style="width:90px"/>&nbsp;-&nbsp;<input type="text" value="<%=util.checkNull(machObj.getCallrTelExtn()) %>" id="callerExtn" name="callerExtn" class="numeric" size=4 maxlength="5" style="width:45px"> </td></td><td  style="width:1px" nowrap>&nbsp;
...
<div id="taskDiv">		
							   ...
							   for(CanonE307TaskDetailsRec taskObj: taskList){
								   ...
								   %>
										...
										<table width="98%" class="model-table" cellspacing="1">
											...
											//String strDisabled ="";
											String strCls = "";
											String strStyle="style='width:182px;margin-left: 5px;'";
											...
											<td style="width:45%">
												<table width="97%" cellspacing="1" class="meter-read-date">
													<tr><td width=5%>Task Status </td>
														...
														%>
															<select id="taskStatDet<%=taskCnt%>" name="taskStatDet<%=taskCnt%>" <%=strCls%> <%=strStyle%>  >
															...
															<!-- 	<option value="" selected></option> -->
																...
																%>
																<option value="<%=util.checkNull(taskObj.getStrTaskStsCd()) %>"><%=util.checkNull(taskObj.getStrTaskStatus()) %></option>
																<%
															}
															 %>
															 </select> 
	...
	<tr>
		...
		<%
			...
			System.out.println("SummaryReport fsrReportList strFSRNumber : " + servObj.getStrFSRNum()+" strSerNum "+ strSerNum);
		%>
	...
	<table width="100%">
		<tr>
			<td width="92%"></td>
			<td><a class="btn" name="Close" href="javascript:closeWin();">Close</a>  </td>
			<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
		</tr>
...
function fnDebriefTask(tskNum, iVal){
	var tmZn = $('#tmZn').val();
    showPleaseWait();
	var url="canonE307ServiceRequestDebrief.jsp?debriefTsk="+tskNum+'&tmZn='+tmZn;
	$("#frmSummary").attr("action",url);	 
	$('#frmSummary').submit();	
}
...
function fnAddTask(){
	 ...
	 $('#errorWidget').hide();
	  ...
	  if((srStatus=='Completed')||(srStatus=='Pending Charge')||(srStatus=='Closed')||(srStatus=='Cancelled')||(srStatus=='Debrief Error')){
 	  ...
 	  var tmZn = $('#tmZn').val();
	  var urlDetail = 'canonE307AddTaskLov.jsp?action=open&contActFlg='+contActFlg+'&tmZn='+tmZn+"&svcMachMstrPk="+svcMachMstrPk;
			modelName = "#"+modelName;
	   showPleaseWait();
	   $(modelName).dialog({
					height: 350,
...
function searchPrefTech(pageNum) {
	...
	//assignName= "test";
	prefTechNm = encodeURIComponent(prefTechNm);
    showPleaseWait();
	var qryStr="pageNum="+pageNum+"&prefTechNm="+prefTech+"&brnchNm="+branch;
	 $.ajax({
			url:"canonE307ServReqPrefTechLov.jsp",
			data:qryStr,
			type:"POST",
...
/*		if($("#ccNum").length>0 && data.ccAccountNum){
