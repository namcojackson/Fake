 ...
 */
package com.canon.cusa.s21.batch.NSB.NSBB054001;

import static com.canon.cusa.s21.batch.NSB.NSBB054001.constant.NSBB054001Constant.*;
...
import business.db.NSBI2290_01TMsg;
...
/**
 * <pre>
 * BEI Parts Used for Service Interface Batch
 *
 * Date         Company         Name            Create/Update   Defect No 
 * ---------------------------------------------------------------------- 
 * 2016/07/06   Hitachi         Y.Osawa          CREATE          NEW
 *</pre>
 */
public class NSBB054001 extends S21BatchMain {
    ...
    public static void main(String[] args) {
        ...
        new NSBB054001().executeBatch(NSBB054001.class.getSimpleName());
    ...
    protected void mainRoutine() {
        ...
        List<NSBI2290_01TMsg> inTMsgList = new ArrayList<NSBI2290_01TMsg>();
        ...
        Map<String, Object> ssmParam = setSearchParam();
    ...
    private NSBI2290_01TMsg setNSBI2290_01(ResultSet fsrData) {

        NSBI2290_01TMsg setParamMsg = new NSBI2290_01TMsg();
        try {
            String svcRsvdTxt = "";

            // add Part Number
            svcRsvdTxt = ZYPCommonFunc.leftPad(fsrData.getString("PARTS_NUMBER"), LEN_18, SPACE);
            // add Part Description
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("PARTS_DESCRIPTION"), LEN_32, RIGHT, SPACE);
            // add Add / Sub indicator (+ or -)
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("ADD_SUB_INDICATIOR"), LEN_01, LEFT, SPACE);
            // add Quantity Used
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("USED_QTY"), LEN_04, LEFT, SPACE);
            // add Call ID Number
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("CALL_ID_NUM"), LEN_15, LEFT, SPACE);
            // add Model Number
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("MODEL_NUMBER"), LEN_30, LEFT, SPACE);
            // add Serial Number
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("SERIAL_NUMBER"), LEN_18, LEFT, SPACE);
            // add Installation Date of Part
            svcRsvdTxt = setConcatVal(svcRsvdTxt, ZYPDateUtil.DateFormatter(fsrData.getString("INSTALL_DATE"), "yyyyMMdd", "MM/dd/yyyy"), LEN_08, null, null);
            // add Meter Reading at installation of Part
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("METER_READING"), LEN_10, LEFT, ZERO);
            // add Parts Cost (Dealer Cost Extended)
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("PARTS_COST"), LEN_12, LEFT, SPACE);
            // add Branch ID
            svcRsvdTxt = setConcatVal(svcRsvdTxt, fsrData.getString("BRANCH_ID"), LEN_06, LEFT, SPACE);
    ...
    private int insertInterfaceData(List<NSBI2290_01TMsg> inMsgLst) {
        NSBI2290_01TMsg[] inMsgArray;
        inMsgArray = new NSBI2290_01TMsg[inMsgLst.size()];
    ...
    private Map<String, Object> setSearchParam() {
        Map<String, Object> setMap = new HashMap<String, Object>();
        setMap.put("glblCmpyCd", this.glblCmpyCd);
        // Get PSN Tp Cd
        ...
        if (ZYPCommonFunc.hasValue(psnTpCd)) {
            ...
            setMap.put("psnTpCdList", psnTpCdList);
        }
        setMap.put("fsrStsCd", SVC_TASK_STS.CLOSED);
        setMap.put("space", SPACE);

        List<String> svcTmEventCdList = new ArrayList<String>();
        svcTmEventCdList.add(SVC_TM_EVENT.LABOR);
        svcTmEventCdList.add(SVC_TM_EVENT.TRAVEL);
        setMap.put("svcTmEventCd", svcTmEventCdList);
        setMap.put("dateFormat", DT_FORMAT);

        if (this.dsBizProcLogTMsg != null) {
            setMap.put("cloDt", this.dsBizProcLogTMsg.dsBizLastProcTs.getValue().substring(0, VAL_8));
        }
        return setMap;
    }
}
