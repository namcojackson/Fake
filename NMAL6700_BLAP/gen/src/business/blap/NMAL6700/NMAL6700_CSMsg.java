//This file was automatically generated by Business Component Interface Definition Document (Define Business Application Global Area) and XLA200710010.
// Generated on    :20231106120032000
// Velocity macro:EZDMsg.vm V200607010
// Excel Add-in :BusinessComponentInterfaceDefinitionDocumentAddIn_XLS200710010.xla XLA200710010
/*
 *NMAL6700_CSMsg.java  Copyright  FUJITSU LIMITED 2007
 *Outline :
 *    1.
 *Release:
 *    No.  Date  Ver  Updater  Content
 *    1
 *
 */
package business.blap.NMAL6700;

import parts.common.*;
import parts.common.EZDSchemaItemDefines;

/**
 * It is NMAL6700 Business Application Global Area Message class.
 * @author
 * @version XLA200710010
 */
public class NMAL6700_CSMsg extends EZDSMsg implements EZDSchemaItemDefines {

	// Serial Version UID
	private static final long serialVersionUID = 1L;

	// Data Definition
    /** DS_ACCT_RELN_PK_C1*/
	public final EZDSBigDecimalItem              dsAcctRelnPk_C1;

    /** DS_ACCT_RELN_TP_CD_C1*/
	public final EZDSStringItemArray              dsAcctRelnTpCd_C1;

    /** DS_ACCT_RELN_TP_NM_C2*/
	public final EZDSStringItemArray              dsAcctRelnTpNm_C2;

    /** DS_ACCT_RELN_TP_CD_C3*/
	public final EZDSStringItem              dsAcctRelnTpCd_C3;

    /** SELL_TO_CUST_PK_C1*/
	public final EZDSBigDecimalItem              sellToCustPk_C1;

    /** DS_ACCT_NM_C1*/
	public final EZDSStringItem              dsAcctNm_C1;

    /** DS_ACCT_NUM_C1*/
	public final EZDSStringItem              dsAcctNum_C1;

    /** XX_CHK_BOX_C1*/
	public final EZDSStringItem              xxChkBox_C1;

    /** XX_CHK_BOX_CB*/
	public final EZDSStringItem              xxChkBox_CB;

    /** XX_CHK_BOX_CS*/
	public final EZDSStringItem              xxChkBox_CS;

    /** XX_CHK_BOX_CR*/
	public final EZDSStringItem              xxChkBox_CR;

    /** EFF_FROM_DT_C1*/
	public final EZDSDateItem              effFromDt_C1;

    /** EFF_THRU_DT_C1*/
	public final EZDSDateItem              effThruDt_C1;

    /** DS_ACCT_STS_CD_C1*/
	public final EZDSStringItem              dsAcctStsCd_C1;

    /** _EZUpdateDateTime_C1*/
	public final EZDSStringItem              ezUpTime_C1;

    /** _EZUpTimeZone_C1*/
	public final EZDSStringItem              ezUpTimeZone_C1;

    /** XX_REC_HIST_CRAT_TS_C1*/
	public final EZDSStringItem              xxRecHistCratTs_C1;

    /** XX_REC_HIST_CRAT_BY_NM_C1*/
	public final EZDSStringItem              xxRecHistCratByNm_C1;

    /** XX_REC_HIST_UPD_TS_C1*/
	public final EZDSStringItem              xxRecHistUpdTs_C1;

    /** XX_REC_HIST_UPD_BY_NM_C1*/
	public final EZDSStringItem              xxRecHistUpdByNm_C1;

    /** XX_REC_HIST_TBL_NM_C1*/
	public final EZDSStringItem              xxRecHistTblNm_C1;


	/**
	 * NMAL6700_CSMsg is constructor.
	 * The initialization when the instance of NMAL6700_CSMsg is generated.
	 */
	public NMAL6700_CSMsg() {
		this(false, -1);
	}

	/**
	 * NMAL6700_CSMsg is constructor.
	 * The initialization when the instance of NMAL6700_CSMsg is generated.
	 * @param child  Flag whether it is child message
	 * @param eleNo  Index Number of array
	 */
	public NMAL6700_CSMsg(boolean child, int eleNo) {
		super(child, eleNo);

		// Initialization of item

		dsAcctRelnPk_C1 = (EZDSBigDecimalItem)newItem("dsAcctRelnPk_C1");
		dsAcctRelnTpCd_C1 = (EZDSStringItemArray)newItemArray("dsAcctRelnTpCd_C1");
		dsAcctRelnTpNm_C2 = (EZDSStringItemArray)newItemArray("dsAcctRelnTpNm_C2");
		dsAcctRelnTpCd_C3 = (EZDSStringItem)newItem("dsAcctRelnTpCd_C3");
		sellToCustPk_C1 = (EZDSBigDecimalItem)newItem("sellToCustPk_C1");
		dsAcctNm_C1 = (EZDSStringItem)newItem("dsAcctNm_C1");
		dsAcctNum_C1 = (EZDSStringItem)newItem("dsAcctNum_C1");
		xxChkBox_C1 = (EZDSStringItem)newItem("xxChkBox_C1");
		xxChkBox_CB = (EZDSStringItem)newItem("xxChkBox_CB");
		xxChkBox_CS = (EZDSStringItem)newItem("xxChkBox_CS");
		xxChkBox_CR = (EZDSStringItem)newItem("xxChkBox_CR");
		effFromDt_C1 = (EZDSDateItem)newItem("effFromDt_C1");
		effThruDt_C1 = (EZDSDateItem)newItem("effThruDt_C1");
		dsAcctStsCd_C1 = (EZDSStringItem)newItem("dsAcctStsCd_C1");
		ezUpTime_C1 = (EZDSStringItem)newItem("ezUpTime_C1");
		ezUpTimeZone_C1 = (EZDSStringItem)newItem("ezUpTimeZone_C1");
		xxRecHistCratTs_C1 = (EZDSStringItem)newItem("xxRecHistCratTs_C1");
		xxRecHistCratByNm_C1 = (EZDSStringItem)newItem("xxRecHistCratByNm_C1");
		xxRecHistUpdTs_C1 = (EZDSStringItem)newItem("xxRecHistUpdTs_C1");
		xxRecHistUpdByNm_C1 = (EZDSStringItem)newItem("xxRecHistUpdByNm_C1");
		xxRecHistTblNm_C1 = (EZDSStringItem)newItem("xxRecHistTblNm_C1");
	}

	/**
	 * get the type of array which is stored
	 * @return NMAL6700_CSMsgArray
	 */
	public EZDMsgArray getMyArray() {
		return new NMAL6700_CSMsgArray();
	}


	/**
	 * Array of schema data(Basic data)
	 */
	private static final String[][] BASE_CONTENTS = {

	{"dsAcctRelnPk_C1", "dsAcctRelnPk_C1", "C1", null, TYPE_SEISU_SYOSU, "28", "0"},
	{"dsAcctRelnTpCd_C1", "dsAcctRelnTpCd_C1", "C1", "99", TYPE_HANKAKUEISU, "2", null},
	{"dsAcctRelnTpNm_C2", "dsAcctRelnTpNm_C2", "C2", "99", TYPE_HANKAKUEISU, "20", null},
	{"dsAcctRelnTpCd_C3", "dsAcctRelnTpCd_C3", "C3", null, TYPE_HANKAKUEISU, "2", null},
	{"sellToCustPk_C1", "sellToCustPk_C1", "C1", null, TYPE_SEISU_SYOSU, "28", "0"},
	{"dsAcctNm_C1", "dsAcctNm_C1", "C1", null, TYPE_HANKAKUEISU, "360", null},
	{"dsAcctNum_C1", "dsAcctNum_C1", "C1", null, TYPE_HANKAKUEISU, "20", null},
	{"xxChkBox_C1", "xxChkBox_C1", "C1", null, TYPE_HANKAKUEIJI, "1", null},
	{"xxChkBox_CB", "xxChkBox_CB", "CB", null, TYPE_HANKAKUEIJI, "1", null},
	{"xxChkBox_CS", "xxChkBox_CS", "CS", null, TYPE_HANKAKUEIJI, "1", null},
	{"xxChkBox_CR", "xxChkBox_CR", "CR", null, TYPE_HANKAKUEIJI, "1", null},
	{"effFromDt_C1", "effFromDt_C1", "C1", null, TYPE_NENTSUKIHI, "8", null},
	{"effThruDt_C1", "effThruDt_C1", "C1", null, TYPE_NENTSUKIHI, "8", null},
	{"dsAcctStsCd_C1", "dsAcctStsCd_C1", "C1", null, TYPE_HANKAKUEISU, "3", null},
	{"ezUpTime_C1", "ezUpTime_C1", "C1", null, TYPE_HANKAKUEISU, "17", null},
	{"ezUpTimeZone_C1", "ezUpTimeZone_C1", "C1", null, TYPE_HANKAKUEISU, "5", null},
	{"xxRecHistCratTs_C1", "xxRecHistCratTs_C1", "C1", null, TYPE_HANKAKUSUJI, "17", null},
	{"xxRecHistCratByNm_C1", "xxRecHistCratByNm_C1", "C1", null, TYPE_HANKAKUEISU, "150", null},
	{"xxRecHistUpdTs_C1", "xxRecHistUpdTs_C1", "C1", null, TYPE_HANKAKUSUJI, "17", null},
	{"xxRecHistUpdByNm_C1", "xxRecHistUpdByNm_C1", "C1", null, TYPE_HANKAKUEISU, "150", null},
	{"xxRecHistTblNm_C1", "xxRecHistTblNm_C1", "C1", null, TYPE_HANKAKUEISU, "60", null},
	};

	/**
	 * Array of schema data(Visible Field)
	 */
	private static final String[][] DISP_CONTENTS = {

        {"DS_ACCT_RELN_PK",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctRelnPk_C1
        {"DS_ACCT_RELN_TP_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctRelnTpCd_C1
        {"DS_ACCT_RELN_TP_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctRelnTpNm_C2
        {"DS_ACCT_RELN_TP_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctRelnTpCd_C3
        {"SELL_TO_CUST_PK",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//sellToCustPk_C1
        {"DS_ACCT_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctNm_C1
        {"DS_ACCT_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctNum_C1
        {"XX_CHK_BOX",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxChkBox_C1
        {"XX_CHK_BOX",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxChkBox_CB
        {"XX_CHK_BOX",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxChkBox_CS
        {"XX_CHK_BOX",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxChkBox_CR
        {"EFF_FROM_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//effFromDt_C1
        {"EFF_THRU_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//effThruDt_C1
        {"DS_ACCT_STS_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dsAcctStsCd_C1
        {"_EZUpdateDateTime",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//ezUpTime_C1
        {"_EZUpTimeZone",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//ezUpTimeZone_C1
        {"XX_REC_HIST_CRAT_TS",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistCratTs_C1
        {"XX_REC_HIST_CRAT_BY_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistCratByNm_C1
        {"XX_REC_HIST_UPD_TS",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistUpdTs_C1
        {"XX_REC_HIST_UPD_BY_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistUpdByNm_C1
        {"XX_REC_HIST_TBL_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistTblNm_C1
	};

	/**
	 * get Array of common (basic) data.
	 * @return Array of common (basis) data
	 */
	protected String[][] getBaseContents() {
		return BASE_CONTENTS;
	}

	/**
	 * get Array of Display Field.
	 * @return Array of  Display  Fields
	 */
	protected String[][] getDispContents() {
		return DISP_CONTENTS;
	}

}

