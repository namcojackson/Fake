 ...
 *</pre>
 */
public class NMAL0150_NMAL6800 extends S21CommonHandler implements NMAL0150Constant {
	...
	protected EZDCMsg setRequestData(EZDApplicationContext ctx, EZDBMsg bMsg) {
		...
		if (!CMN_CLOSE.equals(getLastGuard())) {
			if ("NMAL0150Scrn00_LineItem".equals(event)) {
				...
				bizMsg.A.no(row).mdseDescShortTxt_A1.setValue(scrnMsg.P.no(7).xxPopPrm.getValue());
			}
		}
 		return bizMsg;
	...
	protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
		...
		if (!CMN_CLOSE.equals(getLastGuard())) {
			if ("NMAL0150Scrn00_LineItem".equals(event)) {
				...
				scrnMsg.A.no(row).mdseDescShortTxt_A1.setValue(scrnMsg.P.no(7).xxPopPrm.getValue());
