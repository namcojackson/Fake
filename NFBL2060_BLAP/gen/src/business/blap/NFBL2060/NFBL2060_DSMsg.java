//This file was automatically generated by Business Component Interface Definition Document (Define Business Application Global Area) and XLA200710010.
// Generated on    :20240202111126000
// Velocity macro:EZDMsg.vm V200607010
// Excel Add-in :BusinessComponentInterfaceDefinitionDocumentAddIn_XLS200710010.xla XLA200710010
/*
 *NFBL2060_DSMsg.java  Copyright  FUJITSU LIMITED 2007
 *Outline :
 *    1.
 *Release:
 *    No.  Date  Ver  Updater  Content
 *    1
 *
 */
package business.blap.NFBL2060;

import parts.common.*;
import parts.common.EZDSchemaItemDefines;

/**
 * It is NFBL2060 Business Application Global Area Message class.
 * @author
 * @version XLA200710010
 */
public class NFBL2060_DSMsg extends EZDSMsg implements EZDSchemaItemDefines {

	// Serial Version UID
	private static final long serialVersionUID = 1L;

	// Data Definition
    /** AP_INV_CATG_DESC_TXT_D1*/
	public final EZDSStringItem              apInvCatgDescTxt_D1;

    /** XX_LINK_PROT_D1*/
	public final EZDSStringItem              xxLinkProt_D1;

    /** XX_LINK_PROT_D2*/
	public final EZDSStringItem              xxLinkProt_D2;

    /** AP_VND_INV_NUM_D1*/
	public final EZDSStringItem              apVndInvNum_D1;

    /** AP_VND_INV_SQ_NUM_D1*/
	public final EZDSStringItem              apVndInvSqNum_D1;

    /** INV_DT_D1*/
	public final EZDSDateItem              invDt_D1;

    /** INV_AUTH_DT_D1*/
	public final EZDSDateItem              invAuthDt_D1;

    /** PRNT_VND_CD_D1*/
	public final EZDSStringItem              prntVndCd_D1;

    /** DPLY_VND_NM_D1*/
	public final EZDSStringItem              dplyVndNm_D1;

    /** AP_VND_CD_D1*/
	public final EZDSStringItem              apVndCd_D1;

    /** VND_PMT_TERM_DESC_TXT_D1*/
	public final EZDSStringItem              vndPmtTermDescTxt_D1;

    /** AC_INV_TOT_COST_AMT_D1*/
	public final EZDSBigDecimalItem              acInvTotCostAmt_D1;

    /** INV_HDR_DESC_TXT_D1*/
	public final EZDSStringItem              invHdrDescTxt_D1;

    /** ACCT_INV_STS_CD_D1*/
	public final EZDSStringItem              acctInvStsCd_D1;

    /** ACCT_INV_STS_DESC_TXT_D1*/
	public final EZDSStringItem              acctInvStsDescTxt_D1;

    /** PO_NUM_D1*/
	public final EZDSStringItem              poNum_D1;

    /** PO_DT_D1*/
	public final EZDSDateItem              poDt_D1;

    /** VND_RTRN_NUM_D1*/
	public final EZDSStringItem              vndRtrnNum_D1;

    /** VND_RTRN_SUBMT_DT_D1*/
	public final EZDSDateItem              vndRtrnSubmtDt_D1;

    /** DELY_ORD_NUM_D1*/
	public final EZDSStringItem              delyOrdNum_D1;

    /** CM_INV_ACCT_DIST_LINE_NUM_D1*/
	public final EZDSStringItem              cmInvAcctDistLineNum_D1;

    /** AP_LINE_TP_DESC_TXT_D1*/
	public final EZDSStringItem              apLineTpDescTxt_D1;

    /** MDSE_DESC_SHORT_TXT_D1*/
	public final EZDSStringItem              mdseDescShortTxt_D1;

    /** AC_INV_JRNL_COST_AMT_D1*/
	public final EZDSBigDecimalItem              acInvJrnlCostAmt_D1;

    /** AC_INV_TAX_AMT_D1*/
	public final EZDSBigDecimalItem              acInvTaxAmt_D1;

    /** XX_FLD_VAL_TXT_D1*/
	public final EZDSStringItem              xxFldValTxt_D1;

    /** XX_CMNT_TXT_D2*/
	public final EZDSStringItem              xxCmntTxt_D2;

    /** INV_QTY_D1*/
	public final EZDSBigDecimalItem              invQty_D1;

    /** INV_RCV_QTY_D1*/
	public final EZDSBigDecimalItem              invRcvQty_D1;

    /** AP_BILL_QTY_D1*/
	public final EZDSBigDecimalItem              apBillQty_D1;

    /** AP_PMT_CHK_NUM_D1*/
	public final EZDSStringItem              apPmtChkNum_D1;

    /** PMT_DT_D1*/
	public final EZDSDateItem              pmtDt_D1;

    /** CONTR_NUM_D1*/
	public final EZDSStringItem              contrNum_D1;

    /** CUST_DLR_CD_D1*/
	public final EZDSStringItem              custDlrCd_D1;

    /** SER_NUM_D1*/
	public final EZDSStringItem              serNum_D1;

    /** CSMP_INV_NUM_D1*/
	public final EZDSStringItem              csmpInvNum_D1;

    /** ISTL_LOC_CTY_ADDR_D1*/
	public final EZDSStringItem              istlLocCtyAddr_D1;

    /** XX_REC_HIST_CRAT_TS_D1*/
	public final EZDSStringItem              xxRecHistCratTs_D1;

    /** XX_REC_HIST_CRAT_BY_NM_D1*/
	public final EZDSStringItem              xxRecHistCratByNm_D1;

    /** XX_REC_HIST_UPD_TS_D1*/
	public final EZDSStringItem              xxRecHistUpdTs_D1;

    /** XX_REC_HIST_UPD_BY_NM_D1*/
	public final EZDSStringItem              xxRecHistUpdByNm_D1;

    /** XX_REC_HIST_TBL_NM_D1*/
	public final EZDSStringItem              xxRecHistTblNm_D1;

    /** AP_VND_INV_LINE_NUM_D1*/
	public final EZDSStringItem              apVndInvLineNum_D1;

    /** DISP_PO_DTL_LINE_NUM_D1*/
	public final EZDSStringItem              dispPoDtlLineNum_D1;


	/**
	 * NFBL2060_DSMsg is constructor.
	 * The initialization when the instance of NFBL2060_DSMsg is generated.
	 */
	public NFBL2060_DSMsg() {
		this(false, -1);
	}

	/**
	 * NFBL2060_DSMsg is constructor.
	 * The initialization when the instance of NFBL2060_DSMsg is generated.
	 * @param child  Flag whether it is child message
	 * @param eleNo  Index Number of array
	 */
	public NFBL2060_DSMsg(boolean child, int eleNo) {
		super(child, eleNo);

		// Initialization of item

		apInvCatgDescTxt_D1 = (EZDSStringItem)newItem("apInvCatgDescTxt_D1");
		xxLinkProt_D1 = (EZDSStringItem)newItem("xxLinkProt_D1");
		xxLinkProt_D2 = (EZDSStringItem)newItem("xxLinkProt_D2");
		apVndInvNum_D1 = (EZDSStringItem)newItem("apVndInvNum_D1");
		apVndInvSqNum_D1 = (EZDSStringItem)newItem("apVndInvSqNum_D1");
		invDt_D1 = (EZDSDateItem)newItem("invDt_D1");
		invAuthDt_D1 = (EZDSDateItem)newItem("invAuthDt_D1");
		prntVndCd_D1 = (EZDSStringItem)newItem("prntVndCd_D1");
		dplyVndNm_D1 = (EZDSStringItem)newItem("dplyVndNm_D1");
		apVndCd_D1 = (EZDSStringItem)newItem("apVndCd_D1");
		vndPmtTermDescTxt_D1 = (EZDSStringItem)newItem("vndPmtTermDescTxt_D1");
		acInvTotCostAmt_D1 = (EZDSBigDecimalItem)newItem("acInvTotCostAmt_D1");
		invHdrDescTxt_D1 = (EZDSStringItem)newItem("invHdrDescTxt_D1");
		acctInvStsCd_D1 = (EZDSStringItem)newItem("acctInvStsCd_D1");
		acctInvStsDescTxt_D1 = (EZDSStringItem)newItem("acctInvStsDescTxt_D1");
		poNum_D1 = (EZDSStringItem)newItem("poNum_D1");
		poDt_D1 = (EZDSDateItem)newItem("poDt_D1");
		vndRtrnNum_D1 = (EZDSStringItem)newItem("vndRtrnNum_D1");
		vndRtrnSubmtDt_D1 = (EZDSDateItem)newItem("vndRtrnSubmtDt_D1");
		delyOrdNum_D1 = (EZDSStringItem)newItem("delyOrdNum_D1");
		cmInvAcctDistLineNum_D1 = (EZDSStringItem)newItem("cmInvAcctDistLineNum_D1");
		apLineTpDescTxt_D1 = (EZDSStringItem)newItem("apLineTpDescTxt_D1");
		mdseDescShortTxt_D1 = (EZDSStringItem)newItem("mdseDescShortTxt_D1");
		acInvJrnlCostAmt_D1 = (EZDSBigDecimalItem)newItem("acInvJrnlCostAmt_D1");
		acInvTaxAmt_D1 = (EZDSBigDecimalItem)newItem("acInvTaxAmt_D1");
		xxFldValTxt_D1 = (EZDSStringItem)newItem("xxFldValTxt_D1");
		xxCmntTxt_D2 = (EZDSStringItem)newItem("xxCmntTxt_D2");
		invQty_D1 = (EZDSBigDecimalItem)newItem("invQty_D1");
		invRcvQty_D1 = (EZDSBigDecimalItem)newItem("invRcvQty_D1");
		apBillQty_D1 = (EZDSBigDecimalItem)newItem("apBillQty_D1");
		apPmtChkNum_D1 = (EZDSStringItem)newItem("apPmtChkNum_D1");
		pmtDt_D1 = (EZDSDateItem)newItem("pmtDt_D1");
		contrNum_D1 = (EZDSStringItem)newItem("contrNum_D1");
		custDlrCd_D1 = (EZDSStringItem)newItem("custDlrCd_D1");
		serNum_D1 = (EZDSStringItem)newItem("serNum_D1");
		csmpInvNum_D1 = (EZDSStringItem)newItem("csmpInvNum_D1");
		istlLocCtyAddr_D1 = (EZDSStringItem)newItem("istlLocCtyAddr_D1");
		xxRecHistCratTs_D1 = (EZDSStringItem)newItem("xxRecHistCratTs_D1");
		xxRecHistCratByNm_D1 = (EZDSStringItem)newItem("xxRecHistCratByNm_D1");
		xxRecHistUpdTs_D1 = (EZDSStringItem)newItem("xxRecHistUpdTs_D1");
		xxRecHistUpdByNm_D1 = (EZDSStringItem)newItem("xxRecHistUpdByNm_D1");
		xxRecHistTblNm_D1 = (EZDSStringItem)newItem("xxRecHistTblNm_D1");
		apVndInvLineNum_D1 = (EZDSStringItem)newItem("apVndInvLineNum_D1");
		dispPoDtlLineNum_D1 = (EZDSStringItem)newItem("dispPoDtlLineNum_D1");
	}

	/**
	 * get the type of array which is stored
	 * @return NFBL2060_DSMsgArray
	 */
	public EZDMsgArray getMyArray() {
		return new NFBL2060_DSMsgArray();
	}


	/**
	 * Array of schema data(Basic data)
	 */
	private static final String[][] BASE_CONTENTS = {

	{"apInvCatgDescTxt_D1", "apInvCatgDescTxt_D1", "D1", null, TYPE_HANKAKUEISU, "50", null},
	{"xxLinkProt_D1", "xxLinkProt_D1", "D1", null, TYPE_HANKAKUEISU, "1", null},
	{"xxLinkProt_D2", "xxLinkProt_D2", "D2", null, TYPE_HANKAKUEISU, "1", null},
	{"apVndInvNum_D1", "apVndInvNum_D1", "D1", null, TYPE_HANKAKUEISU, "15", null},
	{"apVndInvSqNum_D1", "apVndInvSqNum_D1", "D1", null, TYPE_HANKAKUEISU, "3", null},
	{"invDt_D1", "invDt_D1", "D1", null, TYPE_NENTSUKIHI, "8", null},
	{"invAuthDt_D1", "invAuthDt_D1", "D1", null, TYPE_NENTSUKIHI, "8", null},
	{"prntVndCd_D1", "prntVndCd_D1", "D1", null, TYPE_HANKAKUEISU, "30", null},
	{"dplyVndNm_D1", "dplyVndNm_D1", "D1", null, TYPE_HANKAKUEISU, "320", null},
	{"apVndCd_D1", "apVndCd_D1", "D1", null, TYPE_HANKAKUEISU, "20", null},
	{"vndPmtTermDescTxt_D1", "vndPmtTermDescTxt_D1", "D1", null, TYPE_HANKAKUEISU, "50", null},
	{"acInvTotCostAmt_D1", "acInvTotCostAmt_D1", "D1", null, TYPE_SEISU_SYOSU, "19", "4"},
	{"invHdrDescTxt_D1", "invHdrDescTxt_D1", "D1", null, TYPE_HANKAKUEISU, "240", null},
	{"acctInvStsCd_D1", "acctInvStsCd_D1", "D1", null, TYPE_HANKAKUEISU, "2", null},
	{"acctInvStsDescTxt_D1", "acctInvStsDescTxt_D1", "D1", null, TYPE_HANKAKUEISU, "50", null},
	{"poNum_D1", "poNum_D1", "D1", null, TYPE_HANKAKUEISU, "35", null},
	{"poDt_D1", "poDt_D1", "D1", null, TYPE_NENTSUKIHI, "8", null},
	{"vndRtrnNum_D1", "vndRtrnNum_D1", "D1", null, TYPE_HANKAKUEISU, "8", null},
	{"vndRtrnSubmtDt_D1", "vndRtrnSubmtDt_D1", "D1", null, TYPE_NENTSUKIHI, "8", null},
	{"delyOrdNum_D1", "delyOrdNum_D1", "D1", null, TYPE_HANKAKUEISU, "20", null},
	{"cmInvAcctDistLineNum_D1", "cmInvAcctDistLineNum_D1", "D1", null, TYPE_HANKAKUEISU, "3", null},
	{"apLineTpDescTxt_D1", "apLineTpDescTxt_D1", "D1", null, TYPE_HANKAKUEISU, "50", null},
	{"mdseDescShortTxt_D1", "mdseDescShortTxt_D1", "D1", null, TYPE_HANKAKUEISU, "250", null},
	{"acInvJrnlCostAmt_D1", "acInvJrnlCostAmt_D1", "D1", null, TYPE_SEISU_SYOSU, "19", "4"},
	{"acInvTaxAmt_D1", "acInvTaxAmt_D1", "D1", null, TYPE_SEISU_SYOSU, "19", "4"},
	{"xxFldValTxt_D1", "xxFldValTxt_D1", "D1", null, TYPE_HANKAKUEISU, "1000", null},
	{"xxCmntTxt_D2", "xxCmntTxt_D2", "D2", null, TYPE_HANKAKUEISU, "60", null},
	{"invQty_D1", "invQty_D1", "D1", null, TYPE_SEISU_SYOSU, "10", "0"},
	{"invRcvQty_D1", "invRcvQty_D1", "D1", null, TYPE_SEISU_SYOSU, "10", "0"},
	{"apBillQty_D1", "apBillQty_D1", "D1", null, TYPE_SEISU_SYOSU, "10", "0"},
	{"apPmtChkNum_D1", "apPmtChkNum_D1", "D1", null, TYPE_HANKAKUEISU, "15", null},
	{"pmtDt_D1", "pmtDt_D1", "D1", null, TYPE_NENTSUKIHI, "8", null},
	{"contrNum_D1", "contrNum_D1", "D1", null, TYPE_HANKAKUEISU, "30", null},
	{"custDlrCd_D1", "custDlrCd_D1", "D1", null, TYPE_HANKAKUEISU, "30", null},
	{"serNum_D1", "serNum_D1", "D1", null, TYPE_HANKAKUEISU, "30", null},
	{"csmpInvNum_D1", "csmpInvNum_D1", "D1", null, TYPE_HANKAKUEISU, "100", null},
	{"istlLocCtyAddr_D1", "istlLocCtyAddr_D1", "D1", null, TYPE_HANKAKUEISU, "25", null},
	{"xxRecHistCratTs_D1", "xxRecHistCratTs_D1", "D1", null, TYPE_HANKAKUSUJI, "17", null},
	{"xxRecHistCratByNm_D1", "xxRecHistCratByNm_D1", "D1", null, TYPE_HANKAKUEISU, "150", null},
	{"xxRecHistUpdTs_D1", "xxRecHistUpdTs_D1", "D1", null, TYPE_HANKAKUSUJI, "17", null},
	{"xxRecHistUpdByNm_D1", "xxRecHistUpdByNm_D1", "D1", null, TYPE_HANKAKUEISU, "150", null},
	{"xxRecHistTblNm_D1", "xxRecHistTblNm_D1", "D1", null, TYPE_HANKAKUEISU, "60", null},
	{"apVndInvLineNum_D1", "apVndInvLineNum_D1", "D1", null, TYPE_HANKAKUEISU, "4", null},
	{"dispPoDtlLineNum_D1", "dispPoDtlLineNum_D1", "D1", null, TYPE_HANKAKUEISU, "8", null},
	};

	/**
	 * Array of schema data(Visible Field)
	 */
	private static final String[][] DISP_CONTENTS = {

        {"AP_INV_CATG_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apInvCatgDescTxt_D1
        {"XX_LINK_PROT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxLinkProt_D1
        {"XX_LINK_PROT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxLinkProt_D2
        {"AP_VND_INV_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apVndInvNum_D1
        {"AP_VND_INV_SQ_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apVndInvSqNum_D1
        {"INV_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//invDt_D1
        {"INV_AUTH_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//invAuthDt_D1
        {"PRNT_VND_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//prntVndCd_D1
        {"DPLY_VND_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dplyVndNm_D1
        {"AP_VND_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apVndCd_D1
        {"VND_PMT_TERM_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//vndPmtTermDescTxt_D1
        {"AC_INV_TOT_COST_AMT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//acInvTotCostAmt_D1
        {"INV_HDR_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//invHdrDescTxt_D1
        {"ACCT_INV_STS_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//acctInvStsCd_D1
        {"ACCT_INV_STS_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//acctInvStsDescTxt_D1
        {"PO_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//poNum_D1
        {"PO_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//poDt_D1
        {"VND_RTRN_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//vndRtrnNum_D1
        {"VND_RTRN_SUBMT_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//vndRtrnSubmtDt_D1
        {"DELY_ORD_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//delyOrdNum_D1
        {"CM_INV_ACCT_DIST_LINE_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//cmInvAcctDistLineNum_D1
        {"AP_LINE_TP_DESC_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apLineTpDescTxt_D1
        {"MDSE_DESC_SHORT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//mdseDescShortTxt_D1
        {"AC_INV_JRNL_COST_AMT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//acInvJrnlCostAmt_D1
        {"AC_INV_TAX_AMT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//acInvTaxAmt_D1
        {"XX_FLD_VAL_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxFldValTxt_D1
        {"XX_CMNT_TXT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxCmntTxt_D2
        {"INV_QTY",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//invQty_D1
        {"INV_RCV_QTY",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//invRcvQty_D1
        {"AP_BILL_QTY",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apBillQty_D1
        {"AP_PMT_CHK_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apPmtChkNum_D1
        {"PMT_DT",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//pmtDt_D1
        {"CONTR_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//contrNum_D1
        {"CUST_DLR_CD",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//custDlrCd_D1
        {"SER_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//serNum_D1
        {"CSMP_INV_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//csmpInvNum_D1
        {"ISTL_LOC_CTY_ADDR",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//istlLocCtyAddr_D1
        {"XX_REC_HIST_CRAT_TS",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistCratTs_D1
        {"XX_REC_HIST_CRAT_BY_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistCratByNm_D1
        {"XX_REC_HIST_UPD_TS",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistUpdTs_D1
        {"XX_REC_HIST_UPD_BY_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistUpdByNm_D1
        {"XX_REC_HIST_TBL_NM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//xxRecHistTblNm_D1
        {"AP_VND_INV_LINE_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//apVndInvLineNum_D1
        {"DISP_PO_DTL_LINE_NUM",  NO,  null,null,null, NO, NO, NO, NO,null,null,null, null,  NO,  NO},	//dispPoDtlLineNum_D1
	};

	/**
	 * get Array of common (basic) data.
	 * @return Array of common (basis) data
	 */
	protected String[][] getBaseContents() {
		return BASE_CONTENTS;
	}

	/**
	 * get Array of Display Field.
	 * @return Array of  Display  Fields
	 */
	protected String[][] getDispContents() {
		return DISP_CONTENTS;
	}

}

