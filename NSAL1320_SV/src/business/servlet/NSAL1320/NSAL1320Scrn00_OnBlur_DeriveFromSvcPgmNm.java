...
/**
 *<pre>
 * NSAL1320Scrn00_OnBlur_DeriveFromSvcPgmNm
 *
 ...
 *</pre>
 */
public class NSAL1320Scrn00_OnBlur_DeriveFromSvcPgmNm extends S21CommonHandler {
    ...
    protected void checkInput(EZDApplicationContext ctx, EZDBMsg bMsg) {
        ...
        if (ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).mdseDescShortTxt)) {
            scrnMsg.addCheckItem(scrnMsg.A.no(selectIndex).mdseDescShortTxt);
    ...
    protected void doProcess(EZDApplicationContext ctx, EZDBMsg bMsg, EZDCMsg cMsg) {
        ...
        int selectIndex = getButtonSelectNumber();
        if (!ZYPCommonFunc.hasValue(scrnMsg.A.no(selectIndex).mdseDescShortTxt)) {
            scrnMsg.A.no(selectIndex).svcPgmMdseCd.clear();
            return;
        }
        ...
        if (ZYPConstant.FLG_ON_Y.equals(scrnMsg.xxRqstFlg.getValue())) {
            ...
            Object[] params = NSAL1320CommonLogic.getNWAL1130Prm(scrnMsg, getButtonSelectNumber());
            setArgForSubScreen(params);
        }
        scrnMsg.setFocusItem(scrnMsg.A.no(selectIndex).prcSvcContrTpCd);
    }
}
